...
SmtpClient::SmtpClient(int sockfd, const char* mx_server_name) {
  ...
  while (1) {
    ...
    int res =
        m_lsockfd->lrecv(welstr, 4095, CMailBase::m_connection_idle_timeout);
    if (res <= 0) {
      break;
    }
    if (welstr[3] == ' ') {
...
BOOL SmtpClient::Do_Auth_Command(string& strmsg) {
  ...
  if ((SendCmd(m_sockfd, cmd, strlen(cmd)) == 0) && (RecvReply(cmd, strmsg)) &&
      ((strncmp(cmd, "334", 3) == 0))) {
    return TRUE;
  } else {
    return FALSE;
...
BOOL SmtpClient::Do_User_Command(char* user, string& strmsg) {
  ...
  int outlen =
      BASE64_ENCODE_OUTPUT_MAX_LEN(strlen(user));  // strlen(user)*4 + 1;
...
BOOL SmtpClient::Do_Password_Command(char* password, string& strmsg) {
  ...
  int outlen = BASE64_ENCODE_OUTPUT_MAX_LEN(
      strlen(password));  // strlen(password)*4 + 1;
  char* cmd = (char*)malloc(outlen);
  memset(cmd, 0, outlen);
...
BOOL SmtpClient::Do_StartTLS_Command(string& strmsg) {
  ...
  if ((SendCmd(m_sockfd, cmd, strlen(cmd)) == 0) && (RecvReply(cmd, strmsg)) &&
    ...
    if (stat(ca_crt_root.c_str(), &file_stat) == 0 &&
      ...
      if (connect_ssl(m_sockfd, ca_crt_root.c_str(), ca_crt_client.c_str(),
                      ca_password.c_str(), ca_key_client.c_str(), &m_ssl,
                      &m_ssl_ctx,
                      CMailBase::m_connection_sync_timeout) == FALSE) {
        return FALSE;
      }
    } else {
      if (connect_ssl(m_sockfd, NULL, NULL, NULL, NULL, &m_ssl, &m_ssl_ctx,
                      CMailBase::m_connection_sync_timeout) == FALSE) {
        return FALSE;
      }
    }
...
int SmtpClient::SendCmd(int sockfd, const char* buf, unsigned int buf_len) {
  if (CMailBase::m_smtp_client_trace)
...
BOOL SmtpClient::RecvReply(char* reply_code, string& strmsg) {
  ...
  while (1) {
    ...
    if (m_bInTLS) {
      ...
      n_recv =
          m_lsockfd->lrecv(str, 4095, CMailBase::m_connection_idle_timeout);
    }
    if (n_recv < 0) {
      return FALSE;
    } else {
