  ...
  formdata(const char* buf, int buflen, const char* boundary) {
    ...
    while (x < buflen) {
      if ((memcmp(buf + x, sboundary.c_str(), sboundary.length()) == 0) &&
        ...
        if ((parmeter.m_seg.m_byte_beg == 0) &&
            (parmeter.m_seg.m_byte_end == 0)) {
...
class CHttp : public CMailBase {
 public:
  ...
  BOOL EnableKeepAlive(BOOL keep_alive) {
    m_enabled_keep_alive = keep_alive;
    return TRUE;
  }

  virtual BOOL IsEnabledKeepAlive() {
    return m_enabled_keep_alive;
    return TRUE;
  }
