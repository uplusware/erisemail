...
BOOL Session_Group::Accept(int sockfd,
                           SSL* ssl,
                           SSL_CTX* ssl_ctx,
                           const char* clientip,
                           Service_Type st,
  ...
  Session_Info* pSessionInstance =
      new Session_Info(this, st, m_epoll_fd, sockfd, ssl, ssl_ctx, clientip,
                       storage_engine, memcached, is_ssl);
  if (!pSessionInstance)
    return FALSE;
...
BOOL Session_Group::Connect(const char* hostname,
                            unsigned short port,
                            Service_Type st,
                            StorageEngine* storage_engine,
                            memory_cache* ch,
  ...
  if (isConnectOK == TRUE) {
    Session_Info* pSessionInstance = new Session_Info(
        this, st, m_epoll_fd, sockfd, NULL, NULL, realip, storage_engine,
        memcached, FALSE, TRUE, hostname, isXmppDialBack);
    if (!pSessionInstance)
      return FALSE;
    ...
    if (m_events[i].events & EPOLLIN) {
          ...
          do {
            ...
            if (new_line != std::string::npos) {
              ...
              if (!pSessionInstance->GetProtocol()->Parse(
                      (char*)str_left.c_str(), str_left.length())) {
                break;
              }
            }
      ...
      if (m_session_list.find(m_events[i].data.fd) != m_session_list.end() &&
        ...
        if (!pSessionInstance->GetProtocol() ||
            !pSessionInstance->CreateProtocol()) {
          return -1;
        }
