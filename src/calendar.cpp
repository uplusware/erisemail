		...
		if(m_phase == eNull && strncasecmp(calbuf.c_str(), "BEGIN:VCALENDAR", sizeof("BEGIN:VCALENDAR") - 1) == 0)
		{
			...
			else if(m_phase == eCal && strncasecmp(calbuf.c_str(), "BEGIN:VTIMEZONE", sizeof("BEGIN:VTIMEZONE") - 1) == 0)
			{
				...
				else if(m_phase == eTimeZone && strncasecmp(calbuf.c_str(), "BEGIN:STANDARD", sizeof("BEGIN:STANDARD") - 1) == 0)
				{
					...
					else if(m_phase == eStandard && strncasecmp(calbuf.c_str(), "DTSTART:", sizeof("DTSTART:") - 1) == 0)
					{
						...
						sscanf(strTmp.c_str(), "%04d%02d%02dT%02d%02d%02dZ", &dtTmp.tm_year, &dtTmp.tm_mon, &dtTmp.tm_mday, 
							&dtTmp.tm_hour, &dtTmp.tm_min, &dtTmp.tm_sec);
						dtTmp.tm_year -= 1900;
						dtTmp.tm_mon -= 1;
			...
			else if(m_phase == eCal &&strncasecmp(calbuf.c_str(), "BEGIN:VEVENT", sizeof("BEGIN:VEVENT") - 1) == 0)
			{
				...
				else if(m_phase == eEvent &&strncasecmp(calbuf.c_str(), "DTSTART;", sizeof("DTSTART;") - 1) == 0)
				{
					...
					sscanf(strTmp.c_str(), "%04d%02d%02dT%02d%02d%02d", &dtTmp.tm_year, &dtTmp.tm_mon, &dtTmp.tm_mday,
						&dtTmp.tm_hour, &dtTmp.tm_min, &dtTmp.tm_sec);
					dtTmp.tm_year -= 1900;
					dtTmp.tm_mon -= 1;
...
void Calendar::save()
{
	...
	for(int i = 0; i < m_details.size(); i++)
	{
		...
		for(int x = 0; x < m_details[i].events.size(); x++)
		{
			...
			int a = sprintf(calpath, "%s/cal/%s", CMailBase::m_private_path.c_str(), m_owner.c_str());
			mkdir(calpath, 0744);
			...
			if(m_file)
			{
				fwrite(m_details[i].text.c_str(), m_details[i].text.length(), 1, m_file);
				fclose(m_file);
			}
		}
	}
