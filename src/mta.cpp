  ...
  while (!m_s_relay_stop) {
    ...
    if (sem_timedwait(&m_s_thread_pool_sem, &ts) == 0) {
      ...
      if (reply_info) {
        ...
        StorageEngineInstance stgengine_instance(reply_info->_storageEngine,
                                                 &mailStg);
        if (!mailStg) {
          continue;
        }
        ...
        if (RelayMail(mailStg, reply_info->_memcached,
                      reply_info->mail_info.mid, reply_info->mail_info.uniqid,
                      reply_info->mail_info.mailfrom.c_str(),
                      reply_info->mail_info.rcptto.c_str(),
                      reply_info->mail_info.host.c_str(), errormsg) == FALSE) {
          fprintf(stderr, "%s", errormsg.c_str());

          if (mailStg->GetExternMailForwardedCount(reply_info->mail_info.mid) >=
              MAX_TRY_FORWARD_COUNT) {
            ReturnMail(mailStg, reply_info->_memcached,
                       reply_info->mail_info.mid, reply_info->mail_info.uniqid,
                       reply_info->mail_info.mailfrom.c_str(),
...
BOOL MailTransferAgent::ReturnMail(MailStorage* mailStg,
  ...
  char szBoundary[64];
  srandom(time(NULL));
  sprintf(szBoundary, "%03d_%04X_%08X.%08X", nBoundary, random() % 0xFFFF,
          time(NULL));
  char szMailFrom[64];
  ...
  char szUid[1024];
  sprintf(szUid, "%08x_%08x_%016lx_%08x_%s", time(NULL), getpid(),
  ...
  if (CMailBase::Is_Local_Domain(rcpttodomain.c_str())) {
    ...
    if (host == "" ||
        strcasecmp(host.c_str(), CMailBase::m_localhostname.c_str()) == 0) {
      mType = mtLocal;
      strHost = "";
    } else {
      mType = mtExtern;
  ...
  MailLetter* newLetter = new MailLetter(
      mailStg, CMailBase::m_private_path.c_str(), CMailBase::m_encoding.c_str(),
      memcached, szUid, usermaxsize);
  ...
  MailLetter* oldLetter =
      new MailLetter(mailStg, CMailBase::m_private_path.c_str(),
                     CMailBase::m_encoding.c_str(), memcached, emlfile.c_str());
  if (oldLetter) {
  ...
  mailStg->InsertMailIndex(
      ...
      letter_info.host.c_str(), letter_info.mail_time, letter_info.mail_type,
      letter_info.mail_uniqueid.c_str(), letter_info.mail_dirid,
      letter_info.mail_status, newLetter->GetEmlName(), newLetter->GetSize(),
      letter_info.mail_id);
...
BOOL MailTransferAgent::SendMail(MailStorage* mailStg,
                                 memcached_st* memcached,
                                 const char* mxserver,
                                 const char* fromaddr,
                                 const char* toaddr,
  ...
  int s = getaddrinfo((realip && realip[0] != '\0') ? realip : NULL, "25",
                      &hints, &server_addr);
  if (s != 0) {
  ...
  getsockopt(transfer_sockfd, SOL_SOCKET, SO_ERROR, (char*)&errorCode,
             (socklen_t*)&errorCodeLen);
  if (errorCode != 0) {
    close(transfer_sockfd);
...
BOOL MailTransferAgent::RelayMail(MailStorage* mailStg,
                                  memcached_st* memcached,
                                  int mid,
                                  const char* uniqid,
                                  const char* mail_from,
  ...
  if (strcmp(mail_host, "") == 0) {
    ...
    if (strcasecmp(mail_host, CMailBase::m_localhostname.c_str()) !=
        0)  // not self-host
    {
      DNSQry_List ml;
  ...
  for (int x = 0; x < mxcount; x++) {
    if (SendMail(mailStg, memcached, list[x].address, "", rcpt_to, mid,
                 errormsg) == TRUE) {
      return TRUE;
    }
  }
  ...
  do {
    ...
    for (map<string, int>::iterator iter = CMailBase::m_memcached_list.begin();
      ...
      memcached_servers = memcached_server_list_append(
          memcached_servers, (*iter).first.c_str(), (*iter).second, &memc_rc);
    ...
    sem_t* postmail_sid =
        sem_open(ERISEMAIL_POST_NOTIFY, O_CREAT | O_RDWR, 0666, 0);
    if ((m_mta_qid == (mqd_t)-1) || (m_mta_sid == SEM_FAILED) ||
        postmail_sid == SEM_FAILED) {
      if (m_mta_sid != SEM_FAILED)
        sem_close(m_mta_sid);
    ...
    relay_pool = new ThreadPool(
        CMailBase::m_mta_relaythreadnum, INIT_RELAY_HANDLER,
        BEGIN_RELAY_HANDLER, NULL, 0, EXIT_RELAY_HANDLER,
        CMailBase::m_mta_relaythreadnum > CMailBase::m_thread_increase_step
            ? CMailBase::m_thread_increase_step
            : CMailBase::m_mta_relaythreadnum);
    ...
    while (1) {
      ...
      if (sr == 0 || (sr == -1 && errno == ETIMEDOUT)) {
        ...
        stgengine_instance =
            new StorageEngineInstance(m_storageEngine, &mailStg);
        if (!stgengine_instance || !mailStg) {
          if (stgengine_instance)
            delete stgengine_instance;
          continue;
        ...
        mailStg->GetMTAIndex(CMailBase::m_localhostname.c_str(),
                             4 * CMailBase::m_mta_relaycheckinterval, mta_index,
                             mta_count);

        if (mta_count > 0 && mailStg->ListAvailableExternMail(
                                 mitbl, mta_index, mta_count,
                                 CMailBase::m_mta_relaythreadnum) == 0) {
          for (int x = 0; x < mitbl.size(); x++) {
            ...
            if (current_idle_thread_num < CMailBase::m_thread_increase_step &&
                current_thread_pool_size < CMailBase::m_mta_relaythreadnum) {
              relay_pool->More(
                  (CMailBase::m_mta_relaythreadnum - current_thread_pool_size) <
                          CMailBase::m_thread_increase_step
                      ? (CMailBase::m_mta_relaythreadnum -
