...
char& dynamic_mmap::operator[](int i) {
  if (i >= 0 && i >= m_map_beg && i <= m_map_end) {
    ...
    if (m_hdfs_conn && m_ifile) {
      ...
      int read_count =
          hdfsRead(m_hdfs_conn, m_ifile, m_map_buf, DYNAMIC_MMAP_SIZE);
      if (read_count > 0) {
        m_map_beg = i;
...
////////////////////////////////////////////////////////////////////////////////////////////////////
// MailLetter
MailLetter::MailLetter(MailStorage* mailStg,
...
MailLetter::MailLetter(MailStorage* mailStg,
  ...
  if (ConnectHDFS()) {
    hdfsFileInfo* file_info =
        hdfsGetPathInfo(m_hdfs_conn, m_eml_full_path.c_str());
    if (file_info) {
  ...
  m_letterSummary = new LetterSummary(
      m_encoding.c_str(), m_eml_cache_full_path.c_str(), m_memcached);
}
...
dynamic_mmap& MailLetter::Body(int& len) {
  if (!m_body) {
    ...
    if (ConnectHDFS() && m_ifile_mmap_in_hdfs == NULL) {
      m_ifile_mmap_in_hdfs =
          hdfsOpenFile(m_hdfs_conn, m_eml_full_path.c_str(), O_RDONLY, 0, 0, 0);
    }
  ...
  if (m_att == laOut) {
    ...
    if (m_hdfs_conn && m_ifile_in_hdfs) {
      ...
      while ((new_line = m_line_text.find('\n')) == std::string::npos) {
        char buf[4096];
        int read_count =
            hdfsRead(m_hdfs_conn, m_ifile_in_hdfs, (void*)buf, 4096);
        if (read_count > 0) {
          buf[read_count] = '\0';
...
int MailLetter::Write(const char* buf, unsigned int len) {
  ...
  if (ConnectHDFS() && m_ofile_in_hdfs == NULL) {
    m_ofile_in_hdfs = hdfsOpenFile(m_hdfs_conn, m_eml_full_path.c_str(),
  ...
  if (!m_ofile) {
    ...
    m_ofile = new ofstream(m_eml_full_path.c_str(),
  ...
  if (m_ofile) {
    if (m_ofile->is_open()) {
      ...
      while (m_mailbody_fragment.length() >
             MAX_MAILBODY_FRAGMENT_SIZER)  // must less than 65535, since mysql
                                           // use TEXT type
      {
        string str_block64k =
            m_mailbody_fragment.substr(0, MAX_MAILBODY_FRAGMENT_SIZER);
        m_mailbody_fragment =
            m_mailbody_fragment.substr(MAX_MAILBODY_FRAGMENT_SIZER);
        if (m_mailstg->SaveMailBodyToDB(m_emlfile.c_str(),
                                        str_block64k.c_str()) < 0)
          return -1;
      }
...
#else
    while (m_mailbody_fragment.length() >
           (65535 - 4096))  // must less than 65535, since mysql use TEXT type
    {
      string str_block64k = m_mailbody_fragment.substr(0, (65535 - 4096));
      m_mailbody_fragment = m_mailbody_fragment.substr((65535 - 4096));
      if (m_mailstg->SaveMailBodyToDB(m_emlfile.c_str(), str_block64k.c_str()) <
          0) {
        m_LetterOk = FALSE;
        break;
      }
...
void MailLetter::get_attach_summary(MimeSummary* part,
                                    unsigned long& count,
...
LetterSummary::LetterSummary(const char* encoding, memcached_st* memcached)
    : BlockSummary(0, NULL) {
  m_memcached = memcached;
  m_xml = NULL;
  m_isheader = TRUE;
  ...
  string strXMLInit =
      "<?xml version='1.0' encoding='" + m_encoding + "'?><letter></letter>";
...
LetterSummary::LetterSummary(const char* encoding,
                             const char* szpath,
                             memcached_st* memcached) {
  m_encoding = encoding;
...
void LetterSummary::_parse_() {
  while (1) {
    ...
    for (x = 0; x < m_strfield.length(); x++) {
      if ((m_strfield[x] == '\n') && (x < m_strfield.length() - 1) &&
          (x < m_strfield.length() - 1) && (m_strfield[x + 1] != '\t') &&
          (m_strfield[x + 1] != ' ')) {
...
void LetterSummary::loadXML() {
  ...
  if (m_memcached) {
    ...
    szMD5dst[34] = 'l';
    szMD5dst[35] = '\0';
    xml_text = memcached_get(m_memcached, szMD5dst, 35, &memc_value_length,
                             &memc_flags, &memc_rc);
  }
  if (m_memcached && xml_text && memc_rc == MEMCACHED_SUCCESS &&
      memc_value_length > 0 && m_xml->LoadString(xml_text, memc_value_length)) {
    if (xml_text)
      free(xml_text);
  } else {
    if (xml_text)
    ...
    if (m_memcached) {
      ...
      memc_rc =
          memcached_set(m_memcached, szMD5dst, 35, xml_printer.CStr(),
                        xml_printer.Size(), (time_t)3600, (uint32_t)memc_flags);
    }
  }
  if (m_xml && m_xml->RootElement()) {
    TiXmlNode* pLetterHeaderNode =
        m_xml->RootElement()->FirstChild("letterHeader");
...
///////////////////////////////////////////////////////////////////////
// LetterHeaderSummary
LetterHeaderSummary::LetterHeaderSummary(TiXmlElement* pSelfElement)
    : BlockSummary(pSelfElement) {
  m_from = NULL;
  m_to = NULL;
  m_sender = NULL;
  m_replyTo = NULL;
...
LetterHeaderSummary::LetterHeaderSummary(unsigned int beg,
                                         TiXmlElement* pParentElement)
    : BlockSummary(beg, pParentElement) {
  m_from = NULL;
  m_to = NULL;
  m_sender = NULL;
  m_replyTo = NULL;
...
void LetterHeaderSummary::loadXML() {
  ...
  if (pFromNode && pFromNode->ToElement()) {
    ...
    m_from->m_strfiled = pFromNode->ToElement()->Attribute("field") == NULL
                             ? ""
                             : pFromNode->ToElement()->Attribute("field");

    TiXmlNode* pAddrNode = pFromNode->ToElement()->FirstChild();
    while (pAddrNode) {
      if (pAddrNode->ToElement()) {
        mailaddr maddr;
        maddr.pName = pAddrNode->ToElement()->Attribute("pName") == NULL
                          ? ""
                          : pAddrNode->ToElement()->Attribute("pName");
        maddr.mName = pAddrNode->ToElement()->Attribute("mName") == NULL
                          ? ""
                          : pAddrNode->ToElement()->Attribute("mName");
        maddr.dName = pAddrNode->ToElement()->Attribute("dName") == NULL
                          ? ""
                          : pAddrNode->ToElement()->Attribute("dName");
  ...
  if (pInReplyToNode && pInReplyToNode->ToElement()) {
    m_inReplyTo = pInReplyToNode->ToElement()->Attribute("value") == NULL
                      ? ""
                      : pInReplyToNode->ToElement()->Attribute("value");
  }

  TiXmlNode* pSubjectNode = m_pSelfElement->FirstChild("subject");
  if (pSubjectNode && pSubjectNode->ToElement()) {
    m_subject = pSubjectNode->ToElement()->Attribute("original") == NULL
                    ? ""
                    : pSubjectNode->ToElement()->Attribute("original");
    m_decodedSubject = pSubjectNode->ToElement()->Attribute("decoded") == NULL
                           ? ""
                           : pSubjectNode->ToElement()->Attribute("decoded");
  }
}
...
void LetterHeaderSummary::fieldParse(const char* field) {
  if (strncasecmp(field, "From:", sizeof("From:") - 1) == 0) {
    ...
    for (int i = 0; i < m_sender->m_addrs.size(); i++) {
      TiXmlElement* pNewAddrElement = new TiXmlElement("addr");
      pNewAddrElement->SetAttribute("pName",
                                    m_sender->m_addrs[i].pName.c_str());
      pNewAddrElement->SetAttribute("mName",
                                    m_sender->m_addrs[i].mName.c_str());
      pNewAddrElement->SetAttribute("dName",
                                    m_sender->m_addrs[i].dName.c_str());
    ...
    for (int i = 0; i < m_replyTo->m_addrs.size(); i++) {
      TiXmlElement* pNewAddrElement = new TiXmlElement("addr");
      pNewAddrElement->SetAttribute("pName",
                                    m_replyTo->m_addrs[i].pName.c_str());
      pNewAddrElement->SetAttribute("mName",
                                    m_replyTo->m_addrs[i].mName.c_str());
      pNewAddrElement->SetAttribute("dName",
                                    m_replyTo->m_addrs[i].dName.c_str());
    ...
    char szmon[32];
    sscanf(m_date.c_str(), "%*[^,], %d %[^ ] %d %d:%d:%d %d", &day, szmon,
  ...
  } else if (strncasecmp(field, "Message-ID:", sizeof("Message-ID:") - 1) ==
             0) {
    fnln_strcut(field, ":", " \t\r\n", " \t\r\n", m_messageID);
  ...
  } else if (strncasecmp(field, "User-Agent:", sizeof("User-Agent:") - 1) ==
             0) {
    fnln_strcut(field, ":", " \t\r\n", " \t\r\n", m_userAgent);
  ...
  } else if (strncasecmp(field, "In-Reply-To:", sizeof("In-Reply-To:") - 1) ==
             0) {
    fnln_strcut(field, ":", " \t\r\n", " \t\r\n", m_inReplyTo);
...
///////////////////////////////////////////////////////////////////////
// LetterDataSummary
LetterDataSummary::LetterDataSummary(TiXmlElement* pSelfElement)
    : BlockSummary(pSelfElement) {}

LetterDataSummary::LetterDataSummary(unsigned int beg,
                                     TiXmlElement* pParentElement)
    : BlockSummary(beg, pParentElement) {
  if (!m_pNewElement)
...
MimeSummary::MimeSummary(TiXmlElement* pSelfElement)
    : BlockSummary(pSelfElement) {
  m_isheader = TRUE;
  m_header = NULL;
  m_data = NULL;
...
MimeSummary::MimeSummary(unsigned int beg, TiXmlElement* pParentElement)
    : BlockSummary(beg, pParentElement) {
  m_isheader = TRUE;
...
void MimeSummary::fieldParse(const char* field) {
  if (m_isheader == TRUE) {
    ...
    if (!m_data) {
      ...
      if (m_header->m_Content_Type &&
          strcasecmp(m_header->m_Content_Type->m_type1.c_str(), "multipart") ==
              0) {
...
/////////////////////////////////////////////////////////////////
// MimeHeaderSummary
MimeHeaderSummary::MimeHeaderSummary(TiXmlElement* pSelfElement)
    : BlockSummary(pSelfElement) {
  m_Content_Type = NULL;
  m_Content_Disposition = NULL;
}

MimeHeaderSummary::MimeHeaderSummary(unsigned int beg,
                                     TiXmlElement* pParentElement)
    : BlockSummary(beg, pParentElement) {
  m_Content_Type = NULL;
  m_Content_Disposition = NULL;
...
void MimeHeaderSummary::loadXML() {
  ...
  if (pContentTypeNode && pContentTypeNode->ToElement()) {
    ...
    if (m_Content_Type) {
      m_Content_Type->m_type1 =
          pContentTypeNode->ToElement()->Attribute("type1") == NULL
              ? ""
              : pContentTypeNode->ToElement()->Attribute("type1");
      m_Content_Type->m_type2 =
          pContentTypeNode->ToElement()->Attribute("type2") == NULL
              ? ""
              : pContentTypeNode->ToElement()->Attribute("type2");
      m_Content_Type->m_charset =
          pContentTypeNode->ToElement()->Attribute("charset") == NULL
              ? ""
              : pContentTypeNode->ToElement()->Attribute("charset");
      m_Content_Type->m_filename =
          pContentTypeNode->ToElement()->Attribute("filename") == NULL
              ? ""
              : pContentTypeNode->ToElement()->Attribute("filename");
      m_Content_Type->m_boundary =
          pContentTypeNode->ToElement()->Attribute("boundary") == NULL
              ? ""
              : pContentTypeNode->ToElement()->Attribute("boundary");
    }
  }

  TiXmlNode* pContentIDNode = m_pSelfElement->FirstChild("contentID");
  if (pContentIDNode && pContentIDNode->ToElement()) {
    m_Content_ID = pContentIDNode->ToElement()->Attribute("value") == NULL
                       ? ""
                       : pContentIDNode->ToElement()->Attribute("value");
  }

  TiXmlNode* pContentClassNode = m_pSelfElement->FirstChild("contentClass");
  if (pContentClassNode && pContentClassNode->ToElement()) {
    m_Content_Class = pContentClassNode->ToElement()->Attribute("value") == NULL
                          ? ""
                          : pContentClassNode->ToElement()->Attribute("value");
  }

  TiXmlNode* pContentTransferEncodingNode =
      m_pSelfElement->FirstChild("contentTransferEncoding");
  if (pContentTransferEncodingNode &&
      pContentTransferEncodingNode->ToElement()) {
    m_Content_Transfer_Encoding =
        pContentTransferEncodingNode->ToElement()->Attribute("value") == NULL
            ? ""
            : pContentTransferEncodingNode->ToElement()->Attribute("value");
  }

  TiXmlNode* pContentDispositionNode =
      m_pSelfElement->FirstChild("contentDisposition");

  if (pContentDispositionNode && pContentDispositionNode->ToElement()) {
    m_Content_Disposition = new filedContentDisposition();
    if (m_Content_Disposition) {
      m_Content_Disposition->m_disposition =
          pContentDispositionNode->ToElement()->Attribute("disposition") == NULL
              ? ""
              : pContentDispositionNode->ToElement()->Attribute("disposition");
      m_Content_Disposition->m_filename =
          pContentDispositionNode->ToElement()->Attribute("filename") == NULL
              ? ""
              : pContentDispositionNode->ToElement()->Attribute("filename");
    }
  }
}
...
void MimeHeaderSummary::fieldParse(const char* field) {
  if (strncasecmp(field, "Content-Type:", sizeof("Content-Type:") - 1) == 0) {
    ...
    pNewContentTypeElement->SetAttribute("type1",
                                         m_Content_Type->m_type1.c_str());
    pNewContentTypeElement->SetAttribute("type2",
                                         m_Content_Type->m_type2.c_str());
    pNewContentTypeElement->SetAttribute("boundary",
                                         m_Content_Type->m_boundary.c_str());
    pNewContentTypeElement->SetAttribute("charset",
                                         m_Content_Type->m_charset.c_str());
    pNewContentTypeElement->SetAttribute("filename",
                                         m_Content_Type->m_filename.c_str());

    m_pNewElement->InsertEndChild(*pNewContentTypeElement);
    delete pNewContentTypeElement;

  } else if (strncasecmp(field, "Content-ID:", sizeof("Content-ID:") - 1) ==
             0) {
    fnfy_strcut(field, ":", " \t\"<", " >\t\";\r\n", m_Content_ID);
    ...
    delete pNewContentIDElement;
                         ...
                         "Content-class:", sizeof("Content-class:") - 1) == 0) {
    fnfy_strcut(field, ":", " \t\"<", " >\t\";\r\n", m_Content_Class);
  ...
  } else if (strncasecmp(field, "Content-Transfer-Encoding:",
                         sizeof("Content-Transfer-Encoding:") - 1) == 0) {
    fnfy_strcut(field, ":", " \t\"\r\n", " \t\";\r\n",
                m_Content_Transfer_Encoding);

    TiXmlElement* pNewContentTransferEncodingElement =
        new TiXmlElement("contentTransferEncoding");

    pNewContentTransferEncodingElement->SetAttribute(
        "value", m_Content_Transfer_Encoding.c_str());

    m_pNewElement->InsertEndChild(*pNewContentTransferEncodingElement);
    delete pNewContentTransferEncodingElement;

  } else if (strncasecmp(field, "Content-Disposition:",
                         sizeof("Content-Disposition:") - 1) == 0) {
    m_Content_Disposition = new filedContentDisposition(field);

    TiXmlElement* pNewContentDispositionElement =
        new TiXmlElement("contentDisposition");

    pNewContentDispositionElement->SetAttribute(
        "disposition", m_Content_Disposition->m_disposition.c_str());
    pNewContentDispositionElement->SetAttribute(
        "filename", m_Content_Disposition->m_filename.c_str());
...
///////////////////////////////////////////////////////////////////////
// MimeDataSummary
MimeDataSummary::MimeDataSummary(TiXmlElement* pSelfElement)
    : BlockSummary(pSelfElement) {
  m_boundary = "";
  m_pNewSummary = NULL;
}

MimeDataSummary::MimeDataSummary(unsigned int beg, TiXmlElement* pParentElement)
    : BlockSummary(beg, pParentElement) {
  if (!m_pNewElement)
