...
////////////////////////////////////////////////////////////////
// mimepart
mimepart::mimepart(const char* buffer,
...
void mimepart::parse() {
  ...
  for (int x = m_beg; x <= m_end; x++) {
    ...
    if (strncasecmp(strTmp.c_str(),
                    "Content-Type:", sizeof("Content-Type:") - 1) == 0) {
      m_Content_Type = new filedContentType(strTmp.c_str());
    } else if (strncasecmp(strTmp.c_str(),
                           "Content-ID:", sizeof("Content-ID:") - 1) == 0) {
      fnfy_strcut(strTmp.c_str(), ":", " \t\"<", " >\t\";\r\n", m_Content_ID);
    } else if (strncasecmp(strTmp.c_str(), "Content-Transfer-Encoding:",
                           sizeof("Content-Transfer-Encoding:") - 1) == 0) {
      fnfy_strcut(strTmp.c_str(), ":", " \t\"\r\n", " \t\";\r\n",
                  m_Content_Transfer_Encoding);
    } else if (strncasecmp(strTmp.c_str(), "Content-Disposition:",
  ...
  for (int x = step; x <= m_end; x++) {
    ...
    if ((m_Content_Type) &&
        (strcasecmp(m_Content_Type->m_type1.c_str(), "multipart") == 0)) {
      if (strTmp == strBeg) {
        if (beg == 0xFFFFFFFFUL) {
          beg = x + 1;
        } else {
...
void MIME::GlobalParse() {
  ...
  for (unsigned int x = 0; x < m_vfbufseg.size(); x++) {
    ...
    if (strcasecmp(fstrbuf.c_str(), "\r\n") == 0) {
      break;
    } else if (strncasecmp(fstrbuf.c_str(), "From:", sizeof("From:") - 1) ==
               0) {
      m_From = new filedaddr(fstrbuf.c_str());
    } else if (strncasecmp(fstrbuf.c_str(), "To:", sizeof("To:") - 1) == 0) {
      m_To = new filedaddr(fstrbuf.c_str());
    } else if (strncasecmp(fstrbuf.c_str(), "Sender:", sizeof("Sender:") - 1) ==
               0) {
      m_Sender = new filedaddr(fstrbuf.c_str());
    } else if (strncasecmp(fstrbuf.c_str(),
                           "Reply-To:", sizeof("Reply-To:") - 1) == 0) {
    ...
    } else if (strncasecmp(fstrbuf.c_str(), "Date:", sizeof("Date:") - 1) ==
               0) {
      fnln_strcut(fstrbuf.c_str(), ":", " \t\r\n", " \t\r\n", m_Date);
      ...
      fnln_strcut(fstrbuf.c_str(), ":", " \t\r\n", " \t\r\n", m_User_Agent);
                           ...
                           "In-Reply-To:", sizeof("In-Reply-To:") - 1) == 0) {
      fnln_strcut(fstrbuf.c_str(), ":", " \t\r\n", " \t\r\n", m_In_Reply_To);
    }
  }
