...
CMailSmtp::CMailSmtp(int sockfd,
                     SSL* ssl,
                     SSL_CTX* ssl_ctx,
                     const char* clientip,
  ...
  getpeername(m_sockfd, (struct sockaddr*)&peer_addr,
  ...
  if (pRootElement) {
    ...
    while (pChildNode) {
      if (pChildNode && pChildNode->ToElement()) {
        ...
        if (mfilter.lhandle) {
          if (strcasecmp(pChildNode->ToElement()->Attribute("action") == NULL
                             ? ""
                             : pChildNode->ToElement()->Attribute("action"),
                         "drop") == 0) {
          ...
          mfilter_init =
              (void* (*)(const char*))dlsym(mfilter.lhandle, "mfilter_init");
          const char* errmsg;
          if ((errmsg = dlerror()) == NULL) {
            mfilter.dhandle =
                mfilter_init(pChildNode->ToElement()->Attribute("param")
                                 ? pChildNode->ToElement()->Attribute("param")
                                 : "");
  ...
  for (int x = 0; x < m_filterHandle.size(); x++) {
    if (m_filterHandle[x].lhandle) {
      ...
      mfilter_emaildomain = (void (*)(void*, const char*, unsigned int))dlsym(
          m_filterHandle[x].lhandle, "mfilter_emaildomain");
      const char* errmsg;
      if ((errmsg = dlerror()) == NULL) {
        mfilter_emaildomain(m_filterHandle[x].dhandle, m_email_domain.c_str(),
                            m_email_domain.length());
      } else {
        printf("%s\n", errmsg);
      }

      void (*mfilter_clientip)(void*, const char*, unsigned int);
      mfilter_clientip = (void (*)(void*, const char*, unsigned int))dlsym(
          m_filterHandle[x].lhandle, "mfilter_clientip");
      if ((errmsg = dlerror()) == NULL) {
        mfilter_clientip(m_filterHandle[x].dhandle, m_clientip.c_str(),
                         m_clientip.length());
      } else {
        printf("%s\n", errmsg);
      }
      ...
      mfilter_clientdomain = (void (*)(void*, const char*, unsigned int))dlsym(
          m_filterHandle[x].lhandle, "mfilter_clientdomain");
      if ((errmsg = dlerror()) == NULL) {
        mfilter_clientdomain(m_filterHandle[x].dhandle, m_clientdomain.c_str(),
                             m_clientdomain.length());
      } else {
        printf("%s", errmsg);
      }
    }
...
CMailSmtp::~CMailSmtp() {
  ...
  for (int x = 0; x < m_filterHandle.size(); x++) {
    if (m_filterHandle[x].lhandle) {
      ...
      mfilter_exit =
          (void (*)(void*))dlsym(m_filterHandle[x].lhandle, "mfilter_exit");
      const char* errmsg;
      if ((errmsg = dlerror()) == NULL) {
...
BOOL CMailSmtp::On_Mail_Handler(char* text) {
  ...
  if ((m_status & STATUS_AUTHED) == STATUS_AUTHED) {
    if (CMailBase::Is_Local_Domain(fromdomain.c_str())) {
      ...
      char cmd[256];
      sprintf(cmd,
              "553 sorry, that domain is not in my domain list of allowed "
              "mailhosts\r\n");
    ...
    if (CMailBase::Is_Local_Domain(fromdomain.c_str())) {
      ...
      for (int x = 0; x < m_filterHandle.size(); x++) {
        if (m_filterHandle[x].lhandle) {
          ...
          mfilter_mailfrom = (void (*)(void*, const char*, unsigned int))dlsym(
              m_filterHandle[x].lhandle, "mfilter_mailfrom");
          const char* errmsg;
          if ((errmsg = dlerror()) == NULL) {
            mfilter_mailfrom(m_filterHandle[x].dhandle, m_mailfrom.c_str(),
                             m_mailfrom.length());
          } else {
            printf("%s\n", errmsg);
          }
  ...
  if (strncasecmp(&text[5], "LOGIN", 5) == 0) {
    ...
    if (m_ssl) {
      ...
      if (client_cert != NULL) {
        ...
        if (bFound) {
          ...
          sprintf(cmd, "235 EXTERNAL authentication successful\r\n");
          SmtpSend(cmd, strlen(cmd));
          m_status = m_status | STATUS_AUTHED;
        } else {
          sprintf(cmd, "535 User Logged Failed\r\n");
    ...
    do {
      ...
      if (output_token.length != 0) {
        int len_decode = BASE64_ENCODE_OUTPUT_MAX_LEN(output_token.length + 4);
        char* tmp_encode = (char*)malloc(len_decode + 4);
        ...
        CBase64::Encode((char*)output_token.value, output_token.length,
                        tmp_encode + 4, &outlen_encode);

        SmtpSend(tmp_encode, outlen_encode + 4);
        SmtpSend("\r\n", 2);
    ...
    int len_decode =
        BASE64_ENCODE_OUTPUT_MAX_LEN(output_message_buffer1.length + 4);
    char* tmp_encode = (char*)malloc(len_decode + 4);
    memset(tmp_encode, 0, len_decode);
    tmp_encode[0] = '3';
    tmp_encode[1] = '3';
    ...
    CBase64::Encode((char*)output_message_buffer1.value,
                    output_message_buffer1.length, tmp_encode + 4,
                    &outlen_encode);
...
BOOL CMailSmtp::On_Supose_Username_Handler(char* text) {
  if (m_authType == SMTP_AUTH_LOGIN) {
    ...
    strtrim(username);
    int outlen = BASE64_DECODE_OUTPUT_MAX_LEN(
        username.length());  // username.length()*4+1;
...
BOOL CMailSmtp::On_Supose_Password_Handler(char* text) {
  if (m_authType == SMTP_AUTH_LOGIN) {
    ...
    strtrim(password);
    int outlen = BASE64_DECODE_OUTPUT_MAX_LEN(
        password.length());  // password.length()*4+1;
    char* tmp = (char*)malloc(outlen);
    memset(tmp, 0, outlen);
...
BOOL CMailSmtp::On_Rcpt_Handler(char* text) {
  ...
  for (int x = 0; x < m_filterHandle.size(); x++) {
    if (m_filterHandle[x].lhandle) {
      ...
      mfilter_rcptto = (void (*)(void*, const char*, unsigned int))dlsym(
          m_filterHandle[x].lhandle, "mfilter_rcptto");
      const char* errmsg;
      if ((errmsg = dlerror()) == NULL) {
        mfilter_rcptto(m_filterHandle[x].dhandle, rcpt_to.c_str(),
                       rcpt_to.length());
      } else {
        printf("%s\n", errmsg);
      }
  ...
  if (CMailBase::Is_Local_Domain(rcptto_domain.c_str())) {
    if ((mailStg->VerifyUser(user_or_group.c_str()) != 0) &&
        (mailStg->VerifyGroup(user_or_group.c_str()) != 0)) {
      push_reject_list(MDA_SERVICE_NAME, m_clientip.c_str());

      char cmd[256];
      sprintf(cmd,
              "553 sorry, that domain is not in my deliver domain list\r\n");
      SmtpSend(cmd, strlen(cmd));
      return FALSE;
    } else {
      if (mailStg->VerifyGroup(user_or_group.c_str()) == 0) {
        ...
        for (int x = 0; x < vlen; x++) {
          ...
          letter_obj->letter_info.mail_to =
              member_mail.c_str();  // new string(member_mail.c_str());
          m_letter_obj_vector.push_back(letter_obj);
        }
      } else {
        LETTER_OBJ* lette_obj = new LETTER_OBJ;
        lette_obj->letter = NULL;
        lette_obj->letter_info.mail_to =
            rcpt_to.c_str();  // rcpt_to = new string(rcpt_to.c_str());
  ...
  // To Extern domain
    ...
    if ((m_status & STATUS_AUTHED) == STATUS_AUTHED) {
      ...
      lette_obj->letter = NULL;
      lette_obj->letter_info.mail_to =
          rcpt_to.c_str();  // new string(rcpt_to.c_str());
      m_letter_obj_vector.push_back(lette_obj);
    } else {
      if (!m_enablerelay) {
        push_reject_list(MDA_SERVICE_NAME, m_clientip.c_str());
...
void CMailSmtp::On_Data_Handler(char* text) {
  ...
  for (int x = 0; x < vlen; x++) {
    ...
    strcut(m_letter_obj_vector[x]->letter_info.mail_to.c_str(), "@", NULL,
           rcptto_domain);
    unsigned int mstatus = MSG_ATTR_RECENT;

    sprintf(uid, "%08x_%08x_%016lx_%08x_%s", time(NULL), getpid(),
    ...
    if (CMailBase::Is_Local_Domain(rcptto_domain.c_str())) {
      if (m_status & STATUS_AUTHED == STATUS_AUTHED) {
        ...
        m_letter_obj_vector[x]->letter = new MailLetter(
            mailStg, CMailBase::m_private_path.c_str(),
            CMailBase::m_encoding.c_str(), m_memcached, uid, usermaxsize);

        m_letter_obj_vector[x]->letter_info.mail_from = m_mailfrom.c_str();

        if (mail_host == "" ||
            strcasecmp(mail_host.c_str(), CMailBase::m_localhostname.c_str()) ==
                0) {
    ...
    sprintf(
        ...
        m_clientip.c_str(), m_localhostname.c_str(),
        m_letter_obj_vector[x]->letter_info.mail_uniqueid.c_str(),
        m_letter_obj_vector[x]->letter_info.mail_to.c_str(), strTime.c_str());
...
BOOL CMailSmtp::On_Supose_Dataing_Handler(char* text) {
  for (int x = 0; x < m_filterHandle.size(); x++) {
    if (m_filterHandle[x].lhandle) {
      ...
      mfilter_data = (void (*)(void*, const char*, unsigned int))dlsym(
          m_filterHandle[x].lhandle, "mfilter_data");
      const char* errmsg;
      if ((errmsg = dlerror()) == NULL) {
...
void CMailSmtp::On_Finish_Data_Handler() {
  ...
  for (int x = 0; x < vlen; x++) {
    ...
    if (m_letter_obj_vector[x]->letter_info.mail_type == mtExtern &&
      ...
      m_letter_obj_vector[x]->letter->GetAttachSummary(attach_count,
                                                       attach_sumsize);
    ...
    if (x == 0) /* only check the first mail since all mail body is same */
    {
      for (int y = 0; y < m_filterHandle.size(); y++) {
        if (m_filterHandle[y].lhandle) {
          ...
          mfilter_eml = (void (*)(void*, const char*, unsigned int))dlsym(
              m_filterHandle[y].lhandle, "mfilter_eml");
          const char* errmsg;
          if ((errmsg = dlerror()) == NULL && m_letter_obj_vector.size() > 0) {
            mfilter_eml(
                m_filterHandle[y].dhandle,
                m_letter_obj_vector[x]->letter->GetEmlFullPath(),
          ...
          mfilter_result = (void (*)(void*, int*))dlsym(
              m_filterHandle[y].lhandle, "mfilter_result");
          if ((errmsg = dlerror()) == NULL) {
    ...
    if (isSpam != -1 &&
      ...
      if (action == jaTag) {
        ...
        m_letter_obj_vector[x]->letter_info.mail_dirid =
            DirID;  // move to junk folder
      }
    }

    if (action != jaDrop && m_letter_obj_vector[x]->letter->isOK()) {
      mailStg->InsertMailIndex(
          m_letter_obj_vector[x]->letter_info.mail_from.c_str(),
          ...
          m_letter_obj_vector[x]->letter_info.mail_type,
          m_letter_obj_vector[x]->letter_info.mail_uniqueid.c_str(),
          m_letter_obj_vector[x]->letter_info.mail_dirid,
          m_letter_obj_vector[x]->letter_info.mail_status,
          m_letter_obj_vector[x]->letter->GetEmlName(),
          m_letter_obj_vector[x]->letter->GetSize(),
          m_letter_obj_vector[x]->letter_info.mail_id);
      if (m_letter_obj_vector[x]->letter_info.mail_type == mtExtern) {
        mta_post_notify();
      }
    }
    ...
    if (client_cert != NULL) {
      ...
      for (;;) {
        ...
        if (strcasecmp(commonName, m_helo_argument.c_str()) == 0 ||
            strmatch(commonName, m_helo_argument.c_str())) {
          bFound = TRUE;
          break;
        }
      }
...
void CMailSmtp::On_Ehlo_Handler(char* text) {
  ...
  if (m_enablesmtptls)
    sprintf(cmd,
            ...
            "CRAM-MD5 DIGEST-MD5 %s GSSAPI\r\n250-8BITMIME\r\n250 OK\r\n",
            m_email_domain.c_str(), m_ca_verify_client ? "EXTERNAL" : "");
  else
    sprintf(cmd,
...
void CMailSmtp::On_Service_Ready_Handler() {
  ...
  sprintf(
      reply,
      "220 %s SMTP service is ready by eRisemail-%s powered by Uplusware\r\n",
...
void CMailSmtp::On_STARTTLS_Handler() {
  ...
  if (m_ca_verify_client && m_ssl && m_ssl_ctx) {
    X509* client_cert;
    client_cert = SSL_get_peer_certificate(m_ssl);
    if (client_cert != NULL) {
      X509_NAME* owner = X509_get_subject_name(client_cert);
