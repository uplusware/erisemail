...
CMailImap::CMailImap(int sockfd,
                     SSL* ssl,
                     SSL_CTX* ssl_ctx,
                     const char* clientip,
...
void CMailImap::On_Service_Ready() {
  ...
  char cmd[1024];
  sprintf(cmd,
          "* OK %s IMAP4rev1 Server is ready by eRisemail-%s powered by "
          "Uplusware\r\n",
...
#ifdef _WITH_GSSAPI_
  sprintf(cmd,
          "* CAPABILITY IMAP4rev1 STARTTLS AUTH=CRAM-MD5 AUTH=DIGEST-MD5 %s "
          "AUTH=GSSAPI\r\n",
...
void CMailImap::On_Noop(char* text) {
  ...
  if ((m_status & STATUS_AUTHED) == STATUS_AUTHED) {
    if ((m_status & STATUS_SELECT) == STATUS_SELECT) {
      ...
      mailStg->ListMailByDir((char*)m_username.c_str(), m_maillisttbl,
                             (char*)m_strSelectedDir.c_str());
...
BOOL CMailImap::On_Authenticate(char* text) {
  ...
  if (strcasecmp(strAuthType.c_str(), "PLAIN") == 0) {
    return FALSE;
  } else if (strcasecmp(strAuthType.c_str(), "DIGEST-MD5") ==
             0)  //(strAuthType == "DIGEST-MD5")
  {
    ...
    sprintf(cmd, "<%u%u.%u@%s>", (unsigned int)(random() % 10000),
    ...
    strtrim(strEncoded);
    string strToken;
    outlen = BASE64_DECODE_OUTPUT_MAX_LEN(
    ...
    if (mailStg->GetPassword(m_username.c_str(), strpwd) == 0) {
      ...
      ietf::HMAC_MD5((unsigned char*)strDigest.c_str(), strDigest.length(),
    ...
    string str_buf_desc = CMailBase::m_krb5_imap_service_name;
    str_buf_desc += "@";
    ...
    maj_stat = gss_acquire_cred(&min_stat, server_name, 0, oid_set,
    ...
    push_reject_list(MDA_SERVICE_NAME, m_clientip.c_str());
    sprintf(cmd,
            "%s NO authenticate Failed: unsupported authentication mechanism, "
            "credentials rejects\r\n",
            strTag.c_str());
    ImapSend(cmd, strlen(cmd));
    return FALSE;
...
void CMailImap::On_Select(char* text) {
  ...
  if ((m_status & STATUS_AUTHED) == STATUS_AUTHED) {
    ...
    sprintf(cmd,
            ...
            nExist, nRecent, nUnseen, firstUnseen, 1, strTag.c_str(),
            USER_PRI_TBL[eUserPri]);
...
void CMailImap::On_Delete(char* text) {
  ...
  if ((m_status & STATUS_AUTHED) == STATUS_AUTHED) {
    ...
    if ((strcasecmp(vecDest[2].c_str(), "inbox") == 0) ||
        (strcasecmp(vecDest[2].c_str(), "drafts") == 0) ||
        (strcasecmp(vecDest[2].c_str(), "trash") == 0) ||
        (strcasecmp(vecDest[2].c_str(), "junk") == 0)) {
      sprintf(cmd,
              "%s NO DELETE Failed: Forbid to Delete Reserved Directory\r\n",
              strTag.c_str());
    } else {
      if (mailStg->DeleteDir((char*)m_username.c_str(), vecDest[2].c_str()) ==
          0) {
...
void CMailImap::On_Rename(char* text) {
  ...
  if ((m_status & STATUS_AUTHED) == STATUS_AUTHED) {
    ...
    if ((strcasecmp(vecDest[2].c_str(), "inbox") == 0) ||
        ...
        (strcasecmp(vecDest[2].c_str(), "junk") == 0)) {
      sprintf(cmd,
              "%s NO RENAME Failed: Forbid to RENAME Reserved Directory\r\n",
              strTag.c_str());
    } else {
      if (mailStg->RenameDir(m_username.c_str(), vecDest[2].c_str(),
                             vecDest[3].c_str()) == 0) {
    ...
    if ((strReference == "") && (strName == "")) {
      ...
      for (int i = 0; i < vlen; i++) {
        ...
        sprintf(cmd, "* LIST (%s) \"%s\" \"%s\"\r\n", strStatus.c_str(),
                strReference == "" ? "/" : strReference.c_str(),
                ditbl[i].path.c_str());
        ImapSend(cmd, strlen(cmd));
      }
      sprintf(cmd, "%s OK LIST completed\r\n", strTag.c_str());
...
void CMailImap::On_Status(char* text) {
  ...
  if ((m_status & STATUS_AUTHED) == STATUS_AUTHED) {
    ...
    if (mailStg->ListMailByDir(
            (char*)m_username.c_str(), m_maillisttbl,
            /* m_strSelectedDir.c_str()*/ vecDest[2].c_str()) != 0) {
    ...
    for (int x = 3; x < vecDest.size(); x++) {
      ...
      if (vecDest[x] == "MESSAGES") {
        ...
        if (m_maillisttbl.size() > 0) {
          sprintf(cmd, "UIDNEXT %u",
                  m_maillisttbl[m_maillisttbl.size() - 1].mid + 1);
        ...
        int dirID;
        if (mailStg->GetDirID(m_username.c_str(), vecDest[2].c_str(), dirID) ==
            -1) {
...
void CMailImap::On_Append(char* text) {
  ...
  if ((m_status & STATUS_AUTHED) == STATUS_AUTHED) {
    ...
    if (strStatus.find("\\Answered", 0, sizeof("\\Answered") - 1) !=
        string::npos) {
      status |= MSG_ATTR_ANSWERED;
    }
    if (strStatus.find("\\Flagged", 0, sizeof("\\Flagged") - 1) !=
        string::npos) {
      status |= MSG_ATTR_FLAGGED;
    }
    if (strStatus.find("\\Deleted", 0, sizeof("\\Deleted") - 1) !=
        string::npos) {
    ...
    sprintf(newuid, "%08x_%08x_%016lx_%08x_%s", time(NULL), getpid(),
            pthread_self(), random(), m_localhostname.c_str());
    ...
    if (mailStg->GetDirID(m_username.c_str(), dirName.c_str(), DirID) == -1) {
      ...
      if (mailStg->CreateDir(m_username.c_str(), dirName.c_str()) == -1) {
        ...
        if (mailStg->GetDirID(m_username.c_str(), dirName.c_str(), DirID) ==
            -1) {
          sprintf(cmd, "%s NO APPEND Failed, unable get the dir.\r\n",
                  strTag.c_str());
          ImapSend(cmd, strlen(cmd));
    ...
    if (Letter->isOK()) {
      mailStg->InsertMailIndex(
          ...
          letter_info.mail_dirid, letter_info.mail_status, Letter->GetEmlName(),
          Letter->GetSize(), letter_info.mail_id);
    }
    delete Letter;
...
void CMailImap::On_Close(char* text) {
  ...
  if ((m_status & STATUS_AUTHED) == STATUS_AUTHED) {
    ...
    if ((m_status & STATUS_SELECT) == STATUS_SELECT) {
      if (mailStg->ListMailByDir((char*)m_username.c_str(), m_maillisttbl,
                                 (char*)m_strSelectedDir.c_str()) == -1) {
...
void CMailImap::BodyStruct(MimeSummary* part, string& strStruct, BOOL isEND) {
  ...
  if (fcType) {
    if (strcasecmp(fcType->m_type1.c_str(), "multipart") != 0) {
      ...
      char str6[64];
      sprintf(str6, "%u %u", part->m_data->m_end - part->m_data->m_beg,
              part->m_data->m_linenumber);
      ...
      if (str4 == "") {
        strStruct += str1 + " " + str2 + " (\"CHARSET\" " + str3 +
                     ") NIL NIL " + str5 + " " + str6 + " NIL NIL NIL";
      } else {
        strStruct += str1 + " " + str2 + " (\"CHARSET\" " + str3 +
                     " \"NAME\" \"" + str4 + "\") NIL NIL " + str5 + " " +
                     str6 + " NIL NIL NIL";
      }
      strStruct += ")";
    } else {
    ...
    char str6[64];
    sprintf(str6, "%u %u", part->m_data->m_end - part->m_beg,
            part->m_data->m_linenumber);
    ...
    if (str4 == "") {
      strStruct += str1 + " " + str2 + " (\"CHARSET\" " + str3 + ") NIL NIL " +
                   str5 + " " + str6 + " NIL NIL NIL";
    } else {
      strStruct += str1 + " " + str2 + " (\"CHARSET\" " + str3 +
                   " \"NAME\" \"" + str4 + "\") NIL NIL " + str5 + " " + str6 +
                   " NIL NIL NIL";
    }
    strStruct += ")";
  }
...
void CMailImap::Fetch(const char* szArg, BOOL isUID) {
  ...
  for (int z = 0; z < zLen; z++) {
    ...
    int iLen = vItem.size();
    for (int x = 0; x < vLen; x++) {
      if (((isUID ? m_maillisttbl[x].mid : x + 1) >= nBegin) &&
          ((isUID ? m_maillisttbl[x].mid : x + 1) <= nEnd)) {
        ...
        if (Letter->GetSize() > 0) {
          ...
          for (int i = 0; i < iLen; i++) {
            if (strmatch("BODY.PEEK[*]", vItem[i].c_str()) == TRUE) {
              if (strmatch("BODY.PEEK[HEADER.FIELDS (*)]", vItem[i].c_str()) ==
                  TRUE) {
                string strHeaderFileds;
                vector<string> vecHeaderFileds;
                strcut(szArg, "BODY.PEEK[HEADER.FIELDS (", ")]",
                       strHeaderFileds);
                ...
                for (int j = 0; j < vHeaderLen; j++) {
                  string fstrbuf = "";
                  for (int x = Letter->GetSummary()->m_mime->m_header->m_beg;
                       x < Letter->GetSummary()->m_mime->m_header->m_end; x++) {
                    fstrbuf += lbuf[x];

                    if (lbuf[x] == '\n' && lbuf[x + 1] != ' ' &&
                        lbuf[x] != '\t') {
                      if (strncasecmp(fstrbuf.c_str(),
                                      vecHeaderFileds[j].c_str(),
                                      vecHeaderFileds[j].length()) == 0) {
              ...
              } else if (strmatch("BODY.PEEK[HEADER.FIELDS.NOT (*)]",
                                  vItem[i].c_str()) == TRUE) {
                string strHeaderFileds;
                vector<string> vecHeaderFileds;
                strcut(szArg, "BODY.PEEK[HEADER.FIELDS.NOT (", ")]",
                       strHeaderFileds);
                vSplitString(strHeaderFileds, vecHeaderFileds, " ");
                int vHeaderLen = vecHeaderFileds.size();
                string strRespHeader = "";
                ...
                ImapSend(cmd, strlen(cmd));
                ImapSend(strRespHeader.c_str(), strRespHeader.length());
              } else if (strcasecmp((char*)vItem[i].c_str(),
                                    (char*)"BODY.PEEK[]") == 0) {
                if (i > 0 || isUID)
                  ImapSend(" ", 1);

                sprintf(cmd, "BODY[] {%u}\r\n",
                        Letter->GetSummary()->m_mime->m_end -
                            Letter->GetSummary()->m_mime->m_beg);
                ImapSend(cmd, strlen(cmd));
                ...
                sprintf(cmd, "BODY[HEADER] {%u}\r\n",
                        Letter->GetSummary()->m_mime->m_header->m_end -
                            Letter->GetSummary()->m_mime->m_header->m_beg);

                ImapSend(cmd, strlen(cmd));

                int nRead = 0;
                int tLen = Letter->GetSummary()->m_mime->m_header->m_end -
                           Letter->GetSummary()->m_mime->m_header->m_beg;
                while (1) {
                  if (nRead >= tLen)
                    break;
                  ...
                  dynamic_mmap_copy(
                      sztmp, lbuf,
                      Letter->GetSummary()->m_mime->m_header->m_beg + nRead,
                      (tLen - nRead) > 1024 ? 1024 : (tLen - nRead));
                ...
                sprintf(cmd, "BODY[TEXT] {%u}\r\n",
                        Letter->GetSummary()->m_mime->m_data->m_end -
                            Letter->GetSummary()->m_mime->m_data->m_beg);

                ImapSend(cmd, strlen(cmd));

                int nRead = 0;
                int tLen = Letter->GetSummary()->m_mime->m_data->m_end -
                           Letter->GetSummary()->m_mime->m_data->m_beg;
                while (1) {
                  if (nRead >= tLen)
                    break;
                  ...
                  dynamic_mmap_copy(
                      sztmp, lbuf,
                      Letter->GetSummary()->m_mime->m_data->m_beg + nRead,
                      (tLen - nRead) > 1024 ? 1024 : (tLen - nRead));
                  sztmp[(tLen - nRead) > 1024 ? 1024 : (tLen - nRead)] = '\0';
              ...
              } else if (strmatch("BODY.PEEK[?*.MIME]", vItem[i].c_str()) ==
                         TRUE) {
                if (i > 0 || isUID)
                  ImapSend(" ", 1);
                ...
                if (isExist) {
                  sprintf(cmd, "BODY[%s.MIME] {%u}\r\n", rstrtmp.c_str(),
                          pMp->m_header->m_end - pMp->m_header->m_beg);
                  ImapSend(cmd, strlen(cmd));
                  ...
                  while (1) {
                    ...
                    char sztmp[1025];
                    dynamic_mmap_copy(
                        sztmp, lbuf, pMp->m_header->m_beg + nRead,
                        (tLen - nRead) > 1024 ? 1024 : (tLen - nRead));
                    sztmp[(tLen - nRead) > 1024 ? 1024 : (tLen - nRead)] = '\0';
              ...
              } else if (strmatch("BODY.PEEK[?*.TEXT]", vItem[i].c_str()) ==
                         TRUE) {
                if (i > 0 || isUID)
                  ImapSend(" ", 1);
                ...
                if (isExist) {
                  sprintf(cmd, "BODY[%s.TEXT] {%u}\r\n", rstrtmp.c_str(),
                          pMp->m_data->m_end - pMp->m_data->m_beg);
                  ImapSend(cmd, strlen(cmd));
                  ...
                  while (1) {
                    ...
                    char sztmp[1025];
                    dynamic_mmap_copy(
                        sztmp, lbuf, pMp->m_data->m_beg + nRead,
                        (tLen - nRead) > 1024 ? 1024 : (tLen - nRead));
                    sztmp[(tLen - nRead) > 1024 ? 1024 : (tLen - nRead)] = '\0';
              ...
              } else if (strmatch("BODY.PEEK[?*.HEADER]", vItem[i].c_str()) ==
                         TRUE) {
                if (i > 0 || isUID)
                  ImapSend(" ", 1);
                string rstrtmp;
              ...
              if (strmatch("BODY[HEADER.FIELDS (*)]", vItem[i].c_str()) ==
                  TRUE) {
                string strHeaderFileds;
                vector<string> vecHeaderFileds;
                ...
                ImapSend(cmd, strlen(cmd));
                ImapSend(strRespHeader.c_str(), strRespHeader.length());
              } else if (strmatch("BODY[HEADER.FIELDS.NOT (*)]",
                                  vItem[i].c_str()) == TRUE) {
                string strHeaderFileds;
                vector<string> vecHeaderFileds;
                strcut(szArg, "BODY[HEADER.FIELDS.NOT (", ")]",
                       strHeaderFileds);
                vSplitString(strHeaderFileds, vecHeaderFileds, " ");
                int vHeaderLen = vecHeaderFileds.size();
                string strRespHeader = "";
                ...
                ImapSend(cmd, strlen(cmd));
                ImapSend(strRespHeader.c_str(), strRespHeader.length());
              } else if (strcasecmp((char*)vItem[i].c_str(), (char*)"BODY[]") ==
                         0) {
                if (i > 0 || isUID)
                  ImapSend(" ", 1);
              ...
              } else if (strcasecmp((char*)vItem[i].c_str(), "BODY[HEADER]") ==
                         0) {
                if (i > 0 || isUID)
                  ImapSend(" ", 1);
              ...
              } else if (strcasecmp((char*)vItem[i].c_str(), "BODY[TEXT]") ==
                         0) {
                BOOL isText = FALSE;
                if (i > 0 || isUID)
                  ImapSend(" ", 1);
              ...
              } else if (strmatch("BODY[?*.HEADER]", vItem[i].c_str()) ==
                         TRUE) {
                if (i > 0 || isUID)
                  ImapSend(" ", 1);
            ...
            } else if (strmatch("BODY.PEEK[]<*.*>", vItem[i].c_str()) ||
              ...
              char read_buf[4096];
              int read_count = 0;
              while ((read_count = Letter->Read(
                          read_buf,
                          (llen - wlen) > 4096 ? 4096 : (llen - wlen))) >= 0) {
                if (read_count > 0) {
                  if (ImapSend(read_buf, read_count) < 0)
                    break;
              ...
              int nRead = 0;
              int tLen = Letter->GetSummary()->m_mime->m_end -
                         Letter->GetSummary()->m_mime->m_beg;
              while (1) {
                if (nRead >= tLen)
                  break;
                char sztmp[1025];
                dynamic_mmap_copy(
                    sztmp, lbuf, Letter->GetSummary()->m_mime->m_beg + nRead,
                    (tLen - nRead) > 1024 ? 1024 : (tLen - nRead));
                sztmp[(tLen - nRead) > 1024 ? 1024 : (tLen - nRead)] = '\0';
              ...
              if (Letter->GetSummary()->m_header->GetFrom()) {
                ...
                for (int k = 0;
                     k <
                     Letter->GetSummary()->m_header->GetFrom()->m_addrs.size();
                     k++) {
                  strEnve += "(";

                  strEnve += "\"";
                  strEnve += Letter->GetSummary()
                                 ->m_header->GetFrom()
                                 ->m_addrs[k]
                                 .pName;
                  strEnve += "\"";
              ...
              if (Letter->GetSummary()->m_header->GetSender()) {
                strEnve += "(";
                for (int k = 0; k < Letter->GetSummary()
                                        ->m_header->GetSender()
                                        ->m_addrs.size();
                     k++) {
                  strEnve += "(";
              ...
              if (Letter->GetSummary()->m_header->GetTo()) {
                ...
                for (int k = 0;
                     k <
                     Letter->GetSummary()->m_header->GetTo()->m_addrs.size();
                     k++) {
                  strEnve += "(";

                  strEnve += "\"";
                  strEnve +=
                      Letter->GetSummary()->m_header->GetTo()->m_addrs[k].pName;
                  strEnve += "\"";
                  strEnve += " ";
                  ...
                  strEnve += "\"";
                  strEnve +=
                      Letter->GetSummary()->m_header->GetTo()->m_addrs[k].mName;
                  strEnve += "\"";
                  strEnve += " ";

                  strEnve += "\"";
                  strEnve +=
                      Letter->GetSummary()->m_header->GetTo()->m_addrs[k].dName;
                  strEnve += "\"";
              ...
              if (Letter->GetSummary()->m_header->GetCc()) {
                ...
                for (int k = 0;
                     k <
                     Letter->GetSummary()->m_header->GetCc()->m_addrs.size();
                     k++) {
                  strEnve += "(";

                  strEnve += "\"";
                  strEnve +=
                      Letter->GetSummary()->m_header->GetCc()->m_addrs[k].pName;
                  strEnve += "\"";
                  strEnve += " ";
                  ...
                  strEnve += "\"";
                  strEnve +=
                      Letter->GetSummary()->m_header->GetCc()->m_addrs[k].mName;
                  strEnve += "\"";
                  strEnve += " ";

                  strEnve += "\"";
                  strEnve +=
                      Letter->GetSummary()->m_header->GetCc()->m_addrs[k].dName;
                  strEnve += "\"";
              ...
              if (Letter->GetSummary()->m_header->GetBcc()) {
                ...
                for (int k = 0;
                     k <
                     Letter->GetSummary()->m_header->GetBcc()->m_addrs.size();
                     k++) {
                  strEnve += "(";
              ...
              if (strcmp(Letter->GetSummary()->m_header->GetInReplyTo(), "") ==
                  0) {
                strEnve += "NIL ";
              } else {
                strEnve += "\"";
              ...
              if (strcmp(Letter->GetSummary()->m_header->GetMessageID(), "") ==
                  0) {
                strEnve += "NIL ";
              } else {
                strEnve += "\"";
              ...
              sprintf(cmd, "INTERNALDATE \"%s\"",
                      Letter->GetSummary()->m_header->GetDate());
              if (i > 0 || isUID)
                ImapSend(" ", 1);
              ImapSend(cmd, strlen(cmd));
...
void CMailImap::Store(const char* szArg, BOOL isUID) {
  ...
  for (int z = 0; z < zLen; z++) {
    ...
    for (int x = 0; x < vLen; x++) {
      if (((isUID ? m_maillisttbl[x].mid : x + 1) >= nBegin) &&
          ((isUID ? m_maillisttbl[x].mid : x + 1) <= nEnd)) {
        if (strncasecmp(strMsgItem.c_str(), "FLAGS.SILENT ",
                        sizeof("FLAGS.SILENT ") - 1) == 0) {
          mailStg->SetMailStatus(m_username.c_str(), m_maillisttbl[x].mid, 0);
          if (strMsgItem.find("\\Seen", 0, sizeof("\\Seen") - 1) !=
              string::npos) {
            unsigned int status;
            mailStg->GetMailStatus(m_maillisttbl[x].mid, status);
            status |= MSG_ATTR_SEEN;
            mailStg->SetMailStatus(m_username.c_str(), m_maillisttbl[x].mid,
                                   status);
          }
          if (strMsgItem.find("\\Answered", 0, sizeof("\\Answered") - 1) !=
              string::npos) {
            unsigned int status;
            mailStg->GetMailStatus(m_maillisttbl[x].mid, status);
            status |= MSG_ATTR_ANSWERED;
          ...
          if (strMsgItem.find("\\Flagged", 0, sizeof("\\Flagged") - 1) !=
              string::npos) {
            unsigned int status;
            mailStg->GetMailStatus(m_maillisttbl[x].mid, status);
            status |= MSG_ATTR_FLAGGED;
          ...
          if (strMsgItem.find("\\Deleted", 0, sizeof("\\Deleted") - 1) !=
              string::npos) {
            unsigned int status;
            mailStg->GetMailStatus(m_maillisttbl[x].mid, status);
            status |= MSG_ATTR_DELETED;
          ...
          if (strMsgItem.find("\\Draft", 0, sizeof("\\Draft") - 1) !=
              string::npos) {
            unsigned int status;
            mailStg->GetMailStatus(m_maillisttbl[x].mid, status);
            status |= MSG_ATTR_DRAFT;
          ...
          if (strMsgItem.find("\\Recent", 0, sizeof("\\Recent") - 1) !=
              string::npos) {
            unsigned int status;
            mailStg->GetMailStatus(m_maillisttbl[x].mid, status);
            status |= MSG_ATTR_RECENT;
        ...
        } else if (strncasecmp(strMsgItem.c_str(), "FLAGS ",
                               sizeof("FLAGS ") - 1) == 0) {
          mailStg->SetMailStatus(m_username.c_str(), m_maillisttbl[x].mid, 0);
          if (strMsgItem.find("\\Seen", 0, sizeof("\\Seen") - 1) !=
              string::npos) {
          ...
          if (isUID)
            sprintf(cmd, "* %u FETCH (FLAGS (%s)) UID %u\r\n", x + 1,
                    strStatus.c_str(), m_maillisttbl[x].mid);
          else
            sprintf(cmd, "* %u FETCH (FLAGS (%s))\r\n", x + 1,
                    strStatus.c_str());
          ImapSend(cmd, strlen(cmd));
        } else if (strncasecmp(strMsgItem.c_str(), "+FLAGS.SILENT ",
                               sizeof("+FLAGS.SILENT ") - 1) == 0) {
          if (strMsgItem.find("\\Seen", 0, sizeof("\\Seen") - 1) !=
              string::npos) {
            unsigned int status;
...
int CMailImap::Copy(const char* szArg, BOOL isUID) {
  ...
  for (int z = 0; z < zLen; z++) {
    ...
    strtrim(strMsgItem, "\" ");
    int dirID;
    if (mailStg->GetDirID(m_username.c_str(), strMsgItem.c_str(), dirID) ==
        -1) {
      return -1;
    }
    int vLen = m_maillisttbl.size();
    ...
    for (int x = 0; x < vLen; x++) {
      if (((isUID ? m_maillisttbl[x].mid : x + 1) >= nBegin) &&
        ...
        oldLetter = new MailLetter(mailStg, CMailBase::m_private_path.c_str(),
                                   CMailBase::m_encoding.c_str(), m_memcached,
                                   emlfile.c_str());
        newLetter = new MailLetter(mailStg, CMailBase::m_private_path.c_str(),
                                   CMailBase::m_encoding.c_str(), m_memcached,
                                   newuid, usermaxsize);
...
BOOL CMailImap::Parse(char* text, int len) {
  ...
  if (strncasecmp(strNotag.c_str(), "CAPABILITY", sizeof("CAPABILITY") - 1) ==
      0) {
    On_Capability(text);
  } else if (strncasecmp(strNotag.c_str(), "NOOP", sizeof("NOOP") - 1) == 0) {
    On_Noop(text);

  } else if (strncasecmp(strNotag.c_str(), "LOGOUT", sizeof("LOGOUT") - 1) ==
             0) {
    On_Logout(text);
    return FALSE;
  }
  ...
  // Non-athenticated state
  else if (strncasecmp(strNotag.c_str(), "AUTHENTICATE",
                       sizeof("AUTHENTICATE") - 1) == 0) {
    retValue = On_Authenticate(text);

  } else if (strncasecmp(strNotag.c_str(), "LOGIN", sizeof("LOGIN") - 1) == 0) {
    retValue = On_Login(text);

  } else if (strncasecmp(strNotag.c_str(), "SELECT", sizeof("SELECT") - 1) ==
             0) {
    On_Select(text);

  } else if (strncasecmp(strNotag.c_str(), "EXAMINE", sizeof("EXAMINE") - 1) ==
             0) {
    On_Examine(text);

  } else if (strncasecmp(strNotag.c_str(), "CREATE", sizeof("CREATE") - 1) ==
             0) {
    On_Create(text);

  } else if (strncasecmp(strNotag.c_str(), "DELETE", sizeof("DELETE") - 1) ==
             0) {
    On_Delete(text);

  } else if (strncasecmp(strNotag.c_str(), "RENAME", sizeof("RENAME") - 1) ==
             0) {
    On_Rename(text);

  } else if (strncasecmp(strNotag.c_str(), "SUBSCRIBE",
                         sizeof("SUBSCRIBE") - 1) == 0) {
    On_Subscribe(text);

  } else if (strncasecmp(strNotag.c_str(), "UNSUBSCRIBE",
                         sizeof("UNSUBSCRIBE") - 1) == 0) {
    On_Unsubscribe(text);
    ...
    On_Listsub(text);
  } else if (strncasecmp(strNotag.c_str(), "STATUS", sizeof("STATUS") - 1) ==
             0) {
    On_Status(text);

  } else if (strncasecmp(strNotag.c_str(), "APPEND", sizeof("APPEND") - 1) ==
             0) {
    On_Append(text);
  ...
  } else if (strncasecmp(strNotag.c_str(), "EXPUNGE", sizeof("EXPUNGE") - 1) ==
             0) {
    On_Expunge(text);

  } else if (strncasecmp(strNotag.c_str(), "SEARCH", sizeof("SEARCH") - 1) ==
             0) {
    On_Search(text);
  ...
  } else if (strncasecmp(strNotag.c_str(), "STARTTLS",
                         sizeof("STARTTLS") - 1) == 0) {
    On_STARTTLS(text);
  } else if (strncasecmp(strNotag.c_str(), "X", sizeof("X") - 1) == 0) {
    On_X_atom(text);
  } else if (strncasecmp(strNotag.c_str(), "NAMESPACE",
                         sizeof("NAMESPACE") - 1) == 0) {
    On_Namespace(text);
  } else {
    On_Unknown(text);
  }
...
void CMailImap::SearchRecv(const char* szTag,
                           const char* szCmd,
                           string& strsearch) {
  if (strmatch("*{*}\r\n", strsearch.c_str())) {
    char cmd[1024];
...
void CMailImap::Search(const char* szTag,
                       const char* szCmd,
                       const char* szArg) {
  MailStorage* mailStg;
  StorageEngineInstance stgengine_instance(m_storageEngine, &mailStg);
...
void CMailImap::SubSearch(MailStorage* mailStg,
                          const char* szArg,
                          map<int, int>& vSearchResult) {
    ...
    if (vItem[i].isDeep) {
      ...
      if (vItem[i].item == "ALL") {
        ...
        if (eSearchRelation == ORing) {
          if (bIsNot == TRUE) {
            for (int y = 0; y < listtbllen; y++) {
              if ((m_maillisttbl[y].mstatus & MSG_ATTR_ANSWERED) !=
                  MSG_ATTR_ANSWERED) {
                vSearchResult[m_maillisttbl[y].mid] = 1;
              }
            }
          ...
          if (bIsNot == TRUE) {
            for (int y = 0; y < listtbllen; y++) {
              ...
              MailLetter* Letter = new MailLetter(
                  mailStg, CMailBase::m_private_path.c_str(),
                  CMailBase::m_encoding.c_str(), m_memcached, emlfile.c_str());
              if (Letter->GetSize() > 0) {
                string bcc =
                    Letter->GetSummary()->m_header->GetBcc()->m_strfiled;
                string condition = strmatch("\"*\"", vItem[i + 1].item.c_str())
                                       ? vItem[i + 1].item.substr(
                                             1, vItem[i + 1].item.length() - 2)
                                       : vItem[i + 1].item;
                if (bcc.find(condition.c_str(), 0, condition.length()) ==
                    string::npos) {
                  vSearchResult[m_maillisttbl[y].mid] = 1;
                }
              }
          ...
          if (bIsNot == TRUE) {
            for (int y = 0; y < listtbllen; y++) {
              ...
              if (Letter->GetSize() > 0) {
                if (datecmp(
                        year1, month1, day1,
                        Letter->GetSummary()->m_header->GetTm()->tm_year + 1900,
                        Letter->GetSummary()->m_header->GetTm()->tm_mon,
                        Letter->GetSummary()->m_header->GetTm()->tm_mday) <=
                    0) {
                  vSearchResult[m_maillisttbl[y].mid] = 1;
                }
              }
              ...
              if (Letter->GetSize() > 0) {
                ...
                string condition = strmatch("\"*\"", vItem[i + 1].item.c_str())
                                       ? vItem[i + 1].item.substr(
                                             1, vItem[i + 1].item.length() - 2)
                                       : vItem[i + 1].item;
                if (cc.find(condition.c_str(), 0, condition.length()) ==
                    string::npos) {
                  vSearchResult[m_maillisttbl[y].mid] = 1;
                }
              }
        ...
        if (eSearchRelation == ORing) {
          if (bIsNot == TRUE) {
            for (int y = 0; y < listtbllen; y++) {
              if ((m_maillisttbl[y].mstatus & MSG_ATTR_DELETED) !=
                  MSG_ATTR_DELETED) {
                vSearchResult[m_maillisttbl[y].mid] = 1;
              }
            }
        ...
        if (eSearchRelation == ORing) {
          if (bIsNot == TRUE) {
            for (int y = 0; y < listtbllen; y++) {
              if ((m_maillisttbl[y].mstatus & MSG_ATTR_DRAFT) !=
                  MSG_ATTR_DRAFT) {
                vSearchResult[m_maillisttbl[y].mid] = 1;
              }
            }
        ...
        if (eSearchRelation == ORing) {
          if (bIsNot == TRUE) {
            for (int y = 0; y < listtbllen; y++) {
              if ((m_maillisttbl[y].mstatus & MSG_ATTR_FLAGGED) !=
                  MSG_ATTR_FLAGGED) {
                vSearchResult[m_maillisttbl[y].mid] = 1;
              }
            }
          ...
          if (bIsNot == TRUE) {
            if (strmatch("{*}", vItem[i + 1].item.c_str())) {
              for (int y = 0; y < listtbllen; y++) {
                ...
                MailLetter* Letter =
                    new MailLetter(mailStg, CMailBase::m_private_path.c_str(),
                                   CMailBase::m_encoding.c_str(), m_memcached,
                                   emlfile.c_str());
                if (Letter->GetSize() > 0) {
                  string subject =
                  ...
                  string condition =
                      strmatch("\"*\"", vItem[i + 2].item.c_str())
                          ? vItem[i + 2].item.substr(
                                1, vItem[i + 2].item.length() - 2)
                          : vItem[i + 2].item;
                  if (subject.find(condition.c_str(), 0, condition.length()) ==
                      string::npos) {
                    vSearchResult[m_maillisttbl[y].mid] = 1;
                  }
                }
          ...
          if (bIsNot == TRUE) {
            for (int y = 0; y < listtbllen; y++) {
              if ((time(NULL) - m_maillisttbl[y].mtime > RECENT_MSG_TIME) &&
                  ((m_maillisttbl[y].mstatus & MSG_ATTR_SEEN) ==
                   MSG_ATTR_SEEN)) {
                vSearchResult[m_maillisttbl[y].mid] = 1;
              }
            }
              ...
              if (Letter->GetSize() > 0) {
                ...
                string condition = strmatch("\"*\"", vItem[i + 1].item.c_str())
                                       ? vItem[i + 1].item.substr(
                                             1, vItem[i + 1].item.length() - 2)
                                       : vItem[i + 1].item;
                if (from.find(condition.c_str(), 0, condition.length()) ==
                    string::npos) {
                  vSearchResult[m_maillisttbl[y].mid] = 1;
                }
              }
              ...
              if (Letter->GetSize() > 0) {
                ...
                string condition = strmatch("\"*\"", vItem[i + 1].item.c_str())
                                       ? vItem[i + 1].item.substr(
                                             1, vItem[i + 1].item.length() - 2)
                                       : vItem[i + 1].item;
                if (to.find(condition.c_str(), 0, condition.length()) ==
                    string::npos) {
                  vSearchResult[m_maillisttbl[y].mid] = 1;
                }
              }
