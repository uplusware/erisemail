...
typedef enum { upReadOnly = 0, upReadWrite = 1 } User_Privilege;
...
typedef enum { utMember = 1, utGroup = 2 } UserType;

typedef enum { ldFalse = 0, ldTrue = 1 } LevelDeault;

typedef enum { eaFalse = 0, eaTrue = 1 } EnableAudit;
...
typedef enum { usActive = 0, usDisabled = 1 } UserStatus;

typedef enum { mtLocal = 1, mtExtern = 2, mtFowarding = 3 } MailType;
...
class MailStorage {
 ...
 public:
  MailStorage(const char* encoding,
  ...
  int AddID(const char* username,
            ...
            int level,
            int status = usActive);
  int UpdateID(const char* username,
               const char* alias,
               const char* host,
               UserStatus status,
               int level);
  ...
  int AddLevel(const char* lname,
               const char* ldescription,
               unsigned long long mailmaxsize,
               unsigned long long boxlmaxsize,
               EnableAudit lenableaudit,
               unsigned int mailsizethreshold,
               unsigned int attachsizethreshold,
               unsigned int& lid);
  int UpdateLevel(unsigned int lid,
                  const char* lname,
                  const char* ldescription,
                  unsigned long long mailmaxsize,
                  unsigned long long boxmaxsize,
  ...
  int LoadMailFromFile(const char* mfrom,
                       const char* mto,
                       unsigned int mtime,
                       unsigned int mtx,
                       const char* muniqid,
                       ...
                       int& mailid);
  int UpdateMailFromFile(const char* mfrom,
                         const char* mto,
                         unsigned int mtime,
                         unsigned int mtx,
                         const char* muniqid,
  ...
  int ListMailByDir(const char* username,
  ...
  int ListAvailableExternMail(vector<Mail_Info>& listtbl,
                              unsigned int mta_index = 0,
                              unsigned int mta_count = 1,
                              unsigned int max_num = 0);

  int ListID(vector<User_Info>& listtbl,
             string orderby = "utime",
             BOOL desc = FALSE);

  int FilterID(vector<User_Info>& listtbl,
               const char* filter,
               string orderby = "utime",
               BOOL desc = FALSE);
  ...
  int SetDirStatus(const char* username,
                   const char* dirref,
                   unsigned int status);
  int GetDirStatus(const char* username,
                   const char* dirref,
                   unsigned int& status);

  int SetMailStatus(const char* username,
                    int mid,
                    unsigned int status,
                    unsigned int mtx = mtLocal);
  int GetMailStatus(int mid, unsigned int& status, unsigned int mtx = mtLocal);

  int GetMailFromAndTo(int mid,
                       string& from,
                       string& to,
                       unsigned int mtx = mtLocal);
  ...
  int GetDirParentID(const char* username,
                     const char* dirref,
  ...
  int TraversalListDir(const char* username,
                       const char* dirref,
  ...
  int LimitListMailByDir(const char* username,
                         vector<Mail_Info>& listtbl,
                         unsigned int dirid,
                         int beg,
                         int rows);
  int LimitListUnauditedExternMailByDir(const char* username,
                                        vector<Mail_Info>& listtbl,
                                        int beg,
                                        int rows);

  int InsertMail(const char* mfrom,
                 const char* mto,
                 unsigned int mtime,
                 unsigned int mtx,
                 const char* muniqid,
                 ...
                 unsigned int msize,
                 int& mailid);
  int UpdateMail(const char* mfrom,
                 const char* mto,
                 unsigned int mtime,
                 unsigned int mtx,
                 const char* muniqid,
                 ...
                 unsigned int msize,
                 int mailid);
  int InsertMailIndex(const char* mfrom,
                      const char* mto,
                      const char* mhost,
                      unsigned int mtime,
                      unsigned int mtx,
                      ...
                      unsigned int msize,
                      int& mailid);
  int UpdateMailIndex(const char* mfrom,
                      const char* mto,
                      unsigned int mtime,
                      unsigned int mtx,
                      const char* muniqid,
  ...
  int GetGlobalStorage(unsigned int& commonMailNumber,
                       unsigned int& deletedMailNumber,
                       unsigned int& commonMailSize,
                       unsigned int& deletedMailSize);
  int GetUserStorage(const char* username,
  ...
  int GetMTAIndex(const char* mta,
...
class mysql_query_lock {
 public:
  ...
  virtual ~mysql_query_lock() { pthread_mutex_unlock(m_lock); }
