...
int Service::GetInstanceMaxThreadNum() {
  return CMailBase::m_prod_type == PROD_INSTANT_MESSENGER
             ? CMailBase::m_xmpp_worker_thread_num
             : CMailBase::m_mda_max_conn;
}

void Service::SESSION_HANDLER(Session_Arg* session_arg) {
  Session* pSession = NULL;
  try {
    pSession = new Session(session_arg->sockfd, session_arg->ssl,
                           session_arg->ssl_ctx, session_arg->client_ip.c_str(),
                           session_arg->client_port, session_arg->svr_type,
                           session_arg->is_ssl, session_arg->storage_engine,
                           session_arg->cache, session_arg->memcached);
  ...
  while (m_STATIC_THREAD_POOL_EXIT) {
    ...
    if (sem_timedwait(&m_STATIC_THREAD_POOL_SEM, &ts) == 0) {
      ...
      if (session_arg) {
        ...
        if (CMailBase::m_prod_type == PROD_INSTANT_MESSENGER) {
          p_session_group->Accept(
              session_arg->sockfd, session_arg->ssl, session_arg->ssl_ctx,
              session_arg->client_ip.c_str(), session_arg->client_port,
              session_arg->svr_type, session_arg->is_ssl,
              session_arg->storage_engine, session_arg->cache,
              session_arg->memcached);
        } else {
          pthread_rwlock_wrlock(&m_STATIC_THREAD_IDLE_NUM_LOCK);
          m_STATIC_THREAD_IDLE_NUM--;
          pthread_rwlock_unlock(&m_STATIC_THREAD_IDLE_NUM_LOCK);
        ...
        if (time(NULL) - last_service_idle_time >
            CMailBase::m_service_idle_timeout) {
...
int Service::create_server_service(CUplusTrace& uTrace,
                                   const char* hostip,
  ...
  if (!hostip || hostip[0] == '\0') {
    ...
    nFlag = 1;
    setsockopt(srv_sockfd, SOL_SOCKET, SO_REUSEADDR, (char*)&nFlag,
               sizeof(nFlag));

    nFlag = 0;
    setsockopt(srv_sockfd, SOL_SOCKET, IPV6_V6ONLY, (char*)&nFlag,
               sizeof(nFlag));
    ...
    int s = getaddrinfo((hostip && hostip[0] != '\0') ? hostip : NULL, sz_port,
                        &hints, &server_addr);
    if (s != 0) {
  ...
  if (listen(srv_sockfd, 128) == -1) {
    uTrace.Write(Trace_Error, "Service LISTEN error, %s:%u",
                 hostip ? hostip : "", hostport);
    srv_sockfd = -1;
    return srv_sockfd;
  }
  uTrace.Write(Trace_Msg, "Service works on %s:%u", hostip ? hostip : "",
               hostport);

  return srv_sockfd;
}

int Service::create_client_session(CUplusTrace& uTrace,
                                   int& clt_sockfd,
                                   Service_Type st,
                                   BOOL is_ssl,
  ...
  if (CMailBase::m_permit_list.size() > 0) {
    ...
    for (int x = 0; x < CMailBase::m_permit_list.size(); x++) {
      if (strmatch(CMailBase::m_permit_list[x].c_str(), client_ip.c_str()) ==
          TRUE) {
        access_result = TRUE;
        break;
      }
    ...
    for (int x = 0; x < CMailBase::m_reject_list.size(); x++) {
      if ((strmatch(CMailBase::m_reject_list[x].ip.c_str(),
  ...
  if (access_result == FALSE) {
    ...
    if (pthread_mutex_trylock(&m_STATIC_THREAD_POOL_MUTEX) == 0) {
      while (!m_local_thread_pool_arg_queue.empty()) {
        Session_Arg* previous_session_arg =
            m_local_thread_pool_arg_queue.front();
    ...
    if (current_idle_thread_num < CMailBase::m_thread_increase_step &&
        current_thread_pool_size < GetInstanceMaxThreadNum()) {
      worker_pool.More(
          (GetInstanceMaxThreadNum() - current_thread_pool_size) <
                  CMailBase::m_thread_increase_step
              ? (GetInstanceMaxThreadNum() - current_thread_pool_size)
              : CMailBase::m_thread_increase_step);
    }
  }
...
int Service::Run(int fd, vector<service_param_t>& server_params) {
  ...
  memcached_return rc;
  m_memcached = NULL;
  for (map<string, int>::iterator iter = CMailBase::m_memcached_list.begin();
       iter != CMailBase::m_memcached_list.end(); ++iter) {
    if (!m_memcached)
      m_memcached = memcached_create(NULL);
    memcached_servers = memcached_server_list_append(
        memcached_servers, (*iter).first.c_str(), (*iter).second, &rc);
    rc = memcached_server_push(m_memcached, memcached_servers);

    uTrace.Write(Trace_Msg, "memcached server@%s:%d", (*iter).first.c_str(),
                 (*iter).second);
  }

  m_storageEngine = new StorageEngine(
      CMailBase::m_db_host.c_str(), CMailBase::m_db_username.c_str(),
      CMailBase::m_db_password.c_str(), CMailBase::m_db_name.c_str(),
      CMailBase::m_db_max_conn, CMailBase::m_db_port,
      CMailBase::m_db_sock_file.c_str(), CMailBase::m_private_path.c_str(),
      CMailBase::m_encoding.c_str(), m_memcached);

  if (!m_storageEngine) {
    uTrace.Write(Trace_Error, "%s::%s Create Storage Engine Error", __FILE__,
                 __FUNCTION__);
    return -1;
  }
  m_child_list.clear();
  ...
  ThreadPool worker_pool(
      GetInstanceMaxThreadNum(), INIT_THREAD_POOL_HANDLER,
      BEGIN_THREAD_POOL_HANDLER, NULL, 0, EXIT_THREAD_POOL_HANDLER,
      GetInstanceMaxThreadNum() > CMailBase::m_thread_increase_step
          ? CMailBase::m_thread_increase_step
          : GetInstanceMaxThreadNum());
  ...
  while (!svr_exit) {
    int max_fd = -1;
    for (int x = 0; x < server_params.size(); x++) {
      create_server_service(uTrace, server_params[x].host_ip.c_str(),
                            server_params[x].host_port,
                            server_params[x].sockfd);
      if (server_params[x].sockfd > 0) {
        ...
        max_fd =
            max_fd > server_params[x].sockfd ? max_fd : server_params[x].sockfd;
      }
    }
    ...
    while (1) {
      ...
      if (rc != -1) {
        ...
        if (pQMsg->cmd == MSG_EXIT) {
          ...
          for (x = CMailBase::m_reject_list.begin();
               x != CMailBase::m_reject_list.end();) {
      ...
      if (rc == -1) {
        ...
        if (!m_local_thread_pool_arg_queue.empty() &&
            pthread_mutex_trylock(&m_STATIC_THREAD_POOL_MUTEX) == 0) {
          last_service_idle_time = time(NULL);
        ...
        for (int x = 0; x < server_params.size(); x++) {
          if (server_params[x].sockfd > 0 &&
              FD_ISSET(server_params[x].sockfd, &accept_mask)) {
            FD_CLR(server_params[x].sockfd, &accept_mask);
            clt_sockfd = accept(server_params[x].sockfd, (sockaddr*)&clt_addr,
                                &clt_size);
            if (clt_sockfd > 0) {
              is_ssl = server_params[x].is_ssl;
              if (create_client_session(uTrace, clt_sockfd, server_params[x].st,
                                        is_ssl, clt_addr, clt_size,
                                        worker_pool) < 0)
                continue;
        ...
        if (CMailBase::m_max_service_request_num != 0 &&
            max_service_request_num > CMailBase::m_max_service_request_num) {
          svr_exit = TRUE;
          break;
        }
      }
