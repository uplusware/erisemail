...
#define RSP_200_OK_CACHE                          \
  "HTTP/1.1 200 OK\r\n" ERISEMAIL_WEB_SERVER_NAME \
  "\r\n"                                          \
  "Cache-Control: max-age=31536000\r\n"

#define RSP_200_OK_NO_CACHE                       \
  "HTTP/1.1 200 OK\r\n" ERISEMAIL_WEB_SERVER_NAME \
  "\r\n"                                          \
  "Pragma: no-cache\r\n"                          \
...
#define RSP_200_OK_CACHE_PLAIN                    \
  "HTTP/1.1 200 OK\r\n" ERISEMAIL_WEB_SERVER_NAME \
  "\r\n"                                          \
  "Content-Type: text/plain\r\n"

#define RSP_200_OK_NO_CACHE_PLAIN                 \
  "HTTP/1.1 200 OK\r\n" ERISEMAIL_WEB_SERVER_NAME \
  "\r\n"                                          \
  "Pragma: no-cache\r\n"                          \
  "Cache-Control: no-cache, must-revalidate\r\n"  \
...
#define RSP_301_MOVED_NO_CACHE \
  "HTTP/1.1 301 Moved Permanently\r\n" ERISEMAIL_WEB_SERVER_NAME "\r\n"

#define RSP_304_NOT_MODIFIED \
  "HTTP/1.1 304 Not Modified\r\n" ERISEMAIL_WEB_SERVER_NAME "\r\n"

/* Non html body (Content-Length: 0) */
#define RSP_404_NOT_FOUND                                \
  "HTTP/1.1 404 Not Found\r\n" ERISEMAIL_WEB_SERVER_NAME \
  "\r\n"                                                 \
  "Content-Length: 0\r\n"                                \
  "\r\n"

#define RSP_500_SYS_ERR                                     \
  "HTTP/1.1 500 Server Error\r\n" ERISEMAIL_WEB_SERVER_NAME \
  "\r\n"                                                    \
  "Content-Length: 0\r\n"                                   \
  "\r\n"

#define RSP_501_SYS_ERR                                        \
...
class doc {
 ...
 public:
  ...
  virtual void Response() {
    ...
    if (m_session->m_cache->m_htdoc.find(strDoc) ==
        m_session->m_cache->m_htdoc.end()) {
      ...
      if (!fs_doc || !fs_doc->is_open()) {
        ...
        if (strcasecmp(m_session->GetIfModifiedSince(),
                       strLastModifiedDate.c_str()) == 0) {
      ...
      string strLastModifiedDate;
      OutHTTPDateString(m_session->m_cache->m_htdoc[strDoc].last_modified,
                        strLastModifiedDate);
      ...
      if (strcasecmp(m_session->GetIfModifiedSince(),
        ...
        char szContentLength[64];
        sprintf(szContentLength, "%u",
                m_session->m_cache->m_htdoc[strDoc].flen);
        strResp += "Content-Length: ";
        strResp += szContentLength;
        strResp += "\r\n";
        strResp += "\r\n";
        ...
        m_session->EnableKeepAlive(TRUE);
        m_session->HttpSend(strResp.c_str(), strResp.length());
        m_session->HttpSend(m_session->m_cache->m_htdoc[strDoc].pbuf,
                            m_session->m_cache->m_htdoc[strDoc].flen);
      }
    }
  }
 ...
 protected:
  int generate_cookie_content(const char* szName,
                              const char* szValue,
                              const char* szPath,
                              string& strOut) {
    strOut = szName;
  ...
  int check_userauth_token(const char* token, string& username) {
    ...
    if (Security::Decrypt(encryted_username_role.c_str(),
                          encryted_username_role.length(),
                          decryted_username_role, CMailBase::DESKey()) == -1) {
      return -1;
    }

    for (int x = 0; x < decryted_username_role.length(); x++) {
      if ((decryted_username_role[x] >= 'a' &&
           decryted_username_role[x] <= 'z') ||
          (decryted_username_role[x] >= 'A' &&
           decryted_username_role[x] <= 'Z') ||
          (decryted_username_role[x] >= '0' &&
           decryted_username_role[x] <= '9') ||
          (decryted_username_role[x] == '_' ||
           decryted_username_role[x] == '.' ||
           decryted_username_role[x] == ':')) {
        ...
        if (plain_username != "" && plain_username == decryted_username &&
            decryted_role == "U") {
  ...
  void MailFileData(MimeSummary* part,
                    string filename,
                    string& type,
                    string& dispos,
                    fbuffer& fbuf,
    ...
    if (part->GetSubPartNumber() == 0) {
      if (part->GetContentType() != NULL) {
        if (part->GetContentType()->m_filename == filename ||
            (part->GetContentDisposition() &&
             part->GetContentDisposition()->m_filename == filename)) {
          type = part->GetContentType()->m_type1;
          type += "/";

          type += part->GetContentType()->m_type2;

          dispos = (part->GetContentDisposition() == NULL
                        ? ""
                        : part->GetContentDisposition()->m_disposition);
          ...
          for (int x = part->m_data->m_beg; x < part->m_data->m_end; x++) {
            ...
            if (lbuf[x] == '\n') {
              if (strcasecmp(part->GetContentTransferEncoding(), "base64") ==
                  0) {
                strtrim(strtext);
                if (strtext != "") {
                  ...
                  CBase64::Decode((char*)strtext.c_str(), strtext.length(),
                                  szbuf, &nlen);
                  fbuf.bufcat(szbuf, nlen);
                  free(szbuf);
                }
              } else if ((strcasecmp(part->GetContentTransferEncoding(),
                                     "QP") == 0) ||
                         (strcasecmp(part->GetContentTransferEncoding(),
                                     "quoted-printable") == 0)) {
                strtrim(strtext);
                if (strtext != "") {
                  ...
                  char* szbuf = (char*)malloc(nlen + 1);
                  memset(szbuf, 0, nlen + 1);
                  DecodeQuoted(strtext.c_str(), strtext.length() / 3 * 3,
                               (unsigned char*)szbuf);
  ...
  void MailFileDataEx(MimeSummary* part,
                      string contentid,
                      string& type,
                      string& dispos,
                      fbuffer& fbuf,
  ...
  void ExtractAttach(const char* username,
                     MimeSummary* part,
                     vector<AttInfo>& attlist,
                     dynamic_mmap& lbuf) {
    if (part->GetSubPartNumber() == 0) {
      if (part->GetContentType() != NULL) {
        if (strcasecmp(part->GetContentType()->m_type1.c_str(), "multipart") !=
                0 &&
            strcasecmp(part->GetContentType()->m_type2.c_str(), "mixed") != 0) {
          if (part->GetContentType()->m_filename != "" ||
              (part->GetContentDisposition() &&
               part->GetContentDisposition()->m_filename != "")) {
            AttInfo att;
            att.filename = part->GetContentType()->m_filename == ""
                               ? part->GetContentDisposition()->m_filename
            ...
            if (code_convert_ex(part->GetContentType()->m_charset.c_str(),
                                CMailBase::m_encoding.c_str(),
                                att.filename.c_str(), strSrc) != 0) {
              strSrc = att.filename;
            }
            ...
            sprintf(szFileName, "%s_%08x_%08x_%016lx_%08x.att", username,
            ...
            ofstream* attachfd = new ofstream(
                strFilePath.c_str(), ios_base::binary | ios::out | ios::trunc);
            string strTmp;
            if (attachfd->is_open()) {
              ...
              while (1) {
                ...
                char tmp_buf[1024];
                dynamic_mmap_copy(
                    tmp_buf, lbuf, part->m_beg + nWrite,
                    (tLen - nWrite) > 1024 ? 1024 : (tLen - nWrite));
                attachfd->write(
                    tmp_buf, (tLen - nWrite) > 1024 ? 1024 : (tLen - nWrite));
  ...
  BOOL isCalendar(MimeSummary* part) {
    if (part->GetSubPartNumber() == 0) {
      if (part->GetContentType() != NULL) {
        if ((strcasecmp(part->GetContentType()->m_type1.c_str(), "text") ==
             0) &&
            (part->GetContentType()->m_filename == "")) {
          if (strcasecmp(part->GetContentType()->m_type2.c_str(), "calendar") ==
              0) {
            return TRUE;
          }
        }
  ...
  void MailInfo(MimeSummary* part,
                string& strtext,
                string& strhtml,
                string& strcalendar,
                string& strattach,
    ...
    if (part->GetSubPartNumber() == 0) {
      if (part->GetContentType() != NULL) {
        if ((strcasecmp(part->GetContentType()->m_type1.c_str(), "text") ==
             0) &&
            (part->GetContentType()->m_filename == "")) {
          if (strcasecmp(part->GetContentType()->m_type2.c_str(), "plain") ==
              0) {
            string strtmp = "";
            for (int x = part->m_data->m_beg; x < part->m_data->m_end; x++) {
              strtmp += lbuf[x];
              if (lbuf[x] == '\n') {
                if (strcasecmp(part->GetContentTransferEncoding(), "base64") ==
                  ...
                  CBase64::Decode((char*)strtmp.c_str(), strtmp.length(), bbuf,
                                  &blen);

                  string strConvert;
                  if (code_convert_ex(part->GetContentType()->m_charset.c_str(),
                                      CMailBase::m_encoding.c_str(), bbuf,
                                      strConvert) == 0) {
                    strtext += strConvert;
                  } else {
                    strtext += bbuf;
                  ...
                  char* bbuf = (char*)malloc(blen + 1);
                  memset(bbuf, 0, blen + 1);
                  DecodeQuoted(strtmp.c_str(), strtmp.length() / 3 * 3,
                               (unsigned char*)bbuf);
          ...
          } else if (strcasecmp(part->GetContentType()->m_type2.c_str(),
                                "html") == 0) {
            string strtmp = "";
            for (int x = part->m_data->m_beg; x < part->m_data->m_end; x++) {
              strtmp += lbuf[x];
          ...
          } else if (strcasecmp(part->GetContentType()->m_type1.c_str(),
                                "text") == 0 &&
                     strcasecmp(part->GetContentType()->m_type2.c_str(),
                                "calendar") == 0) {
            string strtmp = "";
            for (int x = part->m_data->m_beg; x < part->m_data->m_end; x++) {
              strtmp += lbuf[x];
      ...
      for (int a = 0; a < part->GetSubPartNumber(); a++) {
        MailInfo(part->GetPart(a), strtext, strhtml, strcalendar, strattach,
                 lbuf);
      }
    }
  }
...
class ApiAddUserToGroup : public doc, public storage {
 public:
  ApiAddUserToGroup(CHttp* session)
      : doc(session), storage(session->GetStorageEngine()) {}
  ...
  virtual void Response() {
    ...
    if (CMailBase::m_webadmin_list.size() == 0) {
      ...
      for (int x = 0; x < CMailBase::m_webadmin_list.size(); x++) {
        if (strmatch(CMailBase::m_webadmin_list[x].c_str(),
                     m_session->m_clientip.c_str()) == TRUE) {
          access_result = TRUE;
          break;
        }
      }
    ...
    if (check_adminauth_token(strauth.c_str(), username) == 0) {
      ...
      strResp +=
          "<?xml version='1.0' encoding='" + CMailBase::m_encoding + "'?>";
      ...
      strResp +=
          "<erisemail><response errno=\"0\" "
          "reason=\"\"></response></erisemail>";
    } else {
      strResp = RSP_200_OK_XML;
      string strHTTPDate;
      OutHTTPDateString(time(NULL), strHTTPDate);
...
class ApiDelUserFromGroup : public doc, public storage {
 public:
  ApiDelUserFromGroup(CHttp* session)
      : doc(session), storage(session->GetStorageEngine()) {}
 ...
 public:
  ...
  virtual void Response() {
    ...
    if (check_adminauth_token(strauth.c_str(), username) == 0) {
      ...
      strHost = "";
#endif /* _WITH_DIST_ */
      code_convert_ex("UTF-8", CMailBase::m_encoding.c_str(), strAlias.c_str(),
                      strAlias);
      ...
      unsigned int lid;
      if (m_mailStg &&
          m_mailStg->AddID(strUserName.c_str(), strUserPwd.c_str(),
                           strAlias.c_str(), strHost.c_str(),
                           strType == "member" ? utMember : utGroup,
                           urGeneralUser, 5000, atoi(strLevel.c_str())) == 0) {
        strResp +=
            "<erisemail><response errno=\"0\" "
            "reason=\"\"></response></erisemail>";
 ...
 public:
  ...
  virtual void Response() {
    ...
    if (check_adminauth_token(strauth.c_str(), username) == 0) {
      ...
      unsigned int lid;
      if (m_mailStg &&
          m_mailStg->UpdateID(strUserName.c_str(), strAlias.c_str(),
                              strHost.c_str(),
                              strStatus == "Active" ? usActive : usDisabled,
                              atoi(strLevel.c_str())) == 0) {
        strResp +=
            "<erisemail><response errno=\"0\" "
            "reason=\"\"></response></erisemail>";
 ...
 public:
  ...
  virtual void Response() {
    ...
    if (check_adminauth_token(strauth.c_str(), username) == 0) {
      ...
      m_session->parse_urlencode_value("EDIT_MAILSIZETHRESHOLD",
                                       strMailsizethreshold);
      m_session->parse_urlencode_value("EDIT_ATTACHSIZETHRESHOLD",
                                       strAttchsizethreshold);
      m_session->parse_urlencode_value("EDIT_DEFAULT", strDefault);

      code_convert_ex("UTF-8", CMailBase::m_encoding.c_str(),
                      strDescription.c_str(), strDescription);
      ...
      if (m_mailStg && m_mailStg->UpdateLevel(
                           lid, strLevelName.c_str(), strDescription.c_str(),
                           atollu(strMailmaxsize.c_str()) * 1024,
                           atollu(strBoxmaxsize.c_str()) * 1024,
                           (strAudit == "true" ? eaTrue : eaFalse),
                           atoi(strMailsizethreshold.c_str()) * 1024,
                           atoi(strAttchsizethreshold.c_str()) * 1024) == 0) {
        if (strDefault == "true") {
 ...
 public:
  ...
  virtual void Response() {
    ...
    if (check_adminauth_token(strauth.c_str(), username) == 0) {
      ...
      m_session->parse_urlencode_value("NEW_MAILSIZETHRESHOLD",
                                       strMailsizethreshold);
      m_session->parse_urlencode_value("NEW_ATTACHSIZETHRESHOLD",
                                       strAttchsizethreshold);
      ...
      unsigned int lid;
      if (m_mailStg &&
          m_mailStg->AddLevel(strLevelName.c_str(), strDescription.c_str(),
                              atollu(strMailmaxsize.c_str()) * 1024,
                              atollu(strBoxmaxsize.c_str()) * 1024,
                              (strAudit == "true" ? eaTrue : eaFalse),
                              atoi(strMailsizethreshold.c_str()) * 1024,
                              atoi(strAttchsizethreshold.c_str()) * 1024,
                              lid) == 0) {
        if (strDefault == "true") {
          if (m_mailStg && m_mailStg->SetDefaultLevel(lid) == 0) {
            strResp +=
...
#ifdef _WITH_DIST_
        string strline;
        ifstream clustersfilein(CMailBase::m_clusters_list_file.c_str(),
                                ios_base::binary);
        if (clustersfilein.is_open()) {
          while (getline(clustersfilein, strline)) {
            ...
            if (strline != "" && strncmp(strline.c_str(), "#", 1) != 0) {
              ...
              if (strcasecmp(strHost.c_str(),
                             CMailBase::m_localhostname.c_str()) != 0) {
...
class ApiListUsers : public doc, public storage {
 public:
  ...
  virtual void Response() {
    ...
    if (check_adminauth_token(strauth.c_str(), username) == 0 ||
        check_userauth_token(strauth.c_str(), username) == 0) {
      ...
      if (m_mailStg &&
          m_mailStg->ListID(listtbl, strorderby.c_str(),
                            strdesc == "true" ? TRUE : FALSE) == 0) {
        char szTmp[64];
        strResp = RSP_200_OK_XML;
        string strHTTPDate;
        OutHTTPDateString(time(NULL), strHTTPDate);
...
class ApiListMembersofGroup : public doc, public storage {
 public:
  ApiListMembersofGroup(CHttp* session)
      : doc(session), storage(session->GetStorageEngine()) {}
  ...
  virtual void Response() {
    ...
    if (check_adminauth_token(strauth.c_str(), username) == 0 ||
      ...
      vector<User_Info> listtbl;
      if (m_mailStg &&
          m_mailStg->ListMemberOfGroup(strgroup.c_str(), listtbl) == 0) {
        strResp = RSP_200_OK_XML;
        string strHTTPDate;
        OutHTTPDateString(time(NULL), strHTTPDate);
        strResp += "Date: ";
  ...
  void List(const char* username,
    ...
    if (m_mailStg && m_mailStg->ListSubDir(username, nParentID, listtbl) == 0) {
      for (int x = 0; x < listtbl.size(); x++) {
        ...
        code_convert_ex("UTF-7", CMailBase::m_encoding.c_str(), strName.c_str(),
                        strName);
        char szTmp[128];
...
class ApiTraversalDirs : public doc, public storage {
 public:
  ApiTraversalDirs(CHttp* session)
      : doc(session), storage(session->GetStorageEngine()) {}

  virtual ~ApiTraversalDirs() {}

  void Traversal(const char* username,
                 int nParentID,
                 const char* globalpath,
                 string& strXML) {
    vector<Dir_Info> listtbl;
 ...
 public:
  ...
  virtual void Response() {
    ...
    if (check_userauth_token(strauth.c_str(), username) == 0) {
      ...
      if (m_mailStg && m_mailStg->LimitListMailByDir(username.c_str(), listtbl,
                                                     nDirID, nBeg, nRow) == 0) {
        if (nDirID < 0)
          nDirID = 0;
        ...
        for (int x = 0; x < listtbl.size(); x++) {
          ...
          Letter = new MailLetter(m_mailStg, CMailBase::m_private_path.c_str(),
                                  CMailBase::m_encoding.c_str(),
                                  m_session->GetMemCached(), emlfile.c_str());
          if (Letter && Letter->GetSize() > 0) {
            ...
            if (Letter->GetSummary()->m_header) {
              ...
              strSender = Letter->GetSummary()->m_header->GetFrom() == NULL
                              ? ""
                              : Letter->GetSummary()
                                    ->m_header->GetFrom()
                                    ->m_addrs[0]
              ...
              if (Letter->GetSummary()->m_header->GetFrom() != NULL) {
                strFromAddr =
                    Letter->GetSummary()->m_header->GetFrom()->m_addrs[0].mName;
                strFromAddr += "@";
                strFromAddr +=
                    Letter->GetSummary()->m_header->GetFrom()->m_addrs[0].dName;
              }
...
class ApiListUnauditedMails : public doc, public storage {
 public:
  ApiListUnauditedMails(CHttp* session)
      : doc(session), storage(session->GetStorageEngine()) {}
  ...
  virtual void Response() {
    ...
    if (check_adminauth_token(strauth.c_str(), username) == 0) {
      ...
      vector<Mail_Info> listtbl;
      if (m_mailStg && m_mailStg->LimitListUnauditedExternMailByDir(
                           username.c_str(), listtbl, nBeg, nRow) == 0) {
        char szTmp[128];
        ...
        for (int x = 0; x < listtbl.size(); x++) {
          ...
          if (Letter && Letter->GetSize() > 0) {
            ...
            if (Letter->GetSummary()->m_header) {
              string strSubject =
                  Letter->GetSummary()->m_header->GetDecodedSubject();
              to_safty_xmlstring(strSubject);
              string strSender =
                  Letter->GetSummary()->m_header->GetFrom() == NULL
                      ? ""
                      : Letter->GetSummary()
                            ->m_header->GetFrom()
                            ->m_addrs[0]
 ...
 public:
  ...
  virtual void Response() {
    ...
    if (check_userauth_token(strauth.c_str(), username) == 0) {
      BOOL isPass = TRUE;
      string mailowner;
      if (m_mailStg->GetMailOwner(nMailID, mailowner) == -1 ||
          strcasecmp(mailowner.c_str(), username.c_str()) != 0) {
        isPass = FALSE;
      }
      ...
      if (Letter->GetSize() > 0) {
        ...
        if (Letter->GetSummary()->m_header) {
          string strFrom =
              Letter->GetSummary()->m_header->GetFrom() == NULL
                  ? ""
                  : Letter->GetSummary()->m_header->GetFrom()->m_strfiled;
          ...
          string strTo =
              Letter->GetSummary()->m_header->GetTo() == NULL
                  ? ""
                  : Letter->GetSummary()->m_header->GetTo()->m_strfiled;
          ...
          string strCC =
              Letter->GetSummary()->m_header->GetCc() == NULL
                  ? ""
                  : Letter->GetSummary()->m_header->GetCc()->m_strfiled;
          ...
          string strBCC =
              Letter->GetSummary()->m_header->GetBcc() == NULL
                  ? ""
                  : Letter->GetSummary()->m_header->GetBcc()->m_strfiled;
        ...
        string strCalendar = "";
        string strAttach = "";
        MailInfo(Letter->GetSummary()->m_mime, strTextBody, strHTMLBody,
                 strCalendar, strAttach, lbuf);
    ...
    if (check_userauth_token(strauth.c_str(), username) == 0) {
      ...
      for (int i = 0; i < allAddr.size(); i++) {
        ...
        if (CMailBase::Is_Local_Domain(domain.c_str())) {
          strcut(mailaddr.c_str(), NULL, "@", id);
          if (m_mailStg && m_mailStg->VerifyUser(id.c_str()) == 0) {
            sprintf(newuid, "%08x_%08x_%016lx_%08x_%s", time(NULL), getpid(),
                    pthread_self(), random(),
                    CMailBase::m_localhostname.c_str());
            int DirID;
            ...
            if (m_mailStg &&
                m_mailStg->GetUserSize(id.c_str(), usermaxsize) == -1) {
            ...
            pLetter =
                new MailLetter(m_mailStg, CMailBase::m_private_path.c_str(),
                               CMailBase::m_encoding.c_str(),
                               m_session->GetMemCached(), newuid, usermaxsize);
          ...
          sprintf(newuid, "%08x_%08x_%016lx_%08x_%s", time(NULL), getpid(),
                  pthread_self(), random(), CMailBase::m_localhostname.c_str());
          int DirID;
          m_mailStg->GetInboxID(id.c_str(), DirID);
          unsigned int mstatus = MSG_ATTR_RECENT;
          ...
          unsigned long long usermaxsize;
          if (m_mailStg &&
              m_mailStg->GetUserSize(username.c_str(), usermaxsize) == -1) {
            usermaxsize = 5000 * 1024;
          }
      ...
      for (int v = 0; v < vLetter.size(); v++) {
        if (vLetter[v]) {
          ...
          email eml(CMailBase::m_email_domain.c_str(), username.c_str(),
                    strAlias.c_str());
          ...
          int wlen = 0;
          while (1) {
            if (vLetter[v]->Write(mbuf + wlen, (mlen - wlen) > MEMORY_BLOCK_SIZE
                                                   ? MEMORY_BLOCK_SIZE
                                                   : (mlen - wlen)) < 0) {
              isOK = FALSE;
              break;
            ...
            wlen += (mlen - wlen) > MEMORY_BLOCK_SIZE ? MEMORY_BLOCK_SIZE
                                                      : (mlen - wlen);
            if (wlen >= mlen)
              break;
          }
          if (isOK) {
            ...
            if (m_mailStg &&
                m_mailStg->GetUserLevel(username.c_str(), linfo) == -1) {
              m_mailStg->GetDefaultLevel(linfo);
            }
            if (vLetterInfo[v]->mail_type == mtExtern &&
                linfo.enableaudit == eaTrue) {
              vLetter[v]->Flush();
              ...
              if (mail_sumsize >= linfo.mailsizethreshold ||
                  attach_sumsize >= linfo.attachsizethreshold) {
          ...
          if (isOK) {
            m_mailStg->InsertMailIndex(
                vLetterInfo[v]->mail_from.c_str(),
                vLetterInfo[v]->mail_to.c_str(), vLetterInfo[v]->host.c_str(),
                vLetterInfo[v]->mail_time, vLetterInfo[v]->mail_type,
                vLetterInfo[v]->mail_uniqueid.c_str(),
                vLetterInfo[v]->mail_dirid, vLetterInfo[v]->mail_status,
                vLetter[v]->GetEmlName(), vLetter[v]->GetSize(),
                vLetterInfo[v]->mail_id);
      ...
      if (isOK) {
        ...
        strResp +=
            "<erisemail><response errno=\"1\" reason=\"Email size is too "
            "huge\"></response></erisemail>";
      }
    } else {
      strResp = RSP_200_OK_XML;
      string strHTTPDate;
 ...
 public:
  ...
  virtual void Response() {
    ...
    if (check_userauth_token(strauth.c_str(), username) == 0) {
      ...
      if (m_mailStg &&
          m_mailStg->GetUserSize(username.c_str(), maxsize) == -1) {
        maxsize = 5000 * 1024;
      }

      if ((m_session->parse_multipart_value("SELFPAGENAME", segSelfName) ==
           0) &&
          (m_session->parse_multipart_value("ATTACHFILEBODY", segAttach) ==
           0)) {
        string strSelfName;
        if (segSelfName.m_byte_end > segSelfName.m_byte_beg) {
          char* szSelfName = (char*)malloc(segSelfName.m_byte_end -
                                           segSelfName.m_byte_beg + 2);
          memset(szSelfName, 0,
                 segSelfName.m_byte_end - segSelfName.m_byte_beg + 2);
          memcpy(szSelfName,
                 m_session->GetFormData()->c_buffer() + segSelfName.m_byte_beg,
                 segSelfName.m_byte_end - segSelfName.m_byte_beg + 1);
          strSelfName = szSelfName;
          free(szSelfName);
        }

        string filename, filetype;
        if (m_session->parse_multipart_filename("ATTACHFILEBODY", filename) ==
                -1 ||
          ...
          strResp +=
              ...
              "src=\"language.js\"></script></head><body><script "
              "language=\"JavaScript\">alert(\"LANG_RESOURCE['uploaderror2']\")"
              ";window.location=\"";
          strResp += strSelfName;
        ...
        if (segAttach.m_byte_end >= segAttach.m_byte_beg &&
            (segAttach.m_byte_end - segAttach.m_byte_beg) > maxsize) {
          strResp = RSP_200_OK_NO_CACHE;
          string strHTTPDate;
          OutHTTPDateString(time(NULL), strHTTPDate);
          strResp += "Date: ";
          ...
          strResp +=
              "<html><head><script type=\"text/javascript\" "
              "src=\"language.js\"></script></head><body><script "
              "language=\"JavaScript\">alert(\"LANG_RESOURCE['uploaderror1']\")"
              ";window.location=\"";
          strResp += strSelfName;
          strResp += "\";</script></body></html>";
          m_session->HttpSend(strResp.c_str(), strResp.length());
          ...
          char szFileName[1024];
          sprintf(szFileName, "%s_%08x_%08x_%016lx_%08x.att", username.c_str(),
                  time(NULL), getpid(), pthread_self(), random());
          string strFilePath;
          strFilePath = CMailBase::m_private_path.c_str();
          strFilePath += "/tmp/";
          ...
          if (attachfd->is_open()) {
            ...
            char* buf = (char*)m_session->GetFormData()->c_buffer() +
                        segAttach.m_byte_beg;
            int len = segAttach.m_byte_end - segAttach.m_byte_beg + 1;
            for (int p = 0; p < ((len % 54) == 0 ? len / 54 : (len / 54 + 1));
                 p++) {
              memset(szb64tmpbuf1, 0, 55);
              memset(szb64tmpbuf2, 0, 73);

              memcpy(szb64tmpbuf1, buf + p * 54,
                     (len - 54 * p) > 54 ? 54 : len - 54 * p);
              CBase64::Encode(szb64tmpbuf1,
                              (len - 54 * p) > 54 ? 54 : len - 54 * p,
          ...
          char szSize[64];
          sprintf(szSize, "%u",
                  segAttach.m_byte_end - segAttach.m_byte_beg + 1);

          string strSrc1, strSrc2;

          if (code_convert_ex(CMailBase::m_encoding.c_str(), "UTF-8",
                              szFileName, strSrc1) != 0) {
            strSrc1 = szFileName;
          }
          if (code_convert_ex(CMailBase::m_encoding.c_str(), "UTF-8",
                              filename.c_str(), strSrc2) != 0) {
            strSrc2 = filename;
          }
...
class ApiDeleteUploadedFile : public doc, public storage {
 public:
  ApiDeleteUploadedFile(CHttp* session)
      : doc(session), storage(session->GetStorageEngine()) {}
  ...
  virtual void Response() {
    ...
    if (check_userauth_token(strauth.c_str(), username) == 0) {
      ...
      if (unlink(strFilePath.c_str()) == 0) {
        ...
        strResp +=
            "<erisemail><response errno=\"1\" reason=\"Delete file "
            "error\"></response></erisemail>";
      }
    } else {
      strResp = RSP_404_NOT_FOUND;
    }
      ...
      for (int i = 0; i < vDirID.size(); i++) {
        ...
        if ((strcasecmp(mail_owner.c_str(), username.c_str()) == 0) &&
          ...
          newLetter =
              new MailLetter(m_mailStg, CMailBase::m_private_path.c_str(),
                             CMailBase::m_encoding.c_str(),
                             m_session->GetMemCached(), newuid, usermaxsize /*m_mailStg,
"", "", mtLocal, newuid, nToDirID, mstatus, (unsigned int)time(NULL),
usermaxsize*/);
          ...
          if (isOK) {
            m_mailStg->InsertMailIndex(
                letter_info.mail_from.c_str(), letter_info.mail_to.c_str(),
                letter_info.host.c_str(), letter_info.mail_time,
                letter_info.mail_type, letter_info.mail_uniqueid.c_str(),
                letter_info.mail_dirid, letter_info.mail_status,
                newLetter->GetEmlName(), newLetter->GetSize(),
                letter_info.mail_id);
          }
          delete oldLetter;
          delete newLetter;
  ...
  virtual void Response() {
    ...
    if (check_userauth_token(strauth.c_str(), username) == 0) {
      ...
      if ((strcasecmp(mail_owner.c_str(), username.c_str()) == 0)) {
        ...
        strResp +=
            "<erisemail><response errno=\"1\" reason=\"Copy Mail "
            "Failed\"></response></erisemail>";
      }
    } else {
      strResp = RSP_200_OK_XML;
      string strHTTPDate;
  ...
  virtual void Response() {
    ...
    if (check_userauth_token(strauth.c_str(), username) == 0) {
      ...
      if (m_mailStg && m_mailStg->DelMail(username.c_str(), nMailID) == 0) {
        ...
        strResp +=
            "<erisemail><response errno=\"1\" reason=\"Delete Mail "
            "Failed.\"></response></erisemail>";
      }
    } else {
      strResp = RSP_200_OK_XML;
 ...
 public:
  ...
  virtual void Response() {
    ...
    if (check_adminauth_token(strauth.c_str(), username) == 0) {
      ...
      if (m_mailStg &&
          m_mailStg->GetMailStatus(nMailID, mstatus, mtExtern) == 0 &&
          m_mailStg->SetMailStatus(username.c_str(), nMailID,
                                   mstatus & (~MSG_ATTR_UNAUDITED),
                                   mtExtern) == 0) {
        strResp = RSP_200_OK_XML;
        string strHTTPDate;
        OutHTTPDateString(time(NULL), strHTTPDate);
 ...
 public:
  ...
  virtual void Response() {
    ...
    if (check_adminauth_token(strauth.c_str(), username) == 0) {
      ...
      if (m_mailStg &&
          m_mailStg->GetMailFromAndTo(nMailID, strFrom, strTo, mtExtern) == 0) {
        if (strFrom != "") {
          BOOL isLocal = FALSE;
          int nToDirID = -1;
          string strHost;
          ...
          char szBoundary[64];
          srandom(time(NULL));
          sprintf(szBoundary, "%03d_%04X_%08X.%08X", nBoundary,
                  random() % 0xFFFF, time(NULL));
 ...
 public:
  ...
  virtual void Response() {
    ...
    if (check_adminauth_token(strauth.c_str(), username) == 0) {
      ...
      if (m_mailStg &&
              m_mailStg->GetMailStatus(nMailID, mailstatus, mtExtern) == -1 ||
          mailstatus & MSG_ATTR_UNAUDITED != MSG_ATTR_UNAUDITED) {
        isPass = FALSE;
      }
...
class ApiReviewAttachment : public doc, public storage {
 public:
  ApiReviewAttachment(CHttp* session)
      : doc(session), storage(session->GetStorageEngine()) {}
  ...
  virtual void Response() {
    ...
    code_convert_ex("UTF-8", CMailBase::m_encoding.c_str(), strSrc.c_str(),
                    strFileName);
    unsigned int nMailID = atoi(strID.c_str());
    string username, password;
    ...
    if (check_adminauth_token(strauth.c_str(), username) == 0) {
      ...
      if (Letter->GetSize() > 0) {
        ...
        if (strFileName != "") {
          ...
          MailFileData(Letter->GetSummary()->m_mime, strFileName.c_str(),
                       strType, dispos, fbuf, lbuf);
          ...
          while (1) {
            if (nsend >= lbuf) {
              ...
              m_session->HttpSend(
                  pbuf + nsend, (lbuf - nsend) > 1448 ? 1448 : (lbuf - nsend));
          ...
          fbuffer fbuf;
          string dispos = "attachment";
          MailFileDataEx(Letter->GetSummary()->m_mime, strContentID.c_str(),
                         strType, dispos, fbuf, lbuf);
...
class ApiGetDirMailNum : public doc, public storage {
 public:
  ApiGetDirMailNum(CHttp* session)
      : doc(session), storage(session->GetStorageEngine()) {}
...
class ApiGetUnauditedMailNum : public doc, public storage {
 public:
  ApiGetUnauditedMailNum(CHttp* session)
      : doc(session), storage(session->GetStorageEngine()) {}
      ...
      if (strstr(strTmpFileName.c_str(), "..") == NULL) {
        ...
        if (tmpfd.is_open()) {
          ...
          while (getline(tmpfd, strLine)) {
            ...
            if (strLine == "\r\n" || strLine == "\n") {
              if (strncasecmp(strCell.c_str(), "Content-Type:",
                              sizeof("Content-Type:") - 1) == 0) {
                fnfy_strcut(strCell.c_str(), "Content-Type:", " \t\r\n\"",
                            "\r\n\";", strContentType);
                fnfy_strcut(strCell.c_str(), "name=", " \t\r\n\"", "\r\n\";",
                            strFileName1);
              } else if (strncasecmp(strCell.c_str(), "Content-Disposition:",
                                     sizeof("Content-Disposition:") - 1) == 0) {
                fnfy_strcut(strCell.c_str(),
                            "Content-Disposition:", " \t\"\r\n", "\";\r\n",
                            strDisposition);
                fnfy_strcut(strCell.c_str(), "filename=", " \t\"\r\n",
                            "\";\r\n", strFileName2);
              } else if (strncasecmp(
                             strCell.c_str(), "Content-Transfer-Encoding:",
                  ...
                  if (strncasecmp(strCell.c_str(), "Content-Type:",
                    ...
                    fnfy_strcut(strCell.c_str(), "name=", " \t\r\n\"",
                                "\r\n\";", strFileName1);
                                 ...
                                 strCell.c_str(), "Content-Disposition:",
                                 sizeof("Content-Disposition:") - 1) == 0) {
                    fnfy_strcut(strCell.c_str(),
                                "Content-Disposition:", " \t\"\r\n", "\";\r\n",
                                strDisposition);
          ...
          while (getline(tmpfd, strLine)) {
            ...
            if (strcasecmp(strTransferEncoding.c_str(), "base64") == 0) {
              ...
              if (strLine != "") {
                ...
                CBase64::Decode((char*)strLine.c_str(), strLine.length(), cbuf,
                                &clen);
                fbuf.bufcat(cbuf, clen);
                delete[] cbuf;
              }
            } else if ((strcasecmp(strTransferEncoding.c_str(), "QP") == 0) ||
                       (strcasecmp(strTransferEncoding.c_str(),
                                   "quoted-printable") == 0)) {
              strtrim(strLine);
              if (strLine != "") {
                int clen = strLine.length();
                char* cbuf = new char[clen + 1];
                DecodeQuoted((char*)strLine.c_str(), strLine.length() / 3 * 3,
                             (unsigned char*)cbuf);
          ...
          strResp += ";\r\n name=\"";
          strResp += strFileName1 == ""
                         ? (strFileName2 == "" ? szBackup : strFileName2)
                         : strFileName1;
          strResp += "\"\r\n";
...
class ApiExtractAttach : public doc, public storage {
 public:
  ApiExtractAttach(CHttp* session)
      : doc(session), storage(session->GetStorageEngine()) {}
  ...
  virtual void Response() {
    ...
    if (check_userauth_token(strauth.c_str(), username) == 0 ||
        check_adminauth_token(strauth.c_str(), username) == 0) {
      char szTmp[128];
      ...
      if (Letter->GetSize() > 0) {
        ...
        vector<AttInfo> attlist;
        ExtractAttach(username.c_str(), Letter->GetSummary()->m_mime, attlist,
                      lbuf);
        char tmpbuf[1024];
        for (int x = 0; x < attlist.size(); x++) {
          ...
          sprintf(tmpbuf,
                  "<attach tmpname=\"%s\" filename=\"%s\" attsize=\"%u\" />",
                  strTmpname.c_str(), strFilename.c_str(), attlist[x].size);
          strResp += tmpbuf;
        }
        strResp += "</response></erisemail>";
      }
...
class ApiSaveDraft : public doc, public storage {
 public:
  ...
  virtual void Response() {
    ...
    if (check_userauth_token(strauth.c_str(), username) == 0) {
      string strDraftID;
      string strFrom, strTo, strCc, strBcc, strSubject, strContent,
          strAttachFiles;
      ...
      code_convert_ex("UTF-8", CMailBase::m_encoding.c_str(), strTo.c_str(),
                      strTo);

      m_session->parse_urlencode_value("CC_ADDRS", strCc);
      code_convert_ex("UTF-8", CMailBase::m_encoding.c_str(), strCc.c_str(),
                      strCc);

      m_session->parse_urlencode_value("BCC_ADDRS", strBcc);
      code_convert_ex("UTF-8", CMailBase::m_encoding.c_str(), strBcc.c_str(),
                      strBcc);
      ...
      code_convert_ex("UTF-8", CMailBase::m_encoding.c_str(),
                      strAttachFiles.c_str(), strAttachFiles);
      ...
      BOOL isOK = TRUE;
      while (1) {
        if (pLetter->Write(mbuf + wlen, (mlen - wlen) > MEMORY_BLOCK_SIZE
                                            ? MEMORY_BLOCK_SIZE
                                            : (mlen - wlen)) < 0) {
          isOK = FALSE;
          break;
      ...
      if (pLetter->isOK()) {
        if (letter_info.mail_id == -1) {
          ...
          m_mailStg->UpdateMailIndex(
              letter_info.mail_from.c_str(), letter_info.mail_to.c_str(),
              letter_info.mail_time, letter_info.mail_type,
              letter_info.mail_uniqueid.c_str(), letter_info.mail_dirid,
              letter_info.mail_status, pLetter->GetEmlName(),
              pLetter->GetSize(), letter_info.mail_id);
        }
      }
      ...
      if (isOK) {
        ...
        strResp +=
            "<erisemail><response errno=\"1\" reason=\"Mail size is too "
            "huge\"><draftid>";
        strResp += szDraftID;
        strResp += "</draftid></response></erisemail>";
      }
...
class ApiLogin : public doc, public storage {
 public:
  ...
  virtual void Response() {
    ...
    string strXML;
    string username, password;
    if ((m_session->parse_urlencode_value("USER_NAME", username) == -1) ||
        (m_session->parse_urlencode_value("USER_PWD", password) == -1)) {
      ...
      if (m_mailStg &&
          m_mailStg->CheckLogin(username.c_str(), password.c_str()) == 0) {
        ...
        Security::Encrypt(name_and_role.c_str(), name_and_role.length(),
                          strEncoded, CMailBase::DESKey());
 ...
 public:
  ...
  virtual void Response() {
    ...
    if ((m_session->parse_urlencode_value("USER_NAME", username) == -1) ||
      ...
      if (m_mailStg &&
          m_mailStg->CheckAdmin(username.c_str(), password.c_str()) == 0) {
        string name_and_role = username;
        name_and_role += ":";
        name_and_role += "A:";
...
class ApiPasswd : public doc, public storage {
 public:
  ...
  virtual void Response() {
    ...
    if (m_mailStg &&
        m_mailStg->CheckLogin(username.c_str(), strOldPwd.c_str()) == 0) {
      MailStorage* master_storage;
#ifdef _WITH_DIST_
      master_storage =
          new MailStorage(CMailBase::m_encoding.c_str(),
                          CMailBase::m_private_path.c_str(), m_memcached);

      if (master_storage->Connect(
              CMailBase::m_master_db_host.c_str(),
              CMailBase::m_master_db_username.c_str(),
              CMailBase::m_master_db_password.c_str(),
              CMailBase::m_master_db_name.c_str(), CMailBase::m_master_db_port,
        ...
        strResp +=
            "<erisemail>"
            "<response errno=\"1\" reason=\"Couldn't connect master "
            "server\"></response>"
            "</erisemail>";
      }
...
#endif /* _WITH_DIST_ */
      if (master_storage &&
          master_storage->Passwd(username.c_str(), strNewPwd.c_str()) == 0) {
        strResp = RSP_200_OK_XML;
        string strHTTPDate;
        OutHTTPDateString(time(NULL), strHTTPDate);
        strResp += "Date: ";
...
#endif /* _WITH_DIST_ */
      if (master_storage &&
          master_storage->Alias(username.c_str(), strAlias.c_str()) == 0) {
        strResp = RSP_200_OK_XML;
        string strHTTPDate;
        OutHTTPDateString(time(NULL), strHTTPDate);
        strResp += "Date: ";
 ...
 public:
  ...
  virtual void Response() {
    ...
    if (check_userauth_token(strauth.c_str(), username) == 0) {
      ...
      if (m_mailStg && m_mailStg->GetID(username.c_str(), uinfo) == 0 &&
          m_mailStg->GetLevel(uinfo.level, linfo) == 0) {
        char szTmp[128];
 ...
 public:
  ...
  virtual void Response() {
    ...
    if (check_userauth_token(strauth.c_str(), username) == 0) {
      ...
      if (m_mailStg && m_mailStg->CreateDir(username.c_str(), strTmp.c_str(),
                                            parentid) == 0) {
        strResp +=
            "<erisemail><response errno=\"0\" "
            "reason=\"\"></response></erisemail>";
...
class ApiLoadConfigFile : public doc, public storage {
 public:
  ApiLoadConfigFile(CHttp* session)
      : doc(session), storage(session->GetStorageEngine()) {}
  ...
  virtual void Response() {
    ...
    if (check_adminauth_token(strauth.c_str(), username) == 0) {
      ...
      if (strConfigName == "GLOBAL_REJECT_LIST") {
        sem_t* plock =
            sem_open(ERISEMAIL_GLOBAL_REJECT_LIST, O_CREAT | O_RDWR, 0644, 1);
        if (plock != SEM_FAILED)
          sem_wait(plock);

        ifstream filein(CMailBase::m_reject_list_file.c_str(),
                        ios_base::binary);
        string strline;
        if (!filein.is_open()) {
          strResp +=
        ...
        sem_t* plock =
            sem_open(ERISEMAIL_GLOBAL_PERMIT_LIST, O_CREAT | O_RDWR, 0644, 1);
        if (plock != SEM_FAILED)
          sem_wait(plock);

        ifstream filein(CMailBase::m_permit_list_file.c_str(),
                        ios_base::binary);
        string strline;
        if (!filein.is_open()) {
          strResp +=
        ...
        sem_t* plock =
            sem_open(ERISEMAIL_DOMAIN_PERMIT_LIST, O_CREAT | O_RDWR, 0644, 1);
        if (plock != SEM_FAILED)
          sem_wait(plock);

        ifstream filein(CMailBase::m_domain_list_file.c_str(),
                        ios_base::binary);
        string strline;
        if (!filein.is_open()) {
          strResp +=
        ...
        sem_t* plock =
            sem_open(ERISEMAIL_WEBADMIN_PERMIT_LIST, O_CREAT | O_RDWR, 0644, 1);
        if (plock != SEM_FAILED)
          sem_wait(plock);

        ifstream filein(CMailBase::m_webadmin_list_file.c_str(),
                        ios_base::binary);
        string strline;
        if (!filein.is_open()) {
          strResp +=
...
class ApiSaveConfigFile : public doc, public storage {
 public:
  ApiSaveConfigFile(CHttp* session)
      : doc(session), storage(session->GetStorageEngine()) {}
  ...
  virtual void Response() {
    ...
    if (check_adminauth_token(strauth.c_str(), username) == 0) {
      ...
      if (strConfigName == "GLOBAL_REJECT_LIST") {
        ...
        code_convert_ex("UTF-8", CMailBase::m_encoding.c_str(), strText.c_str(),
                        strText);
        ...
        ofstream fileout(CMailBase::m_reject_list_file.c_str(),
        ...
        ofstream fileout(CMailBase::m_permit_list_file.c_str(),
                         ios_base::binary | ios::out | ios::trunc);
        if (!fileout.is_open()) {
          strResp +=
              "<erisemail><response errno=\"1\" "
        ...
        ofstream fileout(CMailBase::m_domain_list_file.c_str(),
                         ios_base::binary | ios::out | ios::trunc);
        if (!fileout.is_open()) {
          strResp +=
              "<erisemail><response errno=\"1\" "
        ...
        ofstream fileout(CMailBase::m_webadmin_list_file.c_str(),
                         ios_base::binary | ios::out | ios::trunc);
        if (!fileout.is_open()) {
          strResp +=
              "<erisemail><response errno=\"1\" "
  ...
  virtual void Response() {
    ...
    if (check_adminauth_token(strauth.c_str(), username) == 0) {
      ...
      if (dp) {
        ...
        strResp +=
            "<erisemail><response errno=\"1\" reason=\"list log error\">";
        strResp += "</response></erisemail>";
      }
    } else {
      strResp = RSP_200_OK_XML;
 ...
 public:
  ...
  virtual void Response() {
    ...
    if (check_adminauth_token(strauth.c_str(), username) == 0) {
      ...
      if (strmatch("*.log", strLogName.c_str()) &&
          strstr(strLogName.c_str(), "\\") == NULL &&
          strstr(strLogName.c_str(), "/") == NULL &&
          strstr(strLogName.c_str(), "../") == NULL &&
          strstr(strLogName.c_str(), "..\\") == NULL) {
        string strPath = "/var/log/erisemail/";
 ...
 public:
  ...
  virtual void Response() {
    ...
    if (check_adminauth_token(strauth.c_str(), username) == 0) {
      ...
      m_mailStg->GetGlobalStorage(commonMailNumber, deletedMailNumber,
                                  commonMailSize, deletedMailSize);
...
class ApiCurrentUsername : public doc, public storage {
 public:
  ApiCurrentUsername(CHttp* session)
      : doc(session), storage(session->GetStorageEngine()) {}
...
class Webmail {
 ...
 public:
  Webmail(CHttp* session) { m_session = session; }
  virtual ~Webmail() {}
  void Response();
};
