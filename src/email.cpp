...
int email::output(const char* clientip)
{
	...
	if(m_subject!= "")
	{
		...
		CBase64::Encode((char*)m_subject.c_str(), m_subject.length(), szb64buf, &nb64Len);
	...
	strTmp += "Content-Type: text/" + m_content_type + "; charset=" + CMailBase::m_encoding + "\r\n";
	...
	else
	{
		...
		for(int p = 0; p < ((plen % 54) == 0 ? plen/54 : (plen/54 + 1)) ; p++)
		{
			...
			memcpy(szb64tmpbuf1, pbuf + p * 54, (plen - 54 * p) > 54 ? 54 : plen - 54 * p);
			CBase64::Encode(szb64tmpbuf1, (plen - 54 * p) > 54 ? 54 : plen - 54 * p, szb64tmpbuf2, &nszb64tmplen2);
