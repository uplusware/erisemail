...
typedef enum { sRD = 0, sWT, sWR } SumMode;
...
void inline dynamic_mmap_copy(char* dest,
                              dynamic_mmap& src,
                              int from,
                              int size) {
...
class MimeSummary : public BlockSummary {
 public:
  ...
  const char* GetContentTransferEncoding() {
    return m_header->m_Content_Transfer_Encoding.c_str();
  }
  filedContentDisposition* GetContentDisposition() {
    return m_header->m_Content_Disposition;
  }
...
class LetterSummary : public BlockSummary {
 public:
  ...
  LetterSummary(const char* encoding,
                const char* szpath,
...
typedef enum { laOut = 0, laIn = 1 } lAtt;
...
typedef enum { MMAPED = 0, CACHED } MemoryType;
...
class MailLetter {
 protected:
  ...
  void get_attach_summary(MimeSummary* part,
                          unsigned long& count,
 ...
 public:
  MailLetter(MailStorage* mailStg,
  ...
  LetterSummary* GetSummary() { return m_letterSummary; }

  const char* GetEmlFullPath() { return m_eml_full_path.c_str(); }

  const char* GetEmlName() { return m_emlfile.c_str(); }

  unsigned int GetSize() { return m_size; }

  void SetOK() { m_LetterOk = TRUE; }

  BOOL isOK() { return m_LetterOk; }
};
