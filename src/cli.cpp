...
BOOL CLI::Parse(char* text) {
  ...
  if ((strline == "exit") || (strline == "quit")) {
    ...
    int vlen = m_queueList.size();
    for (int x = 0; x < vlen; x++) {
      int rc = mq_send(m_queueList[x].qid, (const char*)&qMsg,
                       sizeof(stQueueMsg), 0);
      if (rc < 0) {
        if (errno != EINTR) {
          perror("error");
    ...
    ret = FALSE;
  } else if (strncasecmp(strline.c_str(), "adduser", sizeof("adduser") - 1) ==
             0) {
  } else if (strncasecmp(strline.c_str(), "addgroup", sizeof("addgroup") - 1) ==
             0) {
  } else if (strncasecmp(strline.c_str(), "deluser", sizeof("deluser") - 1) ==
             0) {
  } else if (strncasecmp(strline.c_str(), "delgroup", sizeof("delgroup") - 1) ==
             0) {
  } else
