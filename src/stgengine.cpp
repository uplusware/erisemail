...
StorageEngine::StorageEngine(const char* host,
  ...
  for (int i = 0; i < m_maxConn; i++) {
    m_engine[i].storage = new MailStorage(m_encoding.c_str(),
                                          m_private_path.c_str(), m_memcached);
  ...
  for (int i = 0; i < m_maxConn; i++) {
    if (m_engine[i].inUse == FALSE) {
      if (m_engine[i].isConnected == FALSE) {
        if (m_engine[i].storage->Connect(m_host.c_str(), m_username.c_str(),
                                         m_password.c_str(), m_database.c_str(),
                                         m_port, m_sock_file.c_str()) != 0) {
