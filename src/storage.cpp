...
static void show_error(MYSQL* mysql,
  ...
  if (!CMailBase::m_close_stderr || printit) {
    ...
    CUplusTrace* t =
        new CUplusTrace(ERISEMAIL_MYSQLERR_LOGNAME, ERISEMAIL_MYSQLERR_LCKNAME);
    t->Write(Trace_Error, "%sMySQL error(%d) [%s] \"%s\" <%s>", tag,
             mysql_errno(mysql), mysql_sqlstate(mysql), mysql_error(mysql),
             sqlcmd);
    delete t;
  }
...
int inline _mysql_real_query_(MYSQL* mysql,
                              const char* stmt_str,
...
void MailStorage::SqlSafetyString(string& strInOut) {
  ...
  mysql_real_escape_string(&m_hMySQL, szOut, strInOut.c_str(),
                           strInOut.length());
  strInOut = szOut;
  delete szOut;
}

MailStorage::MailStorage(const char* encoding,
...
int MailStorage::LdapSync() {
  ...
  int rc =
      ldap_set_option(m_ldap, LDAP_OPT_PROTOCOL_VERSION, &protocol_version);
  if (rc == LDAP_SUCCESS) {
    int bind_msgid = -1;
    ...
    int rc =
        ldap_sasl_bind(m_ldap, CMailBase::m_ldap_manager.c_str(),
                       LDAP_SASL_SIMPLE, &bind_pwd, NULL, NULL, &bind_msgid);
    ...
    if (rc != -1) {
      ...
      rc = ldap_search_ext(m_ldap, CMailBase::m_ldap_search_base.c_str(),
                           LDAP_SCOPE_SUBTREE, szFilter, NULL, 0, NULL, NULL,
                           NULL, 0, &msgid);
      ...
      if (rc == LDAP_SUCCESS) {
        ...
        if (rc != -1) {
          if (search_result) {
            ...
            for (LDAPMessage* single_entry =
                     ldap_first_entry(m_ldap, search_result);
                 single_entry != NULL && c > 0;
                 single_entry = ldap_next_entry(m_ldap, search_result), c--) {
              char* ldap_cn;
              char* ldap_uid;

              uid_cn = ldap_get_values_len(
                  m_ldap, single_entry,
                  CMailBase::m_ldap_search_attribute_user_alias.c_str());
              if (uid_cn && *uid_cn) {
              ...
              uid = ldap_get_values_len(
                  m_ldap, single_entry,
                  CMailBase::m_ldap_search_attribute_user_id.c_str());
              if (uid && *uid) {
...
#ifdef _WITH_DIST_
              sprintf(sqlcmd,
                      "INSERT INTO usertbl(uname, upasswd, ualias, uhost, "
                      "utype, urole, usize, ustatus, ulevel, utime) select "
                      "'%s', ENCODE('%s','%s'), '%s', (select uhost from "
                      "(select uhost, min(C) from (select uhost, count(*) as C "
                      "from usertbl as A group by uhost order by C) as B) as "
                      "C), %d, %d, %d, %d, (SELECT lid FROM leveltbl WHERE "
                      "ldefault = %d), %d FROM dual WHERE NOT EXISTS (SELECT "
                      "uid FROM usertbl WHERE uname='%s')",
                      strSafetyUsername.c_str(), strSafetyPassword.c_str(),
                      CODE_KEY, strSafetyAlias.c_str(), utMember, urGeneralUser,
                      5000 * 1024, usActive, ldTrue, ldap_update_time,
                      strSafetyUsername.c_str());
#else
              sprintf(sqlcmd,
                      "INSERT INTO usertbl(uname, upasswd, ualias, utype, "
                      "urole, usize, ustatus, ulevel, utime) SELECT '%s', "
                      "ENCODE('%s','%s'), '%s', %d, %d, %d, %d, (SELECT lid "
                      "FROM leveltbl WHERE ldefault = %d), %d FROM dual WHERE "
                      "NOT EXISTS (SELECT uid FROM usertbl WHERE uname='%s')",
                      strSafetyUsername.c_str(), strSafetyPassword.c_str(),
                      CODE_KEY, strSafetyAlias.c_str(), utMember, urGeneralUser,
                      5000 * 1024, usActive, ldTrue, ldap_update_time,
                      strSafetyUsername.c_str());
              ...
              sprintf(sqlcmd,
                      "UPDATE usertbl SET utime=%d where uname='%s' AND "
                      "utype=%d AND urole=%d",
                      ldap_update_time, strSafetyUsername.c_str(), utMember,
                      urGeneralUser);
              // fprintf(stderr,"%s\n", sqlcmd);
              if (Query(sqlcmd, strlen(sqlcmd)) != 0) {
                show_error(&m_hMySQL, sqlcmd);
            ...
            sprintf(
                sqlcmd,
                "DELETE FROM usertbl where utime<%d AND utype=%d AND urole=%d",
                ldap_update_time, utMember, urGeneralUser);
            // fprintf(stderr,"%s\n", sqlcmd);
            if (Query(sqlcmd, strlen(sqlcmd)) != 0) {
              show_error(&m_hMySQL, sqlcmd);
              ret_val = -1;
            ...
            sprintf(sqlcmd,
                    "DELETE FROM grouptbl where membername NOT IN (SELECT "
                    "uname FROM usertbl where utype=%d)",
                    utMember);
            // fprintf(stderr,"%s\n", sqlcmd);
            if (Query(sqlcmd, strlen(sqlcmd)) != 0) {
              show_error(&m_hMySQL, sqlcmd);
            ...
            sprintf(sqlcmd,
                    "DELETE FROM dirtbl WHERE downer NOT IN (SELECT uname FROM "
                    "usertbl)");
            // fprintf(stderr,"%s\n", sqlcmd);
            if (Query(sqlcmd, strlen(sqlcmd)) != 0) {
              show_error(&m_hMySQL, sqlcmd);
            ...
            sprintf(sqlcmd,
                    "UPDATE mailtbl SET mstatus=(mstatus|%d) WHERE mdirid NOT "
                    "IN (SELECT did FROM dirtbl)",
                    MSG_ATTR_DELETED);
            // fprintf(stderr,"%s\n", sqlcmd);
            if (Query(sqlcmd, strlen(sqlcmd)) != 0) {
              show_error(&m_hMySQL, sqlcmd);
...
int MailStorage::Connect(const char* host,
                         const char* username,
                         const char* password,
                         const char* database,
                         unsigned short port,
  ...
  mysql_options(&m_hMySQL, MYSQL_OPT_CONNECT_TIMEOUT,
                (const void*)&timeout_val);
  ...
  if (mysql_real_connect(&m_hMySQL, host, username, password, database, port,
                         sock_file && sock_file[0] != '\0' ? sock_file : NULL,
                         0) != NULL) {
    m_is_opened = TRUE;
  } else {
...
void MailStorage::KeepLive() {
  if (Ping() != 0) {
    ...
    Connect(m_host.c_str(), m_username.c_str(), m_password.c_str(),
            m_database.c_str(), m_port,
            m_sock_file == "" ? NULL : m_sock_file.c_str());
  }
}
...
int MailStorage::Install(const char* database) {
  ...
  if (strcasecmp(m_encoding.c_str(), "GB2312") == 0) {
    sprintf(sqlcmd,
            "CREATE DATABASE IF NOT EXISTS %s DEFAULT CHARACTER SET gb2312 "
    ...
    sprintf(sqlcmd,
            "CREATE DATABASE IF NOT EXISTS %s DEFAULT CHARACTER SET ucs2 "
  ...
  sprintf(sqlcmd, "CREATE INDEX leveltbl1a ON %s.leveltbl (ldefault)",
          database);
  if (Query(sqlcmd, strlen(sqlcmd)) != 0) {
    show_error(&m_hMySQL, sqlcmd, "Install: ", TRUE);
    return -1;
  ...
  sprintf(sqlcmd, "CREATE INDEX dirtbl3a ON %s.dirtbl (downer, dname, dparent)",
          database);
  if (Query(sqlcmd, strlen(sqlcmd)) != 0) {
    show_error(&m_hMySQL, sqlcmd, "Install: ", TRUE);
    return -1;
  ...
  sprintf(sqlcmd, "CREATE INDEX usertbl2a ON %s.usertbl (ustatus, utype)",
          database);
  if (Query(sqlcmd, strlen(sqlcmd)) != 0) {
    show_error(&m_hMySQL, sqlcmd, "Install: ", TRUE);
    return -1;
  ...
  sprintf(sqlcmd, "CREATE INDEX usertbl2b ON %s.usertbl (uname, utype)",
          database);
  if (Query(sqlcmd, strlen(sqlcmd)) != 0) {
    show_error(&m_hMySQL, sqlcmd, "Install: ", TRUE);
    return -1;
  ...
  sprintf(sqlcmd, "CREATE INDEX grouptbl1a ON %s.grouptbl (groupname)",
          database);
  if (Query(sqlcmd, strlen(sqlcmd)) != 0) {
    show_error(&m_hMySQL, sqlcmd, "Install: ", TRUE);
    return -1;
  ...
  if (CMailBase::m_is_master) {
    unsigned int lid;
    if (AddLevel("default", "The system's default level", 5000 * 1024,
                 500000 * 1024, eaFalse, 5000 * 1024, 5000 * 1024, lid) == 0) {
      SetDefaultLevel(lid);
    } else {
    ...
    if (AddID("admin", "admin", "Cluster Administrator",
              CMailBase::m_master_hostname.c_str(), utMember, urAdministrator,
              MAX_EMAIL_LEN, -1) == -1) {
  ...
  if (AddID("admin", "admin", "Administrator", "", utMember, urAdministrator,
            MAX_EMAIL_LEN, -1) == -1) {
  ...
  sprintf(sqlcmd,
          "CREATE TRIGGER postmail_notify_insert AFTER INSERT ON extmailtbl "
          "FOR EACH ROW BEGIN SET @rs = post_notify(); END");
  if (Query(sqlcmd, strlen(sqlcmd)) != 0) {
    show_error(&m_hMySQL, sqlcmd, "Install: ", TRUE);
    return -1;
  }
  ...
  sprintf(sqlcmd,
          "CREATE TRIGGER postmail_notify_update AFTER UPDATE ON extmailtbl "
          "FOR EACH ROW BEGIN SET @rs = post_notify(); END");
  if (Query(sqlcmd, strlen(sqlcmd)) != 0) {
    show_error(&m_hMySQL, sqlcmd, "Install: ", TRUE);
    return -1;
...
int MailStorage::CheckAdmin(const char* username, const char* password) {
  ...
  sprintf(sqlcmd,
          "SELECT uname FROM usertbl WHERE uname='%s' AND DECODE(upasswd,'%s') "
          "= '%s' AND ustatus = %d AND urole=%d AND utype = %d and uhost IN "
          "('%s', '')",
          strSafetyUsername.c_str(), CODE_KEY, password, usActive,
          urAdministrator, utMember, strSafetyHost.c_str());
#else
  sprintf(sqlcmd,
          "SELECT uname FROM usertbl WHERE uname='%s' AND DECODE(upasswd,'%s') "
          "= '%s' AND ustatus = %d AND urole=%d AND utype = %d",
          strSafetyUsername.c_str(), CODE_KEY, password, usActive,
          urAdministrator, utMember);
#endif /* _WITH_DIST_ */
          ...
          if (search_result) {
            if (ldap_count_entries(m_ldap, search_result) == 1) {
              for (LDAPMessage* single_entry =
                       ldap_first_entry(m_ldap, search_result);
                   single_entry != NULL;
                   single_entry = ldap_next_entry(m_ldap, search_result)) {
...
int MailStorage::CheckLogin(const char* username, const char* password) {
  ...
  if (rc == LDAP_SUCCESS) {
    ...
    int rc = ldap_sasl_bind(m_ldap, user_dn.c_str(), LDAP_SASL_SIMPLE,
                            &bind_pwd, NULL, NULL, &bind_msgid);
          ...
          if (search_result) {
            if (ldap_count_entries(m_ldap, search_result) == 1) {
              for (LDAPMessage* single_entry =
                   ...
                   single_entry != NULL;
                   single_entry = ldap_next_entry(m_ldap, search_result)) {
                uid_password = ldap_get_values_len(
                    m_ldap, single_entry,
                    CMailBase::m_ldap_search_attribute_user_password.c_str());
                if (uid_password && *uid_password) {
                  char* ldap_password =
                      (char*)malloc((*uid_password)->bv_len + 1);
                  memcpy(ldap_password, (*uid_password)->bv_val,
                         (*uid_password)->bv_len);
  ...
  if (ldap_rt_val == 0) {
    ...
    SqlSafetyString(strSafetyHost);
    sprintf(sqlcmd,
            "SELECT uname FROM usertbl WHERE uname = '%s' AND ustatus = %d AND "
            "utype = %d and uhost IN ('%s', '')",
            username, usActive, utMember, strSafetyHost.c_str());
#else
    sprintf(sqlcmd,
            "SELECT uname FROM usertbl WHERE uname = '%s' AND ustatus = %d AND "
            "utype = %d",
            username, usActive, utMember);
#endif /* _WITH_DIST_ */
  ...
  SqlSafetyString(strSafetyHost);
  sprintf(sqlcmd,
          "SELECT DECODE(upasswd,'%s') FROM usertbl WHERE uname = '%s' AND "
          "ustatus = %d AND utype = %d and uhost IN ('%s', '')",
          CODE_KEY, username, usActive, utMember, strSafetyHost.c_str());
#else
  sprintf(sqlcmd,
          "SELECT DECODE(upasswd,'%s') FROM usertbl WHERE uname = '%s' AND "
          "ustatus = %d AND utype = %d",
          CODE_KEY, username, usActive, utMember);
#endif /* _WITH_DIST_ */
...
int MailStorage::GetPassword(const char* username, string& password) {
  ...
  if (rc == LDAP_SUCCESS) {
    ...
    if (rc != -1) {
      ...
      int rc = ldap_search_ext(m_ldap, CMailBase::m_ldap_search_base.c_str(),
                               LDAP_SCOPE_SUBTREE, szFilter, NULL, 0, NULL,
                               NULL, NULL, 1, &msgid);
  ...
  sprintf(sqlcmd,
          "SELECT DECODE(upasswd,'%s') FROM usertbl WHERE uname='%s' AND "
          "utype=%d AND uhost='%s'",
          CODE_KEY, strSafetyUsername.c_str(), utMember, strSafetyHost.c_str());
#else
  sprintf(
      sqlcmd,
      "SELECT DECODE(upasswd,'%s') FROM usertbl WHERE uname='%s' AND utype=%d",
      CODE_KEY, strSafetyUsername.c_str(), utMember);
#endif /* _WITH_DIST_ */
  if (Query(sqlcmd, strlen(sqlcmd)) == 0) {
    MYSQL_RES* query_result;
    query_result = mysql_store_result(&m_hMySQL);
...
int MailStorage::GetHost(const char* username, string& host) {
  ...
  sprintf(sqlcmd, "SELECT uhost FROM usertbl WHERE uname='%s' AND utype=%d",
          strSafetyUsername.c_str(), utMember);
...
int MailStorage::VerifyUser(const char* username) {
  ...
  sprintf(sqlcmd, "SELECT uname FROM usertbl WHERE uname='%s' AND utype=%d",
          strSafetyUsername.c_str(), utMember);
...
int MailStorage::AddLevel(const char* lname,
  ...
  sprintf(
      sqlcmd,
      "INSERT INTO leveltbl(lname, ldescription, lmailmaxsize, lboxmaxsize, "
      "lenableaudit, lmailsizethreshold, lattachsizethreshold, ldefault, "
      "ltime) VALUES('%s', '%s', %llu, %llu, %d, %d, %d, %d, %d)",
      strSafetyLevelname.c_str(), strSafetyDescription.c_str(), mailmaxsize,
      boxmaxsize, lenableaudit, mailsizethreshold, attachsizethreshold, ldFalse,
      time(NULL));
...
int MailStorage::UpdateLevel(unsigned int lid,
                             const char* lname,
                             const char* ldescription,
                             unsigned long long mailmaxsize,
                             unsigned long long boxmaxsize,
  ...
  sprintf(sqlcmd,
          "UPDATE leveltbl SET lname='%s', ldescription='%s', "
          "lmailmaxsize=%llu, lboxmaxsize=%llu, lenableaudit=%d, "
          "lmailsizethreshold=%d, lattachsizethreshold=%d WHERE lid=%d",
          strSafetyLevelname.c_str(), strSafetyDescription.c_str(), mailmaxsize,
          boxmaxsize, lenableaudit, mailsizethreshold, attachsizethreshold,
          lid);
...
int MailStorage::DelLevel(unsigned int lid) {
  ...
  sprintf(sqlcmd, "UPDATE usertbl SET ulevel=%d WHERE ulevel=%d", defaultLid,
          lid);
  if (Query(sqlcmd, strlen(sqlcmd)) != 0) {
    show_error(&m_hMySQL, sqlcmd);
    return -1;
  ...
  sprintf(sqlcmd, "DELETE FROM leveltbl WHERE lid=%d AND ldefault <> %d", lid,
          ldTrue);
  if (Query(sqlcmd, strlen(sqlcmd)) != 0) {
    show_error(&m_hMySQL, sqlcmd);
    return -1;
...
int MailStorage::SetDefaultLevel(unsigned int lid) {
  ...
  sprintf(sqlcmd, "UPDATE leveltbl SET ldefault = %d WHERE ldefault = %d",
          ldFalse, ldTrue);
  if (Query(sqlcmd, strlen(sqlcmd)) != 0) {
    show_error(&m_hMySQL, sqlcmd);
    return -1;
  ...
  sprintf(sqlcmd, "UPDATE leveltbl SET ldefault = %d WHERE lid = %d", ldTrue,
          lid);
  if (Query(sqlcmd, strlen(sqlcmd)) != 0) {
    show_error(&m_hMySQL, sqlcmd);
    return -1;
...
int MailStorage::GetUserLevel(const char* username, Level_Info& linfo) {
  ...
  if (username && username[0] != '\0') {
    sprintf(sqlcmd,
            "SELECT A.lid, A.lname, A.ldescription, A.lmailmaxsize, "
            "A.lboxmaxsize, A.lenableaudit, A.lmailsizethreshold, "
            "A.lattachsizethreshold, A.ldefault, A.ltime FROM leveltbl as A, "
            "usertbl as B WHERE A.lid=B.ulevel and B.uname = '%s'",
            username);
  } else {
    return -1;
  }
...
int MailStorage::SetUserLevel(const char* username, int lid) {
  ...
  sprintf(sqlcmd, "UPDATE usertbl SET ulevel=%d WHERE uname='%s'", lid,
          strSafetyUsername.c_str());
...
int MailStorage::GetDefaultLevel(Level_Info& liinfo) {
  ...
  sprintf(sqlcmd,
          "SELECT lid, lname, ldescription, lmailmaxsize, lboxmaxsize, "
          "lenableaudit, lmailsizethreshold, lattachsizethreshold, ldefault, "
          "ltime  FROM leveltbl WHERE ldefault = %d",
          ldTrue);
...
int MailStorage::ListLevel(vector<Level_Info>& litbl) {
  ...
  sprintf(sqlcmd,
          "SELECT lid, lname, ldescription, lmailmaxsize, lboxmaxsize, "
          "lenableaudit, lmailsizethreshold, lattachsizethreshold, ldefault, "
          "ltime FROM leveltbl ORDER BY ltime");
...
int MailStorage::GetLevel(int lid, Level_Info& linfo) {
  ...
  if (lid != -1) {
    sprintf(sqlcmd,
            "SELECT lid, lname, ldescription, lmailmaxsize, lboxmaxsize, "
            "lenableaudit, lmailsizethreshold, lattachsizethreshold, ldefault, "
            "ltime FROM leveltbl WHERE lid=%d",
            lid);
  } else {
    return -1;
  }
...
int MailStorage::CheckRequiredDir(const char* username) {
  ...
  sprintf(sqlcmd,
          "INSERT INTO dirtbl(dname, downer, dparent , dstatus, dusage, dtime) "
          "SELECT 'Inbox','%s',-1, %d, %d, %d FROM dual WHERE NOT EXISTS "
          "(SELECT did FROM dirtbl WHERE downer='%s' AND dusage=%d)",
          strSafetyUsername.c_str(), DIR_ATTR_MARKED | DIR_ATTR_SUBSCRIBED,
          duInbox, time(NULL), strSafetyUsername.c_str(), duInbox);
  if (Query(sqlcmd, strlen(sqlcmd)) != 0) {
    show_error(&m_hMySQL, sqlcmd);
    return -1;
  }
  ...
  sprintf(sqlcmd,
          "INSERT INTO dirtbl(dname, downer, dparent , dstatus, dusage, dtime) "
          "SELECT 'Sent','%s',-1, %d, %d, %d FROM dual WHERE NOT EXISTS "
          "(SELECT did FROM dirtbl WHERE downer='%s' AND dusage=%d)",
          strSafetyUsername.c_str(), DIR_ATTR_MARKED | DIR_ATTR_SUBSCRIBED,
          duSent, time(NULL), strSafetyUsername.c_str(), duSent);
  if (Query(sqlcmd, strlen(sqlcmd)) != 0) {
    show_error(&m_hMySQL, sqlcmd);
    return -1;
  }
  ...
  sprintf(sqlcmd,
          "INSERT INTO dirtbl(dname, downer, dparent , dstatus, dusage, dtime) "
          "SELECT 'Drafts','%s',-1, %d, %d, %d FROM dual WHERE NOT EXISTS "
          "(SELECT did FROM dirtbl WHERE downer='%s' AND dusage=%d)",
          strSafetyUsername.c_str(), DIR_ATTR_MARKED | DIR_ATTR_SUBSCRIBED,
          duDrafts, time(NULL), strSafetyUsername.c_str(), duDrafts);
  if (Query(sqlcmd, strlen(sqlcmd)) != 0) {
    show_error(&m_hMySQL, sqlcmd);
    return -1;
  }
  ...
  sprintf(sqlcmd,
          "INSERT INTO dirtbl(dname, downer, dparent , dstatus, dusage, dtime) "
          "SELECT 'Trash','%s',-1, %d, %d, %d FROM dual WHERE NOT EXISTS "
          "(SELECT did FROM dirtbl WHERE downer='%s' AND dusage=%d)",
          strSafetyUsername.c_str(), DIR_ATTR_MARKED | DIR_ATTR_SUBSCRIBED,
          duTrash, time(NULL), strSafetyUsername.c_str(), duTrash);
  if (Query(sqlcmd, strlen(sqlcmd)) != 0) {
    show_error(&m_hMySQL, sqlcmd);
    return -1;
  }
  ...
  sprintf(sqlcmd,
          "INSERT INTO dirtbl(dname, downer, dparent , dstatus, dusage, dtime) "
          "SELECT 'Junk','%s',-1, %d, %d, %d FROM dual WHERE NOT EXISTS "
          "(SELECT did FROM dirtbl WHERE downer='%s' AND dusage=%d)",
          strSafetyUsername.c_str(), DIR_ATTR_MARKED | DIR_ATTR_SUBSCRIBED,
          duJunk, time(NULL), strSafetyUsername.c_str(), duJunk);
  if (Query(sqlcmd, strlen(sqlcmd)) != 0) {
    show_error(&m_hMySQL, sqlcmd);
    return -1;
  }
...
int MailStorage::AddSlavePlaceholderID() {
  ...
  sprintf(sqlcmd,
          "INSERT INTO usertbl(uname, upasswd, ualias, uhost, utype, urole, "
          "usize, ustatus, ulevel, utime) VALUES('%s#%s', ENCODE('%s','%s'), "
          "'%s', '%s', %d, %d, %d, %d, %d, %d)",
          "placeholder", strSafetyHost.c_str(), strSafetyPassword.c_str(),
          CODE_KEY, strSafetyAlias.c_str(), strSafetyHost.c_str(), utMember,
          urDisabled, 0, usDisabled, -1, time(NULL));
...
int MailStorage::AddID(const char* username,
                       const char* password,
                       const char* alias,
                       const char* host,
                       UserType type,
  ...
  if (strcasecmp(username, "postmaster") != 0 &&
      strcasecmp(username, "post_master") != 0) {
    ...
    if ((VerifyUser(username) == -1) && (VerifyGroup(username) == -1)) {
      ...
      if (host && host[0] != '\0') {
        ...
        sprintf(sqlcmd,
                "INSERT INTO usertbl(uname, upasswd, ualias, uhost, utype, "
                "urole, usize, ustatus, ulevel, utime) VALUES('%s', "
                "ENCODE('%s','%s'), '%s', '%s', %d, %d, %d, %d, %d, %d)",
                strSafetyUsername.c_str(), strSafetyPassword.c_str(), CODE_KEY,
                strSafetyAlias.c_str(), strSafetyHost.c_str(), type, role, size,
                status, defaultLevel, time(NULL));
      } else {
        sprintf(
            sqlcmd,
            "INSERT INTO usertbl(uname, upasswd, ualias, uhost, utype, urole, "
            "usize, ustatus, ulevel, utime) VALUES('%s', ENCODE('%s','%s'), "
            "'%s', (select uhost from (select uhost, min(C) from (select "
            "uhost, count(*) as C from usertbl as A group by uhost order by C) "
            "as B) as C), %d, %d, %d, %d, %d, %d)",
            strSafetyUsername.c_str(), strSafetyPassword.c_str(), CODE_KEY,
            strSafetyAlias.c_str(), type, role, size, status, defaultLevel,
            time(NULL));
      }

#else
      sprintf(sqlcmd,
              "INSERT INTO usertbl(uname, upasswd, ualias, utype, urole, "
              "usize, ustatus, ulevel, utime) VALUES('%s', ENCODE('%s','%s'), "
              "'%s', %d, %d, %d, %d, %d, %d)",
              strSafetyUsername.c_str(), strSafetyPassword.c_str(), CODE_KEY,
              strSafetyAlias.c_str(), type, role, size, status, defaultLevel,
...
int MailStorage::UpdateID(const char* username,
                          const char* alias,
                          const char* host,
                          UserStatus status,
                          int level) {
  ...
  SqlSafetyString(strSafetyHost);
  sprintf(sqlcmd,
          "UPDATE usertbl SET ualias='%s', ustatus=%d, ulevel=%d, uhost='%s' "
          "WHERE uname='%s'",
          strSafetyAlias.c_str(), status, realLevel, strSafetyHost.c_str(),
          strSafetyUsername.c_str());

#else
  sprintf(
      sqlcmd,
      "UPDATE usertbl SET ualias='%s', ustatus=%d, ulevel=%d WHERE uname='%s'",
...
int MailStorage::DelAllMailOfDir(int mdirid) {
  char sqlcmd[1024];
  sprintf(sqlcmd, "UPDATE mailtbl SET mstatus=(mstatus|%d) WHERE mdirid=%d",
          MSG_ATTR_DELETED, mdirid);
...
int MailStorage::DelAllMailOfID(const char* username) {
  char sqlcmd[1024];
  sprintf(sqlcmd,
          "UPDATE mailtbl SET mstatus=(mstatus|%d) WHERE mdirid IN (SELECT did "
          "FROM dirtbl WHERE downer='%s')",
          MSG_ATTR_DELETED, username);
...
int MailStorage::DelID(const char* username) {
  ...
  if (idType > 0) {
    ...
    if (idType == 2) {
      sprintf(sqlcmd, "DELETE FROM grouptbl WHERE groupname='%s'",
              strSafetyUsername.c_str());
      if (Query(sqlcmd, strlen(sqlcmd)) != 0) {
        show_error(&m_hMySQL, sqlcmd);
        return -1;
      ...
      sprintf(sqlcmd, "DELETE FROM grouptbl WHERE membername='%s'",
              strSafetyUsername.c_str());
      if (Query(sqlcmd, strlen(sqlcmd)) != 0) {
        show_error(&m_hMySQL, sqlcmd);
        return -1;
      ...
      sprintf(sqlcmd, "DELETE FROM dirtbl WHERE downer='%s'",
              strSafetyUsername.c_str());
      if (Query(sqlcmd, strlen(sqlcmd)) != 0) {
        show_error(&m_hMySQL, sqlcmd);
        return -1;
    ...
    sprintf(sqlcmd, "DELETE FROM usertbl WHERE uname='%s'",
            strSafetyUsername.c_str());
    if (Query(sqlcmd, strlen(sqlcmd)) != 0) {
      show_error(&m_hMySQL, sqlcmd);
      return -1;
...
int MailStorage::LoadMailFromFile(const char* mfrom,
                                  const char* mto,
                                  unsigned int mtime,
                                  unsigned int mtx,
                                  const char* muniqid,
  ...
  sprintf(sqlfilepath, "/tmp/erisemail/%s_%08x_%08x_%016lx_%08x.sql", muniqid,
          time(NULL), getpid(), pthread_self(), random());

  ofstream* sqlfile =
      new ofstream(sqlfilepath, ios_base::binary | ios::out | ios::trunc);
      ...
      if (emlfile) {
        if (emlfile->is_open()) {
          ...
          sprintf(rbuf, "\"%s\",\"%s\",%u,%u,\"%s\",%d,%u,\"",
                  strSafetyFrom.c_str(), strSafetyTo.c_str(), mtime, mtx,
                  muniqid, mdirid, mstatus);
  ...
  sprintf(sqlcmd,
          ...
          "ENCLOSED BY '\"' (mfrom,mto,mtime,mtx,muniqid,mdirid,mstatus,mbody)",
          sqlfilepath, mtx == mtExtern ? "extmailtbl" : "mailtbl");
...
int MailStorage::UpdateMailFromFile(const char* mfrom,
                                    const char* mto,
                                    unsigned int mtime,
                                    unsigned int mtx,
                                    const char* muniqid,
    ...
    if (sqlfile->is_open()) {
      ...
      if (emlfile) {
        if (emlfile->is_open()) {
          ...
          // update
          sprintf(rbuf, "%d,\"%s\",\"%s\",%u,%u,\"%s\",%d,%u,\"", mailid,
                  strSafetyFrom.c_str(), strSafetyTo.c_str(), mtime, mtx,
                  muniqid, mdirid, mstatus);
  ...
  char sqlcmd[1024];
  sprintf(sqlcmd,
          "LOAD DATA LOCAL INFILE '%s' REPLACE INTO TABLE %s FIELDS TERMINATED "
          "BY ',' ENCLOSED BY '\"' "
          "(mid,mfrom,mto,mtime,mtx,muniqid,mdirid,mstatus,mbody)",
          sqlfilepath, mtx == mtExtern ? "extmailtbl" : "mailtbl");
...
int MailStorage::InsertMail(const char* mfrom,
                            const char* mto,
                            unsigned int mtime,
                            unsigned int mtx,
                            const char* muniqid,
  ...
  char* sqlcmd = (char*)malloc(strSafetyBody.length() + strSafetyTo.length() +
                               strSafetyFrom.length() + 1024);
  if (sqlcmd) {
    sprintf(sqlcmd,
            "INSERT INTO "
            "%s(mfrom,mto,mtime,mtx,muniqid,mdirid,mstatus,mbody,msize) "
            "VALUES('%s','%s',%u,%u,'%s',%d,%u,'%s',%u)",
            mtx == mtExtern ? "extmailtbl" : "mailtbl", strSafetyFrom.c_str(),
            strSafetyTo.c_str(), mtime, mtx, muniqid, mdirid, mstatus,
            strSafetyBody.c_str(), msize);
    if (Query(sqlcmd, strlen(sqlcmd)) == 0) {
      mailid = mysql_insert_id(&m_hMySQL);
      free(sqlcmd);
      return 0;
...
int MailStorage::InsertMailIndex(const char* mfrom,
                                 const char* mto,
                                 const char* mhost,
                                 unsigned int mtime,
                                 unsigned int mtx,
  ...
  if (mdirid != -1) {
    ...
    if (GetDirOwner(mdirid, username) == 0 &&
        GetUserStorage(username.c_str(), commonMailNumber, deletedMailNumber,
                       commonMailSize, deletedMailSize) == 0 &&
  ...
  if (sqlcmd) {
    if (strcmp(mhost, "") == 0 ||
        strcasecmp(mhost, CMailBase::m_localhostname.c_str()) == 0) {
      sprintf(sqlcmd,
              "INSERT INTO %s.%s "
              "(mfrom,mto,mtime,mtx,muniqid,mdirid,mstatus,mbody,msize) "
              "VALUES('%s','%s',%u,%u,'%s',%d,%u,'%s','%u')",
              m_database.c_str(), mtx == mtExtern ? "extmailtbl" : "mailtbl",
              strSafetyFrom.c_str(), strSafetyTo.c_str(), mtime, mtx, muniqid,
              mdirid, mstatus, strSafetyBody.c_str(), msize);
    } else {
      ...
      sprintf(sqlcmd,
              "INSERT INTO %s.%s "
              "(mfrom,mto,mhost,mtime,mtx,muniqid,mdirid,mstatus,mbody,msize) "
              "VALUES('%s','%s','%s',%u,%u,'%s',%d,%u,'%s','%u')",
              m_database.c_str(), mtx == mtExtern ? "extmailtbl" : "mailtbl",
              strSafetyFrom.c_str(), strSafetyTo.c_str(), strSafetyHost.c_str(),
              mtime, mtx, muniqid, mdirid, mstatus, strSafetyBody.c_str(),
              msize);
    }
...
int MailStorage::UpdateMail(const char* mfrom,
                            const char* mto,
                            unsigned int mtime,
                            unsigned int mtx,
                            const char* muniqid,
  ...
  if (sqlcmd) {
    sprintf(sqlcmd,
            "UPDATE %s SET "
            "mfrom='%s',mto='%s',mtime=%u,mtx=%u,muniqid='%s',mdirid=%d,"
            "mstatus=%u,mbody='%s',msize='%u' WHERE mid=%d",
            mtx == mtExtern ? "extmailtbl" : "mailtbl", strSafetyFrom.c_str(),
            strSafetyTo.c_str(), mtime, mtx, muniqid, mdirid, mstatus,
            strSafetyBody.c_str(), msize, mailid);
...
int MailStorage::UpdateMailIndex(const char* mfrom,
                                 const char* mto,
                                 unsigned int mtime,
                                 unsigned int mtx,
                                 const char* muniqid,
...
int MailStorage::ChangeMailDir(int mdirid, int mailid) {
  char sqlcmd[1024];
  if (sqlcmd) {
    sprintf(sqlcmd, "UPDATE mailtbl SET mdirid=%d WHERE mid=%d", mdirid,
            mailid);
...
int MailStorage::ListMailByDir(const char* username,
  ...
  sprintf(sqlcmd,
          "SELECT mbody, muniqid,mid,mtime,mstatus,mfrom,mto,mtx,mdirid FROM "
          "mailtbl WHERE mdirid=%d AND mstatus&%d<>%d ORDER BY mid",
          did, MSG_ATTR_DELETED, MSG_ATTR_DELETED);
...
int MailStorage::ListMailByDir(const char* username,
  ...
  sprintf(sqlcmd,
          "SELECT mbody, muniqid,mid,mtime,mstatus,mfrom,mto,mtx,mdirid FROM "
          "mailtbl WHERE mdirid=%d AND mstatus&%d<>%d ORDER BY mtime desc",
          dirid, MSG_ATTR_DELETED, MSG_ATTR_DELETED);
...
int MailStorage::LimitListMailByDir(const char* username,
                                    vector<Mail_Info>& listtbl,
                                    unsigned int dirid,
                                    int beg,
                                    int rows) {
  ...
  sprintf(sqlcmd,
          ...
          "mailtbl WHERE mdirid=%d AND mstatus&%d<>%d ORDER BY mtime desc "
          "limit %d, %d",
          dirid, MSG_ATTR_DELETED, MSG_ATTR_DELETED, beg, rows);
...
int MailStorage::LimitListUnauditedExternMailByDir(const char* username,
                                                   vector<Mail_Info>& listtbl,
                                                   int beg,
                                                   int rows) {
  listtbl.clear();
  ...
  sprintf(sqlcmd,
          "SELECT mbody,muniqid,mid,mtime,mstatus,mfrom,mto,mtx,mdirid FROM "
          "extmailtbl WHERE mstatus&%d<>%d AND mstatus&%d=%d ORDER BY mtime "
          "limit %d, %d",
          MSG_ATTR_DELETED, MSG_ATTR_DELETED, MSG_ATTR_UNAUDITED,
          MSG_ATTR_UNAUDITED, beg, rows);
  if (Query(sqlcmd, strlen(sqlcmd)) == 0) {
    MYSQL_RES* query_result;
    MYSQL_ROW row;
...
int MailStorage::ListAllMail(vector<Mail_Info>& listtbl) {
  ...
  sprintf(sqlcmd,
          "SELECT mbody,muniqid,mid,mtime,mstatus,mfrom,mto,mtx,mdirid FROM "
          "mailtbl WHERE mstatus&%d<>%d ORDER BY mtime desc",
          MSG_ATTR_DELETED, MSG_ATTR_DELETED);
...
int MailStorage::ListAvailableExternMail(vector<Mail_Info>& listtbl,
                                         unsigned int mta_index,
                                         unsigned int mta_count,
                                         unsigned int max_num) {
  listtbl.clear();
  char sqlcmd[1024];
#ifdef _WITH_DIST_
  sprintf(sqlcmd,
          "SELECT mfrom, mto, mhost, muniqid, mid FROM extmailtbl WHERE "
          "mtx='%d' AND mstatus&%d<>%d AND mstatus&%d<>%d AND mid%%%d=%d and "
          "TIMESTAMPDIFF(SECOND, next_fwd_time, CURRENT_TIMESTAMP) > 0 and "
          "tried_count<%d ORDER BY mid",
          mtExtern, MSG_ATTR_DELETED, MSG_ATTR_DELETED, MSG_ATTR_UNAUDITED,
          MSG_ATTR_UNAUDITED, mta_count, mta_index, MAX_TRY_FORWARD_COUNT);
#else
  sprintf(sqlcmd,
          "SELECT mfrom, mto, muniqid, mid FROM extmailtbl WHERE mtx='%d' AND "
          "mstatus&%d<>%d AND mstatus&%d<>%d AND mid%%%d=%d and "
          "TIMESTAMPDIFF(SECOND, next_fwd_time, CURRENT_TIMESTAMP) > 0 and "
          "tried_count<%d ORDER BY mid",
          mtExtern, MSG_ATTR_DELETED, MSG_ATTR_DELETED, MSG_ATTR_UNAUDITED,
          MSG_ATTR_UNAUDITED, mta_count, mta_index, MAX_TRY_FORWARD_COUNT);
...
int MailStorage::ForwardingExternMail(int mid) {
  char sqlcmd[1024];
  sprintf(sqlcmd,
          "UPDATE extmailtbl SET next_fwd_time=TIMESTAMPADD(SECOND, "
          "(tried_count+1)*300, CURRENT_TIMESTAMP), tried_count=tried_count+1 "
          "WHERE mid=%d",
          mid);
  if (Query(sqlcmd, strlen(sqlcmd)) == 0) {
    return 0;
...
int MailStorage::ListMemberOfGroup(const char* group,
                                   vector<User_Info>& listtbl) {
  listtbl.clear();
  char sqlcmd[1024];
  sprintf(sqlcmd, "SELECT membername FROM grouptbl WHERE groupname='%s'",
          group);
...
#ifdef _WITH_DIST_
  sprintf(sqlcmd,
          "SELECT uname, ualias, utype, urole, usize, ustatus, ulevel, uhost "
          "FROM usertbl ORDER BY %s %s",
          orderby == "" ? "utime" : orderby.c_str(), desc ? "desc" : "");
#else
  sprintf(sqlcmd,
          "SELECT uname, ualias, utype, urole, usize, ustatus, ulevel FROM "
          "usertbl ORDER BY %s %s",
          orderby == "" ? "utime" : orderby.c_str(), desc ? "desc" : "");
#endif /* _WITH_DIST_ */
  if (Query(sqlcmd, strlen(sqlcmd)) == 0) {
    MYSQL_RES* query_result;
...
int MailStorage::FilterID(vector<User_Info>& listtbl,
                          const char* filter,
                          string orderby,
                          BOOL desc) {
  string sub_string = "%";
  ...
  sprintf(
      sqlcmd,
      "SELECT uname, ualias, utype, urole, usize, ustatus, ulevel, uhost FROM "
      "usertbl WHERE uname LIKE '%s' OR ualias LIKE '%s' ORDER BY %s %s",
      sub_string.c_str(), sub_string.c_str(),
      orderby == "" ? "utime" : orderby.c_str(), desc ? "desc" : "");
#else
  sprintf(sqlcmd,
          "SELECT uname, ualias, utype, urole, usize, ustatus, ulevel FROM "
...
int MailStorage::ListGroup(vector<User_Info>& listtbl) {
  ...
  char sqlcmd[1024];
  sprintf(sqlcmd,
          "SELECT uname, ualias, utype, urole, usize, ustatus FROM usertbl "
          "WHERE utype=%d ORDER BY uname",
          utGroup);
...
int MailStorage::Passwd(const char* uname, const char* password) {
  if (VerifyUser(uname) == 0) {
    ...
    sprintf(sqlcmd,
            "UPDATE usertbl SET upasswd=ENCODE('%s','%s') WHERE uname='%s'",
            strSafetyPassword.c_str(), CODE_KEY, strSafetyUsername.c_str());
    if (Query(sqlcmd, strlen(sqlcmd)) == 0) {
#ifdef _WITH_LDAP_
      int ldap_rt_val = -1;
      ldap_initialize(&m_ldap, m_ldap_server_uri.c_str());
      ...
      if (rc == LDAP_SUCCESS) {
        ...
        bind_pwd.bv_len = CMailBase::m_ldap_manager_passwd.length();
        bind_pwd.bv_val = (char*)CMailBase::m_ldap_manager_passwd.c_str();
        int rc = ldap_sasl_bind(m_ldap, CMailBase::m_ldap_manager.c_str(),
                                LDAP_SASL_SIMPLE, &bind_pwd, NULL, NULL,
                                &bind_msgid);
        ...
        if (rc != -1) {
          ...
          char* passwd_attr = (char*)malloc(
              CMailBase::m_ldap_search_attribute_user_password.length() + 1);
          strcpy(passwd_attr,
                 CMailBase::m_ldap_search_attribute_user_password.c_str());
...
int MailStorage::Alias(const char* uname, const char* alias) {
  if (VerifyUser(uname) == 0) {
    ...
    sprintf(sqlcmd, "UPDATE usertbl SET ualias='%s' WHERE uname='%s'",
            strSafetyAlias.c_str(), strSafetyUsername.c_str());
    if (Query(sqlcmd, strlen(sqlcmd)) == 0) {
      return 0;
    } else {
      show_error(&m_hMySQL, sqlcmd);
...
int MailStorage::Host(const char* uname, const char* host) {
  ...
  if (VerifyUser(uname) == 0) {
    ...
    sprintf(sqlcmd, "UPDATE usertbl SET uhost='%s' WHERE uname='%s'",
            strSafetyHost.c_str(), strSafetyUsername.c_str());
    if (Query(sqlcmd, strlen(sqlcmd)) == 0) {
      return 0;
    } else {
      show_error(&m_hMySQL, sqlcmd);
...
int MailStorage::SetUserStatus(const char* uname, UserStatus status) {
  if (VerifyUser(uname) == 0) {
    ...
    sprintf(sqlcmd, "UPDATE usertbl SET ustatus=%d WHERE uname='%s'", status,
            strSafetyUsername.c_str());
    if (Query(sqlcmd, strlen(sqlcmd)) == 0) {
      return 0;
    } else {
...
int MailStorage::SetUserSize(const char* uname, unsigned int size) {
  if (VerifyUser(uname) == 0) {
    ...
    sprintf(sqlcmd, "UPDATE usertbl SET usize=%d WHERE uname='%s'", size,
            strSafetyUsername.c_str());
    if (Query(sqlcmd, strlen(sqlcmd)) == 0) {
      return 0;
    } else {
...
int MailStorage::DumpMailToFile(int mid, string& dumpfile) {
  char sqlcmd[1024];
  sprintf(sqlcmd, "/tmp/erisemail/%08x_%08x_%08x_%016lx_%08x.dat", time(NULL),
          getpid(), pthread_self(), random(), mid);
  dumpfile = sqlcmd;
  sprintf(sqlcmd,
          "SELECT mbody into DUMPFILE '%s' FROM mailtbl WHERE mid='%d' AND "
          "mstatus&%d<>%d",
          dumpfile.c_str(), mid, MSG_ATTR_DELETED, MSG_ATTR_DELETED);
  if (Query(sqlcmd, strlen(sqlcmd)) == 0) {
    return 0;
  } else {
    show_error(&m_hMySQL, sqlcmd);
...
int MailStorage::GetMailBody(int mid, char* body) {
  char sqlcmd[1024];
  sprintf(sqlcmd, "SELECT mbody FROM mailtbl WHERE mid='%d' AND mstatus&%d<>%d",
...
int MailStorage::GetMailIndex(int mid, string& path, unsigned int mtx) {
  ...
  sprintf(sqlcmd, "SELECT mbody FROM %s WHERE mid='%d' AND mstatus&%d<>%d;",
          mtx == mtExtern ? "extmailtbl" : "mailtbl", mid, MSG_ATTR_DELETED,
          MSG_ATTR_DELETED);
...
int MailStorage::GetMailFromAndTo(int mid,
                                  string& from,
                                  string& to,
                                  unsigned int mtx) {
  char sqlcmd[1024];
  ...
  MYSQL_RES* query_result;
  MYSQL_ROW row;
  sprintf(sqlcmd, "SELECT mfrom, mto FROM %s WHERE mid='%d'",
          mtx == mtExtern ? "extmailtbl" : "mailtbl", mid);
...
int MailStorage::IsAdmin(const char* username) {
  ...
  MYSQL_RES* query_result;
  MYSQL_ROW row;
  sprintf(sqlcmd, "SELECT uname FROM usertbl WHERE uname='%s' AND urole=%d",
          username, urAdministrator);
...
int MailStorage::DelMail(const char* username, int mid, unsigned int mtx) {
  ...
  string owner = "";
  if (IsAdmin(username) == 0 || (GetMailOwner(mid, owner) == 0 &&
                                 strcasecmp(owner.c_str(), username) == 0)) {
    sprintf(sqlcmd, "UPDATE %s SET mstatus=(mstatus|%d) WHERE mid='%d'",
            mtx == mtExtern ? "extmailtbl" : "mailtbl", MSG_ATTR_DELETED, mid);
    if (Query(sqlcmd, strlen(sqlcmd)) == 0) {
      return 0;
    } else {
      show_error(&m_hMySQL, sqlcmd);
...
int MailStorage::AppendUserToGroup(const char* username,
  ...
  if ((VerifyUser(username) == 0) && (VerifyGroup(groupname) == 0)) {
    ...
    sprintf(sqlcmd,
            "SELECT gid FROM grouptbl WHERE groupname='%s' AND membername='%s'",
            strSafetyGroupname.c_str(), strSafetyUsername.c_str());
    ...
    sprintf(sqlcmd,
            "INSERT INTO grouptbl(groupname, membername, gtime) VALUES('%s', "
            "'%s', %d)",
...
int MailStorage::RemoveUserFromGroup(const char* username,
                                     const char* groupname) {
  char sqlcmd[1024];
  string strSafetyUsername = username;
  SqlSafetyString(strSafetyUsername);
  ...
  sprintf(sqlcmd,
          "DELETE FROM grouptbl WHERE groupname='%s'and membername='%s'",
          strSafetyGroupname.c_str(), strSafetyUsername.c_str());
  if (Query(sqlcmd, strlen(sqlcmd)) == 0) {
    return 0;
  } else {
...
int MailStorage::CreateDir(const char* username, const char* dirref) {
  ...
  if (GetDirParentID(username, dirref, parentID) == 0) {
    ...
    SqlSafetyString(strSafetyUsername);
    sprintf(sqlcmd,
            "INSERT INTO dirtbl(dname,downer,dparent,dstatus,dtime) "
            "VALUES('%s','%s',%d,%d,%d)",
            strNewDir.c_str(), strSafetyUsername.c_str(), parentID,
            DIR_ATTR_SUBSCRIBED | DIR_ATTR_MARKED | DIR_ATTR_NOSELECT,
            time(NULL));
    if (Query(sqlcmd, strlen(sqlcmd)) == 0) {
      return 0;
    } else {
...
int MailStorage::CreateDir(const char* username,
                           const char* dirname,
                           int parentid) {
  char sqlcmd[1024];
...
int MailStorage::GetDirParentID(const char* username,
                                const char* dirref,
                                int& parentid) {
  char sqlcmd[1024];
...
int MailStorage::IsDirExist(const char* username, const char* dirref) {
  ...
  for (int x = 0; x < vDirRefLen; x++) {
    ...
    sprintf(sqlcmd,
            "SELECT did FROM dirtbl WHERE downer='%s' AND dname='%s' AND "
            "dparent=%d",
...
int MailStorage::IsSubDirExist(const char* username,
                               int parentid,
                               const char* dirname) {
  char sqlcmd[1024];
  string strSafetyUsername = username;
  ...
  sprintf(
      sqlcmd,
      "SELECT did FROM dirtbl WHERE downer='%s' AND dname='%s' AND dparent=%d",
      strSafetyUsername.c_str(), strSafetyDirname.c_str(), parentid);
...
int MailStorage::GetDirID(const char* username,
                          const char* dirref,
                          vector<int>& vdirid) {
  char sqlcmd[1024];
  string strDirRef = dirref;
    ...
    for (int i = 0; i < vparentid.size(); i++) {
      ...
      if (strSafetyDirname.find("%") == string::npos)
        sprintf(sqlcmd,
                "SELECT did FROM dirtbl WHERE downer='%s' AND dname='%s' AND "
                "dparent=%d",
                strSafetyUsername.c_str(), strSafetyDirname.c_str(),
                vparentid[i]);
      else
        sprintf(sqlcmd,
                "SELECT did FROM dirtbl WHERE downer='%s' AND dname like '%s' "
                "AND dparent=%d",
                strSafetyUsername.c_str(), strSafetyDirname.c_str(),
                vparentid[i]);
  ...
  if (Query(sqlcmd, strlen(sqlcmd)) == 0) {
    ...
    if (query_result) {
      ...
      if (row == NULL) {
        mysql_free_result(query_result);
        sprintf(sqlcmd,
                "INSERT INTO dirtbl(dname, downer, dparent , dstatus, dusage, "
                "dtime) VALUES('Inbox','%s',-1, %d, %d, %d)",
                strSafetyUsername.c_str(),
                DIR_ATTR_MARKED | DIR_ATTR_SUBSCRIBED, duInbox, time(NULL));
        if (Query(sqlcmd, strlen(sqlcmd)) != 0) {
          sprintf(sqlcmd,
                  "UPDATE dirtbl SET dusage=%d WHERE downer='%s' AND "
                  "dname='%s' AND dparent='-1'",
  ...
  if (Query(sqlcmd, strlen(sqlcmd)) == 0) {
    ...
    if (query_result) {
      ...
      if (row == NULL) {
        ...
        sprintf(sqlcmd,
                "INSERT INTO dirtbl(dname, downer, dparent , dstatus, dusage, "
                "dtime) VALUES('Junk','%s',-1, %d, %d, %d)",
                strSafetyUsername.c_str(),
                DIR_ATTR_MARKED | DIR_ATTR_SUBSCRIBED, duJunk, time(NULL));
        if (Query(sqlcmd, strlen(sqlcmd)) != 0) {
          sprintf(sqlcmd,
                  "UPDATE dirtbl SET dusage=%d WHERE downer='%s' AND "
                  "dname='%s' AND dparent='-1'",
  ...
  if (Query(sqlcmd, strlen(sqlcmd)) == 0) {
    ...
    if (query_result) {
      ...
      if (row == NULL) {
        ...
        sprintf(sqlcmd,
                "INSERT INTO dirtbl(dname, downer, dparent , dstatus, dusage, "
                "dtime) VALUES('Drafts','%s',-1, %d, %d, %d)",
                strSafetyUsername.c_str(),
                DIR_ATTR_MARKED | DIR_ATTR_SUBSCRIBED, duDrafts, time(NULL));
        if (Query(sqlcmd, strlen(sqlcmd)) != 0) {
          sprintf(sqlcmd,
                  "UPDATE dirtbl SET dusage=%d WHERE downer='%s' AND "
                  "dname='%s' AND dparent='-1'",
  ...
  if (Query(sqlcmd, strlen(sqlcmd)) == 0) {
    ...
    if (query_result) {
      ...
      if (row == NULL) {
        ...
        sprintf(sqlcmd,
                "INSERT INTO dirtbl(dname, downer, dparent , dstatus, dusage, "
                "dtime) VALUES('Sent','%s',-1, %d, %d, %d)",
                strSafetyUsername.c_str(),
                DIR_ATTR_MARKED | DIR_ATTR_SUBSCRIBED, duSent, time(NULL));
        if (Query(sqlcmd, strlen(sqlcmd)) != 0) {
          sprintf(sqlcmd,
                  "UPDATE dirtbl SET dusage=%d WHERE downer='%s' AND "
                  "dname='%s' AND dparent='-1'",
  ...
  if (Query(sqlcmd, strlen(sqlcmd)) == 0) {
    ...
    if (query_result) {
      ...
      if (row == NULL) {
        ...
        sprintf(sqlcmd,
                "INSERT INTO dirtbl(dname, downer, dparent , dstatus, dusage, "
                "dtime) VALUES('Trash','%s',-1, %d, %d, %d)",
                strSafetyUsername.c_str(),
                DIR_ATTR_MARKED | DIR_ATTR_SUBSCRIBED, duTrash, time(NULL));
        if (Query(sqlcmd, strlen(sqlcmd)) != 0) {
          sprintf(sqlcmd,
                  "UPDATE dirtbl SET dusage=%d WHERE downer='%s' AND "
                  "dname='%s' AND dparent='-1'",
...
int MailStorage::GetDirMailCount(const char* username,
                                 int dirid,
                                 unsigned int& count) {
  char sqlcmd[1024];
  sprintf(sqlcmd,
          "SELECT mdirid FROM mailtbl WHERE mdirid =%d AND mstatus&%d<>%d",
          dirid, MSG_ATTR_DELETED, MSG_ATTR_DELETED);
...
int MailStorage::GetUnauditedExternMailCount(const char* username,
                                             unsigned int& count) {
  char sqlcmd[1024];
  sprintf(
      sqlcmd,
      "SELECT mdirid FROM extmailtbl WHERE mstatus&%d<>%d AND mstatus&%d=%d",
      MSG_ATTR_DELETED, MSG_ATTR_DELETED, MSG_ATTR_UNAUDITED,
      MSG_ATTR_UNAUDITED);
...
int MailStorage::GetDirStatus(const char* username,
                              const char* dirref,
                              unsigned int& status) {
  int dirID;
  if (GetDirID(username, dirref, dirID) == -1)
...
int MailStorage::SetDirStatus(const char* username,
                              const char* dirref,
                              unsigned int status) {
  int dirID;
  if (GetDirID(username, dirref, dirID) == -1)
...
int MailStorage::GetMailStatus(int mid,
                               unsigned int& status,
                               unsigned int mtx) {
  char sqlcmd[1024];
  sprintf(sqlcmd, "SELECT mstatus FROM %s WHERE mid=%d",
          mtx == mtExtern ? "extmailtbl" : "mailtbl", mid);
...
int MailStorage::SetMailStatus(const char* username,
                               int mid,
                               unsigned int status,
                               unsigned int mtx) {
  char sqlcmd[1024];
  ...
  if (IsAdmin(username) == 0 || (GetMailOwner(mid, owner) == 0 &&
                                 strcasecmp(owner.c_str(), username) == 0)) {
    sprintf(sqlcmd, "UPDATE %s SET mstatus=%d WHERE mid=%d",
            mtx == mtExtern ? "extmailtbl" : "mailtbl", status, mid);
    if (Query(sqlcmd, strlen(sqlcmd)) == 0) {
      return 0;
    } else
      return -1;
...
int MailStorage::RenameDir(const char* username,
                           const char* oldname,
                           const char* newname) {
  char sqlcmd[1024];
  int dirID;
  ...
  sprintf(sqlcmd, "UPDATE dirtbl SET dname='%s',dparent=%d WHERE did=%d",
          strNewDir.c_str(), newParentID, dirID);
...
int MailStorage::TraversalListDir(const char* username,
                                  const char* dirref,
                                  vector<Dir_Tree_Ex>& listtbl) {
  char sqlcmd[1024];
  string strDirRef = dirref;
  ...
  for (int x = 0; x < dirParentID.size(); x++) {
    ...
    SqlSafetyString(strSafetyUsername);
    sprintf(sqlcmd,
            "SELECT dname, dstatus, did FROM dirtbl WHERE downer='%s' AND "
            "dparent=%d AND dname like '%s' ORDER BY dtime",
...
int MailStorage::ListSubDir(const char* username,
                            int pid,
  ...
  sprintf(sqlcmd,
          "SELECT dname, dstatus, did FROM dirtbl WHERE downer='%s' AND "
          "dparent=%d ORDER BY did",
          strSafetyUsername.c_str(), pid);
...
int MailStorage::SplitDir(const char* dirref,
                          string& parentdir,
                          string& dirname) {
  string strDirRef = dirref;
  vector<string> vDirRef;
...
int MailStorage::GetUnseenMail(int dirid, int& num) {
  ...
  sprintf(sqlcmd,
          "SELECT COUNT(*) FROM mailtbl WHERE mdirid='%d' AND mstatus&%d<>%d "
          "AND mstatus&%d<>%d",
          dirid, MSG_ATTR_DELETED, MSG_ATTR_DELETED, MSG_ATTR_SEEN,
          MSG_ATTR_SEEN);
...
int MailStorage::EmptyDir(const char* username, int dirid) {
  ...
  sprintf(sqlcmd,
          "UPDATE mailtbl SET mstatus=(mstatus|%d) WHERE mdirid='%d' AND "
          "mstatus&%d<>%d",
          MSG_ATTR_DELETED, dirid, MSG_ATTR_DELETED, MSG_ATTR_DELETED);
  if (Query(sqlcmd, strlen(sqlcmd)) == 0) {
    sprintf(sqlcmd, "SELECT did FROM dirtbl WHERE dparent=%d", dirid);
...
int MailStorage::GetDirPath(const char* username,
                            int dirid,
                            string& globalpath) {
  string owner;
  GetDirOwner(dirid, owner);
...
int MailStorage::GetGlobalStorage(unsigned int& commonMailNumber,
                                  unsigned int& deletedMailNumber,
                                  unsigned int& commonMailSize,
                                  unsigned int& deletedMailSize) {
  char sqlcmd[1024];
  ...
  sprintf(sqlcmd, "SELECT count(*) FROM mailtbl WHERE mstatus&%d<>%d",
          MSG_ATTR_DELETED, MSG_ATTR_DELETED);
  ...
  sprintf(sqlcmd, "SELECT SUM(msize) FROM mailtbl WHERE mstatus&%d<>%d",
          MSG_ATTR_DELETED, MSG_ATTR_DELETED);
...
int MailStorage::GetUserStorage(const char* username,
                                unsigned int& commonMailNumber,
                                unsigned int& deletedMailNumber,
                                unsigned int& commonMailSize,
                                unsigned int& deletedMailSize) {
  ...
  for (int x = 0; x < didtbl.size(); x++) {
    sprintf(sqlcmd,
            "SELECT count(*) FROM mailtbl WHERE mstatus&%d<>%d AND mdirid=%d",
            MSG_ATTR_DELETED, MSG_ATTR_DELETED, didtbl[x]);
    ...
    sprintf(sqlcmd,
            "SELECT SUM(msize) FROM mailtbl WHERE mstatus&%d<>%d AND mdirid=%d",
            MSG_ATTR_DELETED, MSG_ATTR_DELETED, didtbl[x]);
...
int MailStorage::SaveMailBodyToDB(const char* emlfile, const char* fragment) {
  ...
  char* sqlcmd = (char*)malloc(strSafetyEmlfile.length() +
                               strSafetyFragment.length() + 1024);
  if (sqlcmd) {
...
int MailStorage::DeleteMailBodyFromDB(const char* emlfile) {
  ...
  if (sqlcmd) {
    sprintf(sqlcmd, "DELETE from mbodytbl WHERE mbody='%s'",
            strSafetyEmlfile.c_str());
...
int MailStorage::LoadMailBodyToFile(const char* emlfile, const char* fullpath) {
  ...
  sprintf(sqlcmd,
          "SELECT mfragment FROM mbodytbl WHERE mbody='%s' ORDER BY mtime",
          strSafetyEmlfile.c_str());
  ...
  if (Query(sqlcmd, strlen(sqlcmd)) == 0) {
    ...
    if (query_result) {
      ofstream* ofile =
          new ofstream(fullpath, ios_base::binary | ios::out | ios::trunc);
      if (ofile) {
        if (!ofile->is_open()) {
          delete ofile;
          return -1;
...
int MailStorage::InsertMTA(const char* mta) {
  ...
  sprintf(sqlcmd,
          "INSERT INTO mtatbl(mta, active_time) VALUES('%s',CURRENT_TIMESTAMP)",
          strSafetyMTA.c_str());
...
int MailStorage::UpdateMTA(const char* mta) {
  ...
  sprintf(sqlcmd,
          "UPDATE mtatbl SET active_time=CURRENT_TIMESTAMP WHERE mta='%s'",
          strSafetyMTA.c_str());
...
int MailStorage::GetMTAIndex(const char* mta,
  ...
  sprintf(sqlcmd,
          "SELECT mta FROM mtatbl WHERE TIMESTAMPDIFF(SECOND, active_time, "
          "CURRENT_TIMESTAMP) < %d ORDER BY mta",
          live_sec);
...
int MailStorage::ListMyMessage(const char* to,
                               string& message_text,
                               vector<int>& xids) {
  char sqlcmd[1024];
  sprintf(sqlcmd,
          "SELECT xid, xmessage FROM xmpptbl WHERE xto = '%s' OR xto like "
          "'%s/%%' order by xtime",
          to, to);
  if (Query(sqlcmd, strlen(sqlcmd)) == 0) {
    MYSQL_RES* query_result;
...
int MailStorage::InsertMyMessage(const char* xfrom,
                                 const char* xto,
                                 const char* xmessage) {
  ...
  char* sqlcmd =
      (char*)malloc(strSafetyMessage.length() + strSafetyTo.length() +
                    strSafetyFrom.length() + 1024);
  if (sqlcmd) {
    sprintf(sqlcmd,
            "INSERT INTO xmpptbl(xfrom, xto, xmessage) VALUES('%s','%s', '%s')",
            strSafetyFrom.c_str(), strSafetyTo.c_str(),
            strSafetyMessage.c_str());
    if (Query(sqlcmd, strlen(sqlcmd)) == 0) {
      free(sqlcmd);
      return 0;
    } else {
...
int MailStorage::InsertBuddy(const char* usr1, const char* usr2) {
  ...
  char* sqlcmd =
      (char*)malloc(strSafetyUser1.length() + strSafetyUser2.length() + 1024);
  if (sqlcmd) {
...
int MailStorage::RemoveBuddy(const char* usr1, const char* usr2) {
  ...
  if (sqlcmd) {
    sprintf(sqlcmd,
            "DELETE FROM xmppbuddytbl WHERE (xusr1 = '%s' AND xusr2 = '%s') OR "
            "(xusr1 = '%s' AND xusr2 = '%s') ",
            strSafetyUser1.c_str(), strSafetyUser2.c_str(),
            strSafetyUser2.c_str(), strSafetyUser1.c_str());
...
int MailStorage::ListBuddys(const char* selfid, vector<string>& buddys) {
  char sqlcmd[1024];
  sprintf(sqlcmd,
          "SELECT xusr1, xusr2 FROM xmppbuddytbl WHERE xusr1 = '%s' OR xusr2 = "
          "'%s'",
          selfid, selfid);
  if (Query(sqlcmd, strlen(sqlcmd)) == 0) {
    MYSQL_RES* query_result;
