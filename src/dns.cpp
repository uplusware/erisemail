...
unsigned int udpdns::gethostname(const unsigned char* dnsbuf, unsigned int seek, string& hostname)
{
    unsigned int ret = 0;
    unsigned int i = 0;
    while(1)
...
int udpdns::query(const char* hostname, unsigned short record_type, vector<DNSQry_List> & result_list)
{
