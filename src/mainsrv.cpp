...
static void usage() {
  printf(
      "Usage:erisemaild start | stop | status | reload | version [CONFIG "
      "FILE]\n");
}
...
int Run() {
  ...
  do {
    ...
    if (CMailBase::m_enablexmpp) {
      ...
      service_param1.host_port = CMailBase::m_encryptxmpp == XMPP_OLDSSL_BASED
                                     ? CMailBase::m_xmppsport
                                     : CMailBase::m_xmppport;
      service_param1.is_ssl =
          CMailBase::m_encryptxmpp == XMPP_OLDSSL_BASED ? TRUE : FALSE;
    ...
    if (CMailBase::m_enable_local_ldap) {
      ...
      service_param1.host_ip = CMailBase::m_hostip.c_str();
      service_param1.host_port = CMailBase::m_encrypt_local_ldap == TRUE
                                     ? CMailBase::m_local_ldapsport
                                     : CMailBase::m_local_ldapport;
      service_param1.is_ssl =
          CMailBase::m_encrypt_local_ldap == TRUE ? TRUE : FALSE;
      service_param1.sockfd = -1;
  ...
  if (getgid() != 0) {
    ...
    if (argc == 2) {
      processcmd(argv[1], CONFIG_FILE_PATH, PERMIT_FILE_PATH, REJECT_FILE_PATH,
                 DOMAIN_FILE_PATH, WEBADMIN_FILE_PATH);
    } else {
      usage();
      return -1;
    }
