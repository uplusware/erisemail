...
void mfilter_data(void* filter, const char* data, unsigned int len) {
  ...
  if (!tfilter->fd) {
    ...
    snprintf(command, sizeof(command), "test spamc && spamc -c %s",
             tfilter ? (tfilter->param.c_str()) : "-t 5");
...
void mfilter_result(void* filter, int* is_spam) {
  ...
  if (tfilter->fd && tfilter->fd->from_child != -1) {
    ...
    while (1) {
      int read_len =
          read(tfilter->fd->from_child, result_buf, 1024 - total_len);
      if (read_len < 0) {
        ...
        if (memchr(result_buf, '\n', total_len) != NULL ||
            memchr(result_buf, '\r', total_len) != NULL)
          break;
  ...
  if (sscanf(result_buf, "%lf/%lf", &score, &threshold) == 2 &&
      threshold != 0.0 && score <= threshold) {
    tfilter->is_spam = 1;
  }
