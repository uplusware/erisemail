...
function output_dir(tblobj, pid, path, topusage, nodeObj, layer, begtr)
{
	...
	for(var i = 0; i < nodeObj.length; i++)
	{
		...
		if(nodeObj.item(i).tagName == "dir")
		{
			...
			{
				str_tr +="<td><img src=\"" + strIcon + "\"></td><td><a href=\"listmails.html?TOP_USAGE=trash&GPATH=" + encodeURIComponent(path) + "/" + encodeURIComponent(strDisplayDirName) + "&DIRID="+ nodeObj.item(i).getAttribute("id") +"\" target=\"childframe1\">" + strDisplayDirName + "</a></td><td><div name=\"UNSEEN\" id=\"UNSEEN\" did=\"" + nodeObj.item(i).getAttribute("id") + "\"></div></td><td><a href=\"javascript:empty_trash();\"> [清空]</a></td>\t\n";
