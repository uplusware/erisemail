...
function do_copy_mail(mid, todirs) {
    ...
    xmlHttp.onreadystatechange = function () {
        ...
        if (xmlHttp.readyState == 4 && xmlHttp.status == 200) {
            ...
            $id('PROCESSING_TBL').rows[0].cells[1].innerHTML = "Processing, please wait...";
                ...
                if (responseNode.tagName == "response") {
                    var errno = responseNode.getAttribute("errno")
                        if (errno == "0" || errno == 0) {
                            ...
                            $id("mailsubject").innerHTML = "<input class=\"text3\" size=\"110\" type=\"text\" value=\"" + (strSubject == "" ? "No Subject" : TextToHTML(strSubject)) + "\" readonly>";
                            ...
                            if (strCalendarContent == "") {
                                $id("mailcontent").innerHTML = (strHtmlContent != "" ? strHtmlContent : TextToHTML(strTextContent)) + (strPreview != "" ? ("<p></p><hr><b>" + att_count + " attachment(s)</b>" + strPreview) : "");
                            } else {
                                $id("timecol").style.display = "none";

                                $id("mailcontent").innerHTML = strCalendarContent + (strPreview != "" ? ("<p></p><hr><b>" + att_count + " attachment(s)</b>" + strPreview) : "");
                            }
                        } else {
                            $id("mailcontent").innerHTML = "<table border=\"0\"><tr><td><img src=\"alert.gif\"></td><td>Fail to load</td></tr></table>";
                        }
                }
            } else {
                $id("mailcontent").innerHTML = "<table border=\"0\"><tr><td><img src=\"alert.gif\"></td><td>Fail to load</td></tr></table>";
