...
function toggle_onbeforeunload(val)
{
	if(val)
	{
		window.onbeforeunload = function(e)
		{
			...
			{
				return ("邮件正在处理中，确实要退出吗？");
			}
			else if(isChange)
			{
				return ("邮件尚未发送或保存，确实要退出吗？");
			}
			else
			{
				return ("确实要退出吗？");
...
function init()
{
	...
	_$_('GLOBALPATH').innerHTML = (Request.QueryString('GPATH') == null? ("/草稿箱") : decodeURIComponent(Request.QueryString('GPATH')));
...
function do_savesent()
{
	...
	xmlHttp.onreadystatechange = function()
	{
			...
			if(xmlHttp.status == 200)
			{
				...
				if(responseNode.tagName == "response")
				{
					...
					if(errno == "0" || errno == 0)
					{
						...
						_$_('PROCESSING_TBL').rows[0].cells[0].innerHTML = "保存邮件到已发送邮件文件夹成功";
					...
					{
						_$_('PROCESSING_TBL').rows[0].cells[0].innerHTML = "<font color='#FF0000'>保存邮件到已发送邮件文件夹失败</font>";
						$( "#PROCESSING_DIV" ).dialog();
					}
				}
			}
		...
		else
		{
			...
			_$_('PROCESSING_TBL').rows[0].cells[0].innerHTML = "正在保存邮件到发件箱 - <img src=\"waiting.gif\" />";
			$( "#PROCESSING_DIV" ).dialog();
		}
	}
...
function do_send_mail()
{
	...
	xmlHttp.onreadystatechange = function()
	{
		if (xmlHttp.readyState==4)
		{
			...
			if(xmlHttp.status== 200)
			{
				...
				if(responseNode.tagName == "response")
				{
					...
					if(errno == "0")
					{
						isChange = false;
						toggle_onbeforeunload(isChange);
						_$_('PROCESSING_TBL').rows[0].cells[0].innerHTML = "发送邮件成功";
						$( "#PROCESSING_DIV" ).dialog();
					...
					{
						_$_('PROCESSING_TBL').rows[0].cells[0].innerHTML = "<font color='#FF0000'>发送邮件失败(邮件太大)</font>";
						$( "#PROCESSING_DIV" ).dialog();
					}
				}
			}
			...
			{
				_$_('PROCESSING_TBL').rows[0].cells[0].innerHTML = "<font color='#FF0000'>发送邮件失败</font>";
				$( "#PROCESSING_DIV" ).dialog();
			}
		}
		else
		{
			sending_status = true;
			toggle_onbeforeunload(sending_status);
			_$_('PROCESSING_TBL').rows[0].cells[0].innerHTML = "正在发送邮件 - <img src=\"waiting.gif\" />";
			$( "#PROCESSING_DIV" ).dialog();
		}
	}
	xmlHttp.open("POST", qUrl , true);
	...
	xmlHttp.onreadystatechange = function()
	{
		if(xmlHttp.readyState==4)
		{
			...
			if(xmlHttp.status== 200)
			{
				...
				if(responseNode.tagName == "response")
				{
					...
					if(errno == "0")
					{
						...
						_$_('PROCESSING_TBL').rows[0].cells[0].innerHTML = "保存草稿成功";
						$( "#PROCESSING_DIV" ).dialog();
					...
					else
					{
						...
						_$_('PROCESSING_TBL').rows[0].cells[0].innerHTML = "<font color='#FF0000'>保存草稿出错(邮件太大)</font>";
						$( "#PROCESSING_DIV" ).dialog();
			...
			else
			{
				...
				_$_('PROCESSING_TBL').rows[0].cells[0].innerHTML = "<font color='#FF0000'>保存草稿出错</font>";
				$( "#PROCESSING_DIV" ).dialog();
		...
		else
		{
			saving_status = true;
			toggle_onbeforeunload(saving_status);
			_$_('PROCESSING_TBL').rows[0].cells[0].innerHTML = "正在保存草稿 - <img src=\"waiting.gif\" />";
			$( "#PROCESSING_DIV" ).dialog();
		}
	}
				...
				if(responseNode.tagName == "response")
				{
					...
					if(errno == "0")
					{
						...
						else if(strType == "3")
						{
							...
							strReply += "<tr><td bgcolor='#EFEFEF' align='right' width='80'><b>发件人:</b></td><td bgcolor='#FFFFFF' align='left' width='910'>" + TextToHTML(strFrom) + "</td></tr>";
							strReply += "<tr><td bgcolor='#EFEFEF' align='right' width='80'><b>收件人:</b></td><td bgcolor='#FFFFFF' align='left' width='910'>" + TextToHTML(strTo) + "</td></tr>";
							if(strCc != "")
								strReply += "<tr><td bgcolor='#EFEFEF' align='right' width='80'><b>抄送:</b></td><td bgcolor='#FFFFFF' align='left' width='910'>" + TextToHTML(strCc) + "</td></tr>";
							strReply += "<tr><td bgcolor='#EFEFEF' align='right' width='80'><b>时间:</b></td><td bgcolor='#FFFFFF' align='left' width='910'>" + TextToHTML(strDate) + "</td></tr>";
							strReply += "<tr><td bgcolor='#EFEFEF' align='right' width='80'><b>主题:</b></td><td bgcolor='#FFFFFF' align='left' width='910'>" + TextToHTML(strSubject) + "</td></tr>";
							strReply += "</table>";
...
function do_copy_mail(mid, todirs)
{
	...
	xmlHttp.onreadystatechange = function()
	{
		...
		{
			_$_('PROCESSING_TBL').rows[0].cells[0].innerHTML = "正在复制，请等待...";
			$( "#PROCESSING_DIV" ).dialog();
		}
	}
	xmlHttp.open("GET", qUrl , true);
...
function do_move_mail(mid, todirs)
{
	...
	xmlHttp.onreadystatechange = function()
	{
		...
		{
			_$_('PROCESSING_TBL').rows[0].cells[0].innerHTML = "正在移动，请等待...";
			$( "#PROCESSING_DIV" ).dialog();
		}
